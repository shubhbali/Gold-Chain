...
const getMasterChefAbi = (chainId: number) => {
  return chainId === GOLD_CHAIN ? masterChefGoldChainABI : masterChefV2ABI
}
...
export const fetchFarmUserStakedBalances = async (
  ...
  const rawStakedBalances = await publicClient({ chainId }).multicall({
    contracts: farmsToFetch.map((farm) => {
      return {
        abi: isBscNetwork ? getMasterChefAbi(chainId) : crossFarmingVaultABI,
...
export const fetchFarmUserEarnings = async (
  ...
  const abi = getMasterChefAbi(multiCallChainId)
