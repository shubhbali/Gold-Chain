...
import { Currency, CurrencyAmount, NATIVE, Native, Price, Token, Trade, TradeType } from '@pancakeswap/sdk'
...
function parseCurrencyFromURLParameter(urlParam: any, nativeSymbol: string): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === nativeSymbol.toUpperCase()) return nativeSymbol
    if (valid === undefined) return nativeSymbol
  }
  return ''
}
...
const queryParametersToSwapState = (
  ...
  let inputCurrency = parseCurrencyFromURLParameter(parsedQs.inputCurrency, nativeSymbol) || nativeSymbol || DEFAULT_INPUT_CURRENCY
  let outputCurrency =
    parseCurrencyFromURLParameter(parsedQs.outputCurrency, nativeSymbol) || defaultOutputCurrency || DEFAULT_OUTPUT_CURRENCY
...
export const useDefaultsFromURLSearch = ():
  ...
  useEffect(() => {
    if (!chainId) return
    const nativeSymbol = NATIVE[chainId]?.symbol || DEFAULT_INPUT_CURRENCY
    const defaultOutputCurrency = CAKE[chainId]?.address || DEFAULT_OUTPUT_CURRENCY
    const parsed = queryParametersToSwapState(query, nativeSymbol, defaultOutputCurrency)
