import { ChainId, chainNames, chainNameToChainId, GOLD_CHAIN } from '@pancakeswap/chains'
import {
...
import { enum as enum_, literal, nativeEnum } from 'zod'

const allChainNames = Object.values(chainNames) as [string, ...string[]]

const zChain = nativeEnum(ChainId).or(enum_(allChainNames)).or(literal(GOLD_CHAIN))
...
const handler: NextApiHandler = async (req, res) => {
  ...
  try {
    ...
    const legacyFarmConfig =
      farmConfig.length > 0 ? await formatUniversalFarmToSerializedFarm(farmConfig) : await getLegacyFarmConfig(chainId as ChainId)
