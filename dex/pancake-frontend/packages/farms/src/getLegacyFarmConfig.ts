...
export async function getLegacyFarmConfig(chainId?: ChainId): Promise<SerializedFarmPublicData[]> {
  ...
  try {
    ...
    const loadUniversalConfig = supportedChainIdV4.includes(chainId as number)
    let universalConfig: UniversalFarmConfig[] = loadUniversalConfig ? await fetchUniversalFarms(chainId) : []
    const stablePools = loadUniversalConfig ? await getStableSwapPools(chainId) : []
    ...
    if (legacyFarmConfig && legacyFarmConfig.length > 0) {
      universalConfig = universalConfig.filter((f) => {
        return !!f.pid && !legacyFarmConfig.some((legacy) => legacy.lpAddress?.toLowerCase() === f.lpAddress?.toLowerCase())
      })
    }
