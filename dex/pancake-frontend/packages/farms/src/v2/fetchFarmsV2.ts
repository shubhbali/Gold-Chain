...
import { Address, PublicClient, formatUnits, zeroAddress } from 'viem'
...
> = {
  ...
  [GOLD_CHAIN]: {
    address: (process.env.NEXT_PUBLIC_GOLD_CHAIN_GILT_USDT_LP_ADDRESS || zeroAddress) as Address,
    wNative: 'GILT',
    stable: 'USDT',
  },
    ...
    try {
      return {
        ...
        ...getFarmAllocation({
          ...
          totalRegularAllocPoint: chainId === GOLD_CHAIN ? totalRegularAllocPoint : BigInt(2305),
          totalSpecialAllocPoint,
        }),
      }
    } catch (error) {
...
const masterChefFarmCalls = (farm: SerializedFarmConfig, masterChefAddress: string, chainId: number) => {
  const { pid } = farm
  const abi = isGoldChainMasterChef(chainId) ? masterChefGoldChainAbi : masterChefV2Abi
...
export const fetchMasterChefData = async (
  ...
  try {
    const masterChefCalls = farms.map((farm) => masterChefFarmCalls(farm, masterChefAddress, chainId))
    ...
    return masterChefCalls.map((masterChefCall) => {
      ...
      if (isGoldChainMasterChef(chainId)) {
        return {
...
export const fetchMasterChefV2Data = async ({
  ...
  try {
    if (isGoldChainMasterChef(chainId)) {
      const [poolLength, totalAllocPoint, cakePerBlock] = await provider({
        chainId,
      }).multicall({
        contracts: [
          {
            abi: masterChefGoldChainAbi,
