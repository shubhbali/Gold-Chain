...
import { Edge, Graph, InfinityRoute, InfinityTrade, TradeConfig, Vertice } from '../types'
...
export async function findBestTrade(params: FindBestTradeParams): Promise<InfinityTrade<TradeType> | undefined> {
  ...
  let bestTrade: InfinityTrade<TradeType> | undefined
...
async function getBestTrade({
  ...
  tradeType,
  maxHops = 3,
}: FindBestTradeParams): Promise<InfinityTrade<TradeType> | undefined> {
  const isExactIn = tradeType === TradeType.EXACT_INPUT
  ...
  const getQuoteAmount = (r: InfinityRoute) => (isExactIn ? r.outputAmount : r.inputAmount)

  const buildTrade = (g: Graph, routes: InfinityRoute[]): InfinityTrade<TradeType> => {
    const {
      gasUseEstimate,
      quoteAmount,
      gasUseEstimateQuote,
  ...
  async function findBestRoute(amount: CurrencyAmount<Currency>): Promise<Omit<InfinityRoute, 'percent'> | undefined> {
    ...
    for (;;) {
      ...
      for (const e of vert.edges) {
        ...
        try {
          ...
          const newQuote = adjustQuoteByGas(price.quote(quote) as CurrencyAmount<Currency>, gasSpentInQuote)
  ...
  const finalRoutes: InfinityRoute[] = []
