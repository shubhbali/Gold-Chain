...
import { ZERO_BD, factoryContract, ADDRESS_ZERO, ONE_BD, CAKE_ADDRESS } from "./utils";
  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHex() != ADDRESS_ZERO) {
      ...
      if (pair.token0 == token.id && pair.reserveBNB.gt(MINIMUM_LIQUIDITY_THRESHOLD_BNB)) {
        ...
        return pair.token1Price.times(token1.derivedBNB as BigDecimal); // return token1 per our token * GILT per token 1
      }
      if (pair.token1 == token.id && pair.reserveBNB.gt(MINIMUM_LIQUIDITY_THRESHOLD_BNB)) {
        let token0 = Token.load(pair.token0);
        return pair.token0Price.times(token0.derivedBNB as BigDecimal); // return token0 per our token * GILT per token 0
      }
    }
  }
