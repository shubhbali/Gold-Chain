...
import { BD_1E18, BD_ZERO, BI_ONE, DAR_BNB, TRACKED_TOKEN_BNB_PAIRS, TRACKED_TOKEN_BUSD_PAIRS } from "./utils";
...
export function handleSwap(event: Swap): void {
  let competition = Competition.load("4");
  ...
  if (event.address.toHex() == DAR_BNB) {
    user.darVolumeUSD = user.darVolumeUSD.plus(volumeUSD);
    user.darVolumeBNB = user.darVolumeBNB.plus(volumeBNB);
  }
