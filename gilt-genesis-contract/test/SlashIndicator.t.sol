...
contract SlashIndicatorTest is Deployer {
    ...
    function testMaintenance() public {
        vm.prank(validator0);
        giltValidatorSet.enterMaintenance();
        ...
        // exit maintenance
        vm.prank(validator0);
        giltValidatorSet.exitMaintenance();
        vm.roll(block.number + 1);
        vm.prank(coinbase);
        slashIndicator.slash(validator0);
    ...
    function testMisdemeanor() public {
        ...
        giltValidatorSet.deposit{ value: _deposit }(consensusAddrs[0]);
        assertEq(_incoming, giltValidatorSet.getIncoming(consensusAddrs[0]));
        ...
        assertEq(0, giltValidatorSet.getIncoming(consensusAddrs[0]));
        ...
        giltValidatorSet.updateValidatorSetV2(newVals, newVotingPowers, newVoteAddrs);

        giltValidatorSet.deposit{ value: 2 ether }(newVals[0]);
        assertEq(_incoming * 2, giltValidatorSet.getIncoming(newVals[0]));
        ...
        assertEq(MISDEMEANOR_THRESHOLD, count);
        assertEq(0, giltValidatorSet.getIncoming(newVals[0]));
        assertEq(_incoming, giltValidatorSet.getIncoming(newVals[1]));
        assertEq(_incoming, giltValidatorSet.getIncoming(newVals[2]));

        giltValidatorSet.deposit{ value: _deposit }(newVals[1]);
        assertEq(_incoming * 2, giltValidatorSet.getIncoming(newVals[1]));
        for (uint256 i; i < MISDEMEANOR_THRESHOLD; ++i) {
            vm.roll(block.number + 1);
            slashIndicator.slash(newVals[1]);
        }
        assertEq(_incoming, giltValidatorSet.getIncoming(newVals[0]));
        assertEq(0, giltValidatorSet.getIncoming(newVals[1]));
        assertEq(_incoming * 2, giltValidatorSet.getIncoming(newVals[2]));
        ...
        assertEq(_incoming * 2, giltValidatorSet.getIncoming(newVals[0]));
        assertEq(_incoming, giltValidatorSet.getIncoming(newVals[1]));
        assertEq(0, giltValidatorSet.getIncoming(newVals[2]));
        vm.stopPrank();
    }
    ...
    function testFelony() public {
        ...
        assertEq(0, count);
        assertEq(0, giltValidatorSet.getIncoming(consensusAddrs[0]));
        assertEq(_incoming, giltValidatorSet.getIncoming(consensusAddrs[1]));
        assertEq(_incoming, giltValidatorSet.getIncoming(consensusAddrs[2]));
