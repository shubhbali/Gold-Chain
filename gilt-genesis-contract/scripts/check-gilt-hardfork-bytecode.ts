...
contractNameMap[LIGHT_CLIENT_ADDR] = 'ReservedPredeployContract'

const TOKEN_HUB_ADDR = '0x0000000000000000000000000000000000001004';
contractNameMap[TOKEN_HUB_ADDR] = 'ReservedPredeployContract'

const INCENTIVIZE_ADDR = '0x0000000000000000000000000000000000001005';
contractNameMap[INCENTIVIZE_ADDR] = 'ReservedPredeployContract'

const RELAYERHUB_CONTRACT_ADDR = '0x0000000000000000000000000000000000001006';
contractNameMap[RELAYERHUB_CONTRACT_ADDR] = 'ReservedPredeployContract'
...
contractNameMap[TOKEN_MANAGER_ADDR] = 'ReservedPredeployContract'

const CROSS_CHAIN_CONTRACT_ADDR = '0x0000000000000000000000000000000000002000';
contractNameMap[CROSS_CHAIN_CONTRACT_ADDR] = 'ReservedPredeployContract'

const STAKING_CONTRACT_ADDR = '0x0000000000000000000000000000000000002001';
contractNameMap[STAKING_CONTRACT_ADDR] = 'ReservedPredeployContract'
...
contractNameMap[TOKEN_RECOVER_PORTAL_ADDR] = 'ReservedPredeployContract'

const STATE_RECEIVER_ADDR = '0x0000000000000000000000000000000000003001';
contractNameMap[STATE_RECEIVER_ADDR] = 'ReservedPredeployContract'

const NATIVE_GILT_BRIDGE_ADDR = '0x0000000000000000000000000000000000003002';
contractNameMap[NATIVE_GILT_BRIDGE_ADDR] = 'ReservedPredeployContract'

let hardforkName = process.env.HARDFORK
let giltUrl = process.env.GILT_URL
let giltRepoDir = '/tmp/gilt-chain/gilt'

const checkHardforkBytecode = async () => {
  const giltHardforkBytecodeDir = giltRepoDir + '/core/systemcontracts/' + hardforkName
  const mainnetDir = giltHardforkBytecodeDir + '/mainnet'
  const testnetDir = giltHardforkBytecodeDir + '/testnet'

  log('---------------------------------------------------------------------------')
  log(`Mainnet: compare genesis bytecode with gilt core repo`)
  ...
  log('---------------------------------------------------------------------------')
  log(`Testnet: compare genesis bytecode with gilt core repo`)
...
const compareGenesisWithHardforkBytecodes = async (genesisFile: string, files: string[]) => {
  ...
  for (const addr in genesis['alloc']) {
    ...
    const bytecodeFromGilt = getBytecodeFromGiltRepo(contractName, files)
    if (!bytecodeFromGilt) {
      log(`cannot find bytecode for ${contractName} in gilt core repo`)
      continue;
    }

    log('bytecode from gilt core repo:', bytecodeFromGilt.length, )

    if (bytecode === bytecodeFromGilt) {
      log('Success!')
    } else {
...
const getBytecodeFromGiltRepo = (contractName: string, files: string[]): string | undefined => {
...
const main = async () => {
  ...
  if (!giltUrl) {
    throw new Error('GILT_URL is required in .env')
  }

  hardforkName = hardforkName.trim()
  giltUrl = giltUrl.trim()

  const p= giltUrl.lastIndexOf('/')
  const commitId = giltUrl.substring(p+1)

  log('hardforkName', hardforkName, 'commitId', commitId)

  execSync(`cd /tmp && rm -rf gilt-chain && git clone https://github.com/chatzoneai-spec/Gold-Chain.git gilt-chain && cd gilt-chain && git checkout ${commitId}`)
