...
contract Staking is IStaking, System, IParamSubscriber, IApplication {
    ...
    modifier initParams() {
        if (!alreadyInit) {
            ...
            giltRelayerFee = INIT_GILT_RELAYER_FEE;
    ...
    /**
     * @dev Undelegate GILT from BC to GILT
     *
     * @param validator BC validator encoded address the user delegated
     * @param amount GILT amount the user undelegates
     */
    function undelegate(
        address validator,
        uint256 amount
    ...
    /**
     * @dev claim undelegated GILT from BC staking
     *
     */
