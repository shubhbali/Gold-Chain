...
import "../interface/0.6.x/IGiltValidatorSetV2.sol";
...
contract CrossChain is System, ICrossChain, IParamSubscriber {
    ...
    modifier onlyCabinet() {
        uint256 indexPlus = IGiltValidatorSetV2(VALIDATOR_CONTRACT_ADDR).currentValidatorSetMap(msg.sender);
        uint256 numOfCabinets = IGiltValidatorSetV2(VALIDATOR_CONTRACT_ADDR).numOfCabinets();
        if (numOfCabinets == 0) {
            numOfCabinets = 21;
        }
