...
contract SlashIndicator is ISlashIndicator, System, IParamSubscriber, IApplication {
    ...
    // The GILT validators assign proper values for `misdemeanorThreshold` and `felonyThreshold` through governance.
    // The proper values depends on GILT network's tolerance for continuous missing blocks.
    ...
    function slash(
        ...
        if (indicator.count % felonyThreshold == 0) {
            indicator.count = 0;
            IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).felony(validator);
            _downtimeSlash(validator, indicator.count, false);
        } else if (indicator.count % misdemeanorThreshold == 0) {
            IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).misdemeanor(validator);
        }
    ...
    function submitFinalityViolationEvidence(
        ...
        require(
            IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).isMonitoredForMaliciousVote(_evidence.voteAddr),
        ...
        (address[] memory vals, bytes[] memory voteAddrs) =
            IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).getLivingValidators();
        for (uint256 i; i < voteAddrs.length; ++i) {
            if (BytesLib.equal(voteAddrs[i], _evidence.voteAddr)) {
                ...
                IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).felony(vals[i]);
                break;
            }
        }
    ...
    function submitDoubleSignEvidence(
        ...
        elements[0] = giltChainID.encodeUint();
        ...
        IGiltValidatorSet(VALIDATOR_CONTRACT_ADDR).felony(signer);
    ...
    function encodeSlashPackage(
        ...
        elements[2] = uint256(giltChainID).encodeUint();
