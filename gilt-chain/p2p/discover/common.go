...
func ParseEthFilter(chain string) (NodeFilterFunc, error) {
	...
	switch chain {
	case "gilt":
		filter = forkid.NewStaticFilter(params.GILTChainConfig, core.DefaultGILTGenesisBlock().ToBlock())
	case "testnet":
...
func GetEthEntry(chain string) (enr.Entry, error) {
	...
	switch chain {
	case "gilt":
		eth.ForkID = forkid.NewID(params.GILTChainConfig, core.DefaultGILTGenesisBlock().ToBlock(), uint64(0), uint64(0))
	case "testnet":
