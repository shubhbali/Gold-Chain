...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	eth.engine, err = ethconfig.CreateConsensusEngine(config, chainConfig, chainDb, ethAPI, genesisHash)
	if err != nil {
		return nil, err
	}
...
func (s *Ethereum) Protocols() []p2p.Protocol {
	...
	protos = append(protos, gilt.MakeProtocols((*giltHandler)(s.handler))...)
...
func (s *Ethereum) setupDiscovery() error {
	...
	// Add gilt nodes from DNS.
	if len(s.config.GiltDiscoveryURLs) > 0 {
		iter, err := dnsclient.NewIterator(s.config.GiltDiscoveryURLs...)
		if err != nil {
			return err
		}
		s.discmix.AddSource(iter)
