...
type ethPeer struct {
	*eth.Peer
	...
	giltExt *giltPeer // Satellite `gilt` connection
}
...
// giltPeerInfo represents a short summary of the `gilt` sub-protocol metadata known
// about a connected peer.
type giltPeerInfo struct {
	Version uint `json:"version"` // gilt protocol version negotiated
}
...
// giltPeer is a wrapper around gilt.Peer to maintain a few extra metadata.
type giltPeer struct {
	*gilt.Peer
}
...
// info gathers and returns some `gilt` protocol metadata known about a peer.
func (p *giltPeer) info() *giltPeerInfo {
	return &giltPeerInfo{
		Version: p.Version(),
	}
}
