...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `gilt` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `gilt`.
...
// Handle is the callback invoked to manage the life cycle of a `gilt` peer.
// When this function terminates, the peer is disconnected.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `gilt`", "err", err)
			return err
		}
	}
}
...
var gilt1 = map[uint64]msgHandler{
	GiltCapMsg: handleGiltCap, // ignore capability message for backward compatibility
	VotesMsg:   handleVotes,
}

var gilt2 = map[uint64]msgHandler{
	GiltCapMsg:          handleGiltCap, // ignore capability message for backward compatibility
	VotesMsg:            handleVotes,
...
// handleGiltCap ignores the capability message for backward compatibility.
// Old nodes send GiltCapMsg as part of their handshake, we just ignore it
// since P2P layer already negotiated the protocol version.
func handleGiltCap(backend Backend, msg Decoder, peer *Peer) error {
	// Decode the message to consume it, but ignore the content
	var cap GiltCapPacket
...
// remote peer on the `gilt` protocol. The remote connection is torn down upon
// returning any error.
...
// NodeInfo represents a short summary of the `gilt` sub-protocol metadata
// known about the host peer.
type NodeInfo struct{}

// nodeInfo retrieves some `gilt` protocol metadata about the running host node.
