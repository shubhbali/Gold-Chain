...
const (
	Gilt1 = 1
	Gilt2 = 2
	Gilt3 = 3 // to BAL process
)

// ProtocolName is the official short name of the `gilt` protocol used during
// devp2p capability negotiation.
const ProtocolName = "gilt"

// ProtocolVersions are the supported versions of the `gilt` protocol (first
// is primary).
var ProtocolVersions = []uint{Gilt1, Gilt2, Gilt3}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{Gilt1: 2, Gilt2: 4, Gilt3: 4}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	GiltCapMsg          = 0x00 // gilt capability msg used upon handshake
	VotesMsg            = 0x01
...
// Packet represents a p2p message in the `gilt` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// GiltCapPacket is the network packet for gilt capability message.
type GiltCapPacket struct {
...
func (*GiltCapPacket) Name() string { return "GiltCap" }
func (*GiltCapPacket) Kind() byte   { return GiltCapMsg }
