...
var (
	...
	// errGiltWithoutEth is returned if a peer attempts to connect only on the
	// gilt protocol without advertising the eth main protocol.
	errGiltWithoutEth = errors.New("peer connected on gilt without compatible eth support")
)
...
type peerSet struct {
	...
	giltWait map[string]chan *gilt.Peer // Peers connected on `eth` waiting for their gilt extension
	giltPend map[string]*gilt.Peer      // Peers connected on the `gilt` protocol, but not yet on `eth`
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		giltWait: make(map[string]chan *gilt.Peer),
		giltPend: make(map[string]*gilt.Peer),
...
// registerGiltExtension unblocks an already connected `eth` peer waiting for its
// `gilt` extension, or if no such peer exists, tracks the extension for the time
// being until the `eth` main protocol starts looking for it.
func (ps *peerSet) registerGiltExtension(peer *gilt.Peer) error {
	// Reject the peer if it advertises `gilt` without `eth` as `gilt` is only a
	// satellite protocol meaningful with the chain selection of `eth`
	if !peer.RunningCap(eth.ProtocolName, eth.ProtocolVersions) {
		return errGiltWithoutEth
	}
	// Ensure nobody can double connect
	ps.lock.Lock()
	defer ps.lock.Unlock()
	...
	if _, ok := ps.giltPend[id]; ok {
		return errPeerAlreadyRegistered // avoid connections with the same id as pending ones
	}
	// Inject the peer into an `eth` counterpart is available, otherwise save for later
	if wait, ok := ps.giltWait[id]; ok {
		delete(ps.giltWait, id)
		wait <- peer
		return nil
	}
	ps.giltPend[id] = peer
...
// waitGiltExtension blocks until all satellite protocols are connected and tracked
// by the peerset.
func (ps *peerSet) waitGiltExtension(peer *eth.Peer) (*gilt.Peer, error) {
	// If the peer does not support a compatible `gilt`, don't wait
	if !peer.RunningCap(gilt.ProtocolName, gilt.ProtocolVersions) {
		return nil, nil
	}
	// Ensure nobody can double connect
	ps.lock.Lock()
	...
	if _, ok := ps.giltWait[id]; ok {
		ps.lock.Unlock()
		return nil, errPeerAlreadyRegistered // avoid connections with the same id as pending ones
	}
	// If `gilt` already connected, retrieve the peer from the pending set
	if gilt, ok := ps.giltPend[id]; ok {
		delete(ps.giltPend, id)

		ps.lock.Unlock()
		return gilt, nil
	}
	// Otherwise wait for `gilt` to connect concurrently
	wait := make(chan *gilt.Peer)
	ps.giltWait[id] = wait
	ps.lock.Unlock()
...
func (ps *peerSet) registerPeer(peer *eth.Peer, ext *snap.Peer, giltExt *gilt.Peer) error {
...
func (ps *peerSet) peersWithoutVote(hash common.Hash) []*ethPeer {
	...
	list := make([]*ethPeer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if p.giltExt != nil && !p.giltExt.KnownVote(hash) {
			list = append(list, p)
		}
	}
	return list
