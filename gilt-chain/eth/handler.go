...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	fetchRangeBlocks := func(peer string, startHeight uint64, startHash common.Hash, count uint64) ([]*types.Block, error) {
		...
		if p.giltExt == nil {
			return nil, fmt.Errorf("peer does not support gilt protocol, peer: %v", p.ID())
		}
		if p.giltExt.Version() < gilt.Gilt2 {
			return nil, fmt.Errorf("remote peer does not support the required Gilt2 protocol version, peer: %v", p.ID())
		}
		res, err := p.giltExt.RequestBlocksByRange(startHeight, startHash, count)
		if err != nil {
			return nil, err
		}
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	giltExt, err := h.peers.waitGiltExtension(peer)
	if err != nil {
		peer.Log().Error("Gilt extension barrier failed", "err", err)
		return err
	}
	if giltExt != nil && giltExt.Version() == gilt.Gilt3 {
		peer.CanHandleBAL.Store(true)
		log.Debug("runEthPeer", "giltExt.Version", giltExt.Version(), "CanHandleBAL", peer.CanHandleBAL.Load())
	}
	...
	if err := h.peers.registerPeer(peer, snap, giltExt); err != nil {
	...
	if h.votepool != nil && p.giltExt != nil {
		h.syncVotes(p.giltExt)
	}
...
// runGiltExtension registers a `gilt` peer into the joint eth/gilt peerset and
// starts handling inbound messages. As `gilt` is only a satellite protocol to
// `eth`, all subsystem registrations and lifecycle management will be done by
// the main `eth` handler to prevent strange races.
func (h *handler) runGiltExtension(peer *gilt.Peer, handler gilt.Handler) error {
	if !h.incHandlers() {
		return p2p.DiscQuitting
	}
	defer h.decHandlers()

	if err := h.peers.registerGiltExtension(peer); err != nil {
		if metrics.Enabled() {
			if peer.Inbound() {
				gilt.IngressRegistrationErrorMeter.Mark(1)
			} else {
				gilt.EgressRegistrationErrorMeter.Mark(1)
			}
		}
		peer.Log().Error("Gilt extension registration failed", "err", err, "name", peer.Name())
		return err
	}
	return handler(peer)
}
...
func (h *handler) BroadcastVote(vote *types.VoteEnvelope) {
	...
	for peer, _vote := range voteMap {
		...
		peer.giltExt.AsyncSendVotes(votes)
	}
