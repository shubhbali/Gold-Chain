	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.IsInGILT())) {
...
func (w *worker) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	if w.chainConfig.IsLondon(header.Number) {
		...
		if w.chainConfig.IsNotInGILT() && !w.chainConfig.IsLondon(parent.Number) {
...
func (w *worker) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	var requests [][]byte
	if w.chainConfig.IsPrague(work.header.Number, work.header.Time) && w.chainConfig.IsNotInGILT() {
		requests = [][]byte{}
		// EIP-6110 deposits
