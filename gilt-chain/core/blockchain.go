...
func DefaultConfig() *BlockChainConfig {
	return &BlockChainConfig{
		...
		SnapshotLimit:    256,
		SnapshotWait:     true,
		ChainHistoryMode: history.KeepAll, // only `history.KeepAll` supported for Gilt Chain
...
func (bc *BlockChain) cacheReceipts(hash common.Hash, receipts types.Receipts, block *types.Block) {
	...
	// Please check details in the Gold Chain issue tracker (legacy upstream note).
...
func (bc *BlockChain) initializeHistoryPruning(latest uint64) error {
	...
	case history.KeepAll:
		// TODO(Nathan): Gilt Chain currently supports `history.blocks`, but enabling history pruning
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []rlp.RawValue, ancientLimit uint64) (int, error) {
	...
	if bc.chainConfig.IsInGILT() && bc.chainConfig.IsCancun(lastBlk.Number(), lastBlk.Time()) {
