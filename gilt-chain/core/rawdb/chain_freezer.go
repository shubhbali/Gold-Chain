...
const (
	...
	// TODO(galaio): For Gilt Chain, the 0.75 interval and freezing of 30,000 blocks will seriously affect performance.
...
func (f *chainFreezer) freeze(db ethdb.KeyValueStore, continueFreeze bool) {
	...
	for {
		...
		// Gilt Chain does not use the finalized block as the freeze indicator, for two reasons:
		...
		// TODO(galaio): Temporarily comment that the current Gilt Chain setup is suitable for small-volume writes,
