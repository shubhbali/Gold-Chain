...
func getGenesisState(db ethdb.Database, blockhash common.Hash) (alloc types.GenesisAlloc, err error) {
	...
	case params.GILTGenesisHash:
		genesis = DefaultGILTGenesisBlock()
...
// Typically, these modifications involve hardforks that are not enabled on Gilt Chain mainnet, intended for testing purposes.
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
	...
	if (ghash == common.Hash{}) {
		if genesis == nil {
			log.Info("Writing default Gilt Chain mainnet genesis block")
			genesis = DefaultGILTGenesisBlock()
		} else {
...
func LoadChainConfig(db ethdb.Database, genesis *Genesis) (cfg *params.ChainConfig, ghash common.Hash, err error) {
	...
	return params.GILTChainConfig, params.GILTGenesisHash, nil
}
...
func (g *Genesis) toBlockWithRoot(root common.Hash) *types.Block {
	...
	if g.Config != nil && g.Config.IsLondon(common.Big0) {
		if g.BaseFee != nil {
			...
			if g.Config.IsInGILT() {
				head.BaseFee = new(big.Int).SetUint64(params.InitialBaseFeeForGILT)
			} else {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsNotInGILT() && conf.IsShanghai(num, g.Timestamp) {
		...
		if conf.IsCancun(num, g.Timestamp) {
			...
			// by definition.
			if conf.IsNotInGILT() || conf.IsBohr(num, g.Timestamp) {
...
// DefaultGILTGenesisBlock returns the Gilt Chain mainnet genesis block.
func DefaultGILTGenesisBlock() *Genesis {
	alloc := decodePrealloc(giltMainnetAllocData)
	return &Genesis{
...
// DefaultChapelGenesisBlock returns the Gilt Chain Testnet genesis block.
func DefaultChapelGenesisBlock() *Genesis {
	alloc := decodePrealloc(giltChapelAllocData)
	return &Genesis{
