...
func testSetupGenesis(t *testing.T, scheme string) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantHash:   params.GILTGenesisHash,
			wantConfig: params.GILTChainConfig,
		},
		{
			name: "mainnet block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				DefaultGILTGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), nil)
			},
			wantHash:   params.GILTGenesisHash,
			wantConfig: params.GILTChainConfig,
		...
		{
			name: "testnet block in DB, genesis == testnet",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				DefaultChapelGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
...
func TestConfigOrDefault(t *testing.T) {
	...
	gHash := params.GILTGenesisHash
	config := defaultGenesis.chainConfigOrDefault(gHash, nil)

	if config.ChainID.Cmp(params.GILTChainConfig.ChainID) != 0 {
		t.Errorf("ChainID of resulting config should be %v, but is %v instead", params.GILTChainConfig.ChainID, config.ChainID)
	}

	if config.HomesteadBlock.Cmp(params.GILTChainConfig.HomesteadBlock) != 0 {
		t.Errorf("resulting config should have HomesteadBlock = %v, but instead is %v", params.GILTChainConfig, config.HomesteadBlock)
	}

	if config.PlanckBlock == nil {
		t.Errorf("resulting config should have PlanckBlock = %v , but instead is nil", params.GILTChainConfig.PlanckBlock)
	}

	if config.PlanckBlock.Cmp(params.GILTChainConfig.PlanckBlock) != 0 {
		t.Errorf("resulting config should have PlanckBlock = %v , but instead is %v", params.GILTChainConfig.PlanckBlock, config.PlanckBlock)
	}
}
