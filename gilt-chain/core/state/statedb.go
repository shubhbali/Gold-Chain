...
type StateDB struct {
	...
	// if needBadSharedStorage = true, try read from sharedPool firstly, compatible with old erroneous data(https://forum.giltchain.org/t/about-the-hertzfix/2400).
...
func (s *StateDB) handleDestruction(noStorageWiping bool) (map[common.Hash]*accountDelete, []*trienode.NodeSet, error) {
	...
	for addr, prevObj := range s.stateObjectsDestruct {
		...
		//    a. EIP-158 is active from Gilt Chain genesis, ensuring that "empty accounts" are
