...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	if !p.chain.Config().IsOsaka(oldHead.Number, oldHead.Time) && p.chain.Config().IsOsaka(newHead.Number, newHead.Time) && p.chain.Config().IsNotInGILT() {
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	head := p.head.Load()
	if p.chain.Config().IsOsaka(head.Number, head.Time) && tx.BlobTxSidecar().Version == types.BlobSidecarVersion0 && p.chain.Config().IsNotInGILT() {
		if err := tx.BlobTxSidecar().ToV1(); err != nil {
...
func (p *BlobPool) preCheck(tx *types.Transaction) error {
	...
	if !isOsaka || p.chain.Config().IsInGILT() {
