...
for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_validator "gilt-validator${i}"
done
...
init_genesis_data gilt-rpc gilt-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_genesis_data validator "gilt-validator${i}"
done

#Step 4, prepare bls wallet, used by fast finality vote
prepareBLSWallet gilt-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     prepareBLSWallet "gilt-validator${i}"
done