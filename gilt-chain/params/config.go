...
var (
	...
	GILTGenesisHash   = common.HexToHash("0x0d21840abff46b96c84b2ac9e10e4f5cdaeb5693cb665db62a2f3b02d2d57b5b")
...
var (
	...
	GILTChainConfig = &ChainConfig{
		...
		BlobScheduleConfig: &BlobScheduleConfig{
			Cancun: DefaultCancunBlobConfig,
			Prague: DefaultPragueBlobConfigGILT,
			Osaka:  DefaultOsakaBlobConfigGILT,
		},
	}
...
func GetBuiltInChainConfig(ghash common.Hash) *ChainConfig {
	...
	case GILTGenesisHash:
		return GILTChainConfig
...
var (
	...
	DefaultPragueBlobConfigGILT = DefaultCancunBlobConfig
	DefaultOsakaBlobConfigGILT  = DefaultCancunBlobConfig
)

// NetworkNames are user friendly names to use in the chain spec banner.
var NetworkNames = map[string]string{
	MainnetChainConfig.ChainID.String(): "mainnet",
	GILTChainConfig.ChainID.String():    "gilt",
	ChapelChainConfig.ChainID.String():  "testnet",
...
type ChainConfig struct {
	...
	// Gold Chain final launch configs set this to block inherited BSC/GILT
	// historical system-contract bytecode upgrades from executing on a fresh
	// production genesis.
	DisableHistoricalSystemContractUpgrades bool `json:"disableHistoricalSystemContractUpgrades,omitempty"`
...
func (c *ChainConfig) IsInGILT() bool {
	return c.Parlia != nil
}

func (c *ChainConfig) IsNotInGILT() bool {
	return c.Parlia == nil
}
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	gilt := c.BlobScheduleConfig
	if gilt == nil {
		gilt = new(BlobScheduleConfig)
	}
	for _, cur := range []struct {
		...
		{name: "cancun", timestamp: c.CancunTime, config: gilt.Cancun},
		{name: "prague", timestamp: c.PragueTime, config: gilt.Prague},
		{name: "osaka", timestamp: c.OsakaTime, config: gilt.Osaka},
		{name: "bpo1", timestamp: c.BPO1Time, config: gilt.BPO1},
		{name: "bpo2", timestamp: c.BPO2Time, config: gilt.BPO2},
		{name: "bpo3", timestamp: c.BPO3Time, config: gilt.BPO3},
		{name: "bpo4", timestamp: c.BPO4Time, config: gilt.BPO4},
		{name: "bpo5", timestamp: c.BPO5Time, config: gilt.BPO5},
		{name: "amsterdam", timestamp: c.AmsterdamTime, config: gilt.Amsterdam},
	} {
		if cur.config != nil {
...
func (c *ChainConfig) Rules(num *big.Int, isMerge bool, timestamp uint64) Rules {
	...
	isMerge = isMerge && c.IsLondon(num) // always false in Gilt Chain
