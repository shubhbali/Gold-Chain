...
// Parlia is the consensus engine of Gilt Chain
type Parlia struct {
...
func (p *Parlia) EstimateGasReservedForSystemTxs(chain consensus.ChainHeaderReader, header *types.Header) uint64 {
	parent := chain.GetHeaderByHash(header.ParentHash)
	if parent != nil {
		// Mainnet and Testnet have both passed Feynman. Now, simplify the logic before and during the Feynman hard fork.
...
func (p *Parlia) distributeIncoming(val common.Address, state vm.StateDB, header *types.Header, chain core.ChainContext,
	...
	state.SetBalance(consensus.SystemAddress, common.U2560, tracing.BalanceDecreaseGILTDistributeReward)
	state.AddBalance(coinbase, balance, tracing.BalanceIncreaseGILTDistributeReward)
