...
var (
	...
	wsEndpointMainnet = flag.String("ws.mainnet", "", "URL to ws endpoint of Gilt Chain mainnet")
	...
	minMainnetBalance = big.NewInt(2 * 1e6 * params.GWei) // 0.002 GILT
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("0.%s GILTs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type faucet struct {
	...
	clientMainnet *ethclient.Client   // Client connection to Gilt Chain mainnet for balance check
		...
		if f.clientMainnet != nil {
			...
			if err != nil {
				...
				if balanceMainnet.Cmp(minMainnetBalance) < 0 {
					f.lock.Unlock()
					log.Warn("insufficient GILT on Gilt Chain mainnet", "address", mainnetAddr,
					...
					if err = sendError(wsconn, fmt.Errorf("insufficient GILT on Gilt Chain mainnet (require >=%s GILT)",
...
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Gilt Chain address found to fund")
	}
	var avatar string
...
func authFacebook(url string) (string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Gilt Chain address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
