...
var (
	...
	BridgeHeimdallURLFlag = &cli.StringFlag{
		...
		Usage:    "Heimdall-compatible bridge API base URL for state sync events",
		Category: flags.EthCategory,
	}
	BridgeStateReceiverFlag = &cli.StringFlag{
		Name:     "bridge.state-receiver",
		Usage:    "Override StateReceiver contract address for bridge state sync events",
		Category: flags.EthCategory,
	}
	BridgeStateSyncTimeoutFlag = &cli.DurationFlag{
		Name:     "bridge.state-sync-timeout",
		Usage:    "HTTP timeout for fetching bridge state sync events",
		Category: flags.EthCategory,
	}
	BridgeStateSyncDelayFlag = &cli.DurationFlag{
		Name:     "bridge.state-sync-delay",
		Usage:    "Delay before Gold Chain consumes bridge state sync events",
		Category: flags.EthCategory,
	}
	...
	GILTMainnetFlag = &cli.BoolFlag{
		Name:     "mainnet",
		Usage:    "Gilt Chain mainnet",
		Category: flags.EthCategory,
	}
	ChapelFlag = &cli.BoolFlag{
		Name:     "testnet",
		Usage:    "Testnet network: pre-configured Proof-of-Stake-Authority Gilt Chain test network",
		Category: flags.EthCategory,
	}
...
var (
	...
	NetworkFlags = append([]cli.Flag{GILTMainnetFlag}, TestnetFlags...)
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	flags.CheckExclusive(ctx, GILTMainnetFlag, DeveloperFlag, NetworkIdFlag, OverrideGenesisFlag)
	...
	if ctx.IsSet(NoDiscoverFlag.Name) {
		cfg.EthDiscoveryURLs, cfg.SnapDiscoveryURLs, cfg.GiltDiscoveryURLs = []string{}, []string{}, []string{}
	...
	switch {
	case ctx.Bool(GILTMainnetFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 56
		}
		cfg.Genesis = core.DefaultGILTGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.GILTGenesisHash)
...
func SetDNSDiscoveryDefaults(cfg *ethconfig.Config, genesis common.Hash) {
	...
	if url := params.KnownDNSNetwork(genesis, protocol); url != "" {
		...
		cfg.GiltDiscoveryURLs = cfg.EthDiscoveryURLs
	}
}
...
func parseNodeType() string {
	...
	infos := []string{"GILT", strings.Join(version, "-"), strings.Join(arch, "-"), runtime.Version()}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	switch {
	case ctx.Bool(GILTMainnetFlag.Name):
		genesis = core.DefaultGILTGenesisBlock()
