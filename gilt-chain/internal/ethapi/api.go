...
func (api *BlockChainAPI) GetBlobSidecarByTxHash(ctx context.Context, hash common.Hash, fullBlob *bool) (map[string]interface{}, error) {
	...
	txTarget, blockHash, _, Index := rawdb.ReadCanonicalTransaction(api.b.ChainDb(), hash)
	if txTarget == nil {
		return nil, nil
	}
	block, err := api.b.BlockByHash(ctx, blockHash)
...
// GetTdByNumber returns the total difficulty for a block number using the legacy bridge-compatible RPC shape.
...
func (api *TransactionAPI) currentBlobSidecarVersion() byte {
	...
	if api.b.ChainConfig().IsOsaka(h.Number, h.Time) && api.b.ChainConfig().IsNotInGILT() {
