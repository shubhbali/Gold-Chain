...
func (args *TransactionArgs) setFeeDefaults(ctx context.Context, b Backend, head *types.Header) error {
	...
	if args.GasPrice == nil && eip1559ParamsSet {
		if args.MaxFeePerGas.ToInt().Sign() == 0 {
			...
			log.Warn("EIP-1559 Tx with zero maxFeePerGas") // Gilt Chain accepts zero gas price.
		}
	...
	if args.GasPrice != nil && !eip1559ParamsSet {
		...
		if args.GasPrice.ToInt().Sign() == 0 && isLondon {
			...
			log.Warn("non EIP-1559 Tx with zero gasPrice") // Gilt Chain accepts zero gas price.
		}
