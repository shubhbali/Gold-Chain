...
contract('GiltValidatorSet', async (accounts) => {
    describe('Initial values', async () => {
        let testBVS

        before(async function () {
            testBVS = await TestGiltValidatorSet.deployed()
        })
        ...
        it('Validator set be default', async () => {
            const validators = await testBVS.getGiltValidators(255)
    ...
    describe('commitSpan()', async () => {
        ...
        it('Producer set for span #1', async () => {
            const producers = await testBVS.getGiltValidators(500)
