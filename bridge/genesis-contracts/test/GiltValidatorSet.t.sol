...
import "./helpers/IGiltValidatorSet.sol";

contract GiltValidatorSetTest is Test {
    uint8 constant LIST_SHORT_START = 0xc0;
    address public constant SYSTEM_ADDRESS = 0xffffFFFfFFffffffffffffffFfFFFfffFFFfFFfE;
    bytes32 public constant CHAIN = keccak256("giltconsensus-15001");
    bytes32 public constant ROUND_TYPE = keccak256("vote");
    bytes32 public constant GILT_ID = keccak256("15001");
    ...
    IGiltValidatorSet internal giltValidatorSet;

    function setUp() public {
        _newSpan = 1;
        giltValidatorSet = IGiltValidatorSet(deployCode("out/GiltValidatorSet.sol/GiltValidatorSet.json"));
    }

    function testConstants() public {
        assertEq(giltValidatorSet.CHAIN(), CHAIN);
        assertEq(giltValidatorSet.ROUND_TYPE(), ROUND_TYPE);
        assertEq(giltValidatorSet.GILT_ID(), GILT_ID);
        assertEq(giltValidatorSet.VOTE_TYPE(), VOTE_TYPE);
        assertEq(giltValidatorSet.FIRST_END_BLOCK(), FIRST_END_BLOCK);
        assertEq(giltValidatorSet.SPRINT(), SPRINT);
    }

    function test_currentSprint() public {
        assertEq(giltValidatorSet.currentSprint(), 0);
        vm.roll(SPRINT);
        assertEq(giltValidatorSet.currentSprint(), 1);
        vm.roll(SPRINT * 2 + 1);
        assertEq(giltValidatorSet.currentSprint(), 2);
    }

    function test_getInitialValidators() public {
        (address[] memory addresses, uint256[] memory powers) = giltValidatorSet.getInitialValidators();
    ...
    function testRevert_commitSpan_OnlySystem() public {
        ...
        giltValidatorSet.commitSpan(0, 0, 0, "", "");
    }
    ...
    function testRevert_commitSpan_InvalidEndBlock() public {
        ...
        vm.prank(SYSTEM_ADDRESS);
        giltValidatorSet.commitSpan(1, 0, 0, "", "");
    }
    ...
    function testRevert_commitSpan_IncompleteSprint() public {
        ...
        vm.prank(SYSTEM_ADDRESS);
        giltValidatorSet.commitSpan(1, 0, 64 * 2, "", "");
    }
    ...
    function test_commitSpan() public {
        ...
        vm.prank(SYSTEM_ADDRESS);
        giltValidatorSet.commitSpan(newSpan, startBlock, endBlock, validatorBytes, producerBytes);

        (uint256 number_, uint256 startBlock_, uint256 endBlock_) = giltValidatorSet.spans(0);
        assertEq(number_, 0);
        assertEq(startBlock_, 0);
        assertEq(endBlock_, FIRST_END_BLOCK);
        assertEq(giltValidatorSet.spanNumbers(0), 0);
        (uint256 id, uint256 power, address signer) = giltValidatorSet.validators(0, 0);
        (address[] memory initialAddresses, uint256[] memory initialPowers) = giltValidatorSet.getInitialValidators();
        assertEq(id, 0);
        assertEq(power, initialPowers[0]);
        assertEq(signer, initialAddresses[0]);
        vm.expectRevert();
        giltValidatorSet.validators(0, 1);
        (id, power, signer) = giltValidatorSet.producers(0, 0);
        assertEq(id, 0);
        assertEq(power, initialPowers[0]);
        assertEq(signer, initialAddresses[0]);
        vm.expectRevert();
        giltValidatorSet.producers(0, 1);
        (number_, startBlock_, endBlock_) = giltValidatorSet.spans(newSpan);
        assertEq(number_, newSpan);
        assertEq(startBlock_, startBlock);
        assertEq(endBlock_, endBlock);
        assertEq(giltValidatorSet.spanNumbers(1), newSpan);
        for (uint256 i = 0; i < numOfValidators; i++) {
            (id, power, signer) = giltValidatorSet.validators(newSpan, i);
            assertEq(id, ids[i]);
            assertEq(power, powers[i]);
            assertEq(signer, signers[i]);
            if (i >= numOfProducers) continue;
            (id, power, signer) = giltValidatorSet.producers(newSpan, i);
            assertEq(id, ids[i]);
            assertEq(power, powers[i]);
            assertEq(signer, signers[i]);
        }
        vm.expectRevert();
        giltValidatorSet.validators(newSpan, numOfValidators);
        vm.expectRevert();
        giltValidatorSet.producers(newSpan, numOfProducers);
    }
    ...
    function testRevert_commitSpan_StartLesserThanCurrentSpanStart() public {
        ...
        vm.prank(SYSTEM_ADDRESS);
        giltValidatorSet.commitSpan(2, 0, 64 * 2 - 1, "", "");
    }
    ...
    function test_getSpan() public {
        _commitSpan();
        (uint256 number, uint256 startBlock, uint256 endBlock) = giltValidatorSet.getSpan(0);
        assertEq(number, 0);
        assertEq(startBlock, 0);
        assertEq(endBlock, FIRST_END_BLOCK);
        (number, startBlock, endBlock) = giltValidatorSet.getSpan(1);
        assertEq(number, 1);
    ...
    function test_getSpanByBlock() public {
        assertEq(giltValidatorSet.getSpanByBlock(1_234_567_890), 0);
        _commitSpan();
        _commitSpan(1000);
        assertEq(giltValidatorSet.getSpanByBlock(0), 0);
        assertEq(giltValidatorSet.getSpanByBlock(100), 1);
        assertEq(giltValidatorSet.getSpanByBlock(1001), 2);
        assertEq(giltValidatorSet.getSpanByBlock(1_234_567_890), 2);
    }

    function test_currentSpanNumber() public {
        vm.roll(1_234_567_890);
        assertEq(giltValidatorSet.currentSpanNumber(), 0);
        _commitSpan();
        _commitSpan(1000);
        vm.roll(0);
        assertEq(giltValidatorSet.currentSpanNumber(), 0);
        vm.roll(100);
        assertEq(giltValidatorSet.currentSpanNumber(), 1);
        vm.roll(1001);
        assertEq(giltValidatorSet.currentSpanNumber(), 2);
        vm.roll(1_234_567_890);
        assertEq(giltValidatorSet.currentSpanNumber(), 2);
    }
    ...
    function test_getCurrentSpan() public {
        _commitSpan();
        (uint256 number, uint256 startBlock, uint256 endBlock) = giltValidatorSet.getCurrentSpan();
        assertEq(number, 1);
        assertEq(startBlock, 1);
        assertEq(endBlock, 64 * 2);
    }
    ...
    function test_getNextSpan() public {
        _commitSpan();
        vm.roll(0);
        (uint256 number, uint256 startBlock, uint256 endBlock) = giltValidatorSet.getNextSpan();
        assertEq(number, 1);
        assertEq(startBlock, 1);
        assertEq(endBlock, 64 * 2);
    }
    ...
    function test_getValidatorsTotalStakeBySpan() public {
        ...
        assertEq(giltValidatorSet.getValidatorsTotalStakeBySpan(1), totalStake);
    }
    ...
    function test_getProducersTotalStakeBySpan() public {
        ...
        assertEq(giltValidatorSet.getProducersTotalStakeBySpan(1), totalStake);
    }

    function test_getValidatorBySigner() public {
        (uint256[] memory ids, uint256[] memory powers, address[] memory signers) = _commitSpan();
        Validator memory validator = giltValidatorSet.getValidatorBySigner(1, signers[4]);
    ...
    function test_isValidator() public {
        (uint256[] memory ids, uint256[] memory powers, address[] memory signers) = _commitSpan();
        assertTrue(giltValidatorSet.isValidator(1, signers[4]));
        assertFalse(giltValidatorSet.isValidator(1, makeAddr("not validator")));
    }

    function test_isProducer() public {
        (uint256[] memory ids, uint256[] memory powers, address[] memory signers) = _commitSpan();
        assertFalse(giltValidatorSet.isProducer(1, signers[4]));
        assertFalse(giltValidatorSet.isProducer(1, makeAddr("not producer")));
        assertTrue(giltValidatorSet.isProducer(1, signers[1]));
    }

    function test_isCurrentValidator() public {
        (uint256[] memory ids, uint256[] memory powers, address[] memory signers) = _commitSpan();
        assertTrue(giltValidatorSet.isCurrentValidator(signers[4]));
        vm.roll(0);
        assertFalse(giltValidatorSet.isCurrentValidator(signers[4]));
    }

    function test_isCurrentProducer() public {
        (uint256[] memory ids, uint256[] memory powers, address[] memory signers) = _commitSpan();
        assertTrue(giltValidatorSet.isCurrentProducer(signers[1]));
        vm.roll(0);
        assertFalse(giltValidatorSet.isCurrentProducer(signers[1]));
    }

    function test_getGiltValidators() public {
        (address[] memory addresses, uint256[] memory powers) = giltValidatorSet.getGiltValidators(0);
        assertEq(addresses.length, 1);
        assertEq(powers.length, 1);
        assertEq(addresses[0], 0x6c468CF8c9879006E22EC4029696E005C2319C9D);
        assertEq(powers[0], 40);
        (uint256[] memory ids_, uint256[] memory powers_, address[] memory signers_) = _commitSpan();
        (addresses, powers) = giltValidatorSet.getGiltValidators(FIRST_END_BLOCK + 1);
    ...
    function test_getValidators() public {
        vm.roll(0);
        (address[] memory addresses, uint256[] memory powers) = giltValidatorSet.getValidators();
        assertEq(addresses.length, 1);
        assertEq(powers.length, 1);
        assertEq(addresses[0], 0x6c468CF8c9879006E22EC4029696E005C2319C9D);
        assertEq(powers[0], 40);
        ...
        (addresses, powers) = giltValidatorSet.getValidators();
        for (uint256 i; i < addresses.length; ++i) {
            assertEq(addresses[i], signers_[i]);
            assertEq(powers[i], powers_[i]);
        }
    ...
    function test_getStakePowerBySigs() public {
        ...
        assertEq(giltValidatorSet.getStakePowerBySigs(1, digest, sigs), sum);
    }

    function test_checkMembership_SingleLeaf() public {
        bytes32 leaf = keccak256("leaf");
        assertTrue(giltValidatorSet.checkMembership(leaf, leaf, ""));
        assertFalse(giltValidatorSet.checkMembership(keccak256("not root"), leaf, ""));
    }
    ...
    function test_checkMembership() public {
        ...
        assertTrue(giltValidatorSet.checkMembership(root, leaf, proof));
        bytes memory wrongProof = abi.encodePacked(hex"01", proof1, hex"01", proof2, "garbage");
        assertFalse(giltValidatorSet.checkMembership(root, leaf, wrongProof));
    }

    function test_leafNode(bytes32 d) public returns (bytes32) {
        assertEq(giltValidatorSet.leafNode(d), sha256(abi.encodePacked(bytes1(uint8(0)), d)));
    }

    function test_innerNode(bytes32 left, bytes32 right) public returns (bytes32) {
        assertEq(giltValidatorSet.innerNode(left, right), sha256(abi.encodePacked(bytes1(uint8(1)), left, right)));
    }
        ...
        returns (uint256[] memory ids, uint256[] memory powers, address[] memory signers)
    {
        ...
        vm.prank(SYSTEM_ADDRESS);
        giltValidatorSet.commitSpan(_newSpan, startBlock, endBlock, validatorBytes, producerBytes);
