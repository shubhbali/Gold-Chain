...
import { TestToken } from '../../helpers/artifacts.js'
import { assertBigNumbergt, buildsubmitCheckpointPaylod } from '../../helpers/utils.js'
...
describe('RootChain', async function (accounts) {
  let governance
  let rootChain
  let wallets
  let stakeManager
  ...
  async function freshDeploy() {
    ...
    for (let i = 0; i < validatorsCount; i++) {
      ...
      await stakeToken_i.approve(stakeManager.address, mintAmount.toString())
  ...
  function testCheckpoint(dontBuildHeaderBlockPayload) {
    before(async function () {
      if (!dontBuildHeaderBlockPayload) {
        ...
        const { data, sigs } = buildsubmitCheckpointPaylod(this.proposer, this.start, this.end, this.root, wallets, {
          allValidators: true,
    ...
    it('must submit', async function () {
      this.result = await (await rootChain.submitCheckpoint(this.data, this.sigs)).wait()
    })

    it('must emit NewBlockHeader', async function () {
      await utils.assertInLogs(this.result.events, 'NewHeaderBlock', {
    ...
    describe('when blockInterval is less than checkPointBlockInterval', function () {
      ...
      before(async function () {
        await governance.update(
          stakeManager.address,
          stakeManager.interface.encodeFunctionData('updateCheckPointBlockInterval', [2])
        )
  ...
  describe('updateDepositId', async function () {
    ...
    describe('when numDeposits > MAX_DEPOSITS', function () {
      it('must revert', async function () {
        ...
        await expectRevert(rootChain.updateDepositId(maxDeposits.add(new BN(1)).toString()), 'UNAUTHORIZED_DEPOSIT_MANAGER_ONLY')
      })
    })
  })
})
