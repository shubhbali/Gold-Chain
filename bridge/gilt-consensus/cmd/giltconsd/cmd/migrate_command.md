...
The command will take as input GiltConsensus v1 genesis and do all the necessary transformations to make it compatible with GiltConsensus v2.
...
- **gilt**: 424.81 MB (445450568 bytes)
...
Iterating over all the `spans` and renaming `gilt_chain_id` to `chain_id`, `span_id` to `id`.  
...
Milestone genesis is not exported from GiltConsensus v1, so we just add it to the genesis. It has just one property `params`, and we initialize it with the default params from GiltConsensus v2.

### chainmanager

Renaming `params.mainchain_tx_confirmations` to `params.main_chain_tx_confirmations` and `params.maticchain_tx_confirmations` to `params.gilt_chain_tx_confirmations`.
...
In a lot of the migrations, we can see logic regarding vesting accounts. That's not of interest to us since vesting logic is disabled in GiltConsensus, and we don't have such accounts.  
We skip the module accounts during the migration because GiltConsensus v2 will initialize them from zero anyway.
