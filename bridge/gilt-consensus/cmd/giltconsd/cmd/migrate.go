...
import (
	...
	v036gov "github.com/giltchain/gilt-consensus/cmd/giltconsd/cmd/migration/gov/v036"
...
// MigrateCommand returns a command that migrates the giltconsensus v1 genesis file to giltconsensus v2.
...
func migrateChainManagerModule(genesisData map[string]interface{}, chainId string) error {
	...
	if err := utils.AddProperty(chainmanagerData, "params.chain_params", "giltconsensus_chain_id", chainId); err != nil {
		return fmt.Errorf("failed to add giltconsensus_chain_id field: %w", err)
	}
...
func migrateGovModule(genesisData map[string]interface{}) error {
	...
	newGovState := govTypes.GenesisState{
		...
		Params:             &params,
		Constitution: "GiltConsensus chain serves as the coordination and consensus layer of the Gilt PoS protocol. " +
			...
			"Decisions must be guided by principles of decentralization, transparency, and the long-term resilience of the Gilt ecosystem. " +
...
func replaceMaticWithPol(v interface{}) {
	...
	case map[string]interface{}:
		for key, item := range val {
			// If the value is exactly "gilt", replace it
			if strVal, ok := item.(string); ok && strVal == gilt {
				val[key] = pol
				continue
			}
			// Recurse
	...
	case []interface{}:
		for i := range val {
			// If the element is exactly "gilt", replace it
			if strVal, ok := val[i].(string); ok && strVal == gilt {
				val[i] = pol
				continue
			}
			// Recurse
