...
var logger = helper.Logger.With("module", "cmd/giltconsd")
...
// NewRootCmd creates a new root command for giltconsd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	...
	tempApp := app.NewGiltConsensusApp(tempLogger, db.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(tempDir()))
	defer func() {
	...
	rootCmd := &cobra.Command{
		Use:           "giltconsd",
		Short:         "Gilt Consensus Daemon (server)",
		SilenceErrors: true,
	...
	// adding giltconsensus configuration flags to the root command
	helper.DecorateWithGiltConsensusFlags(rootCmd, viper.GetViper(), logger, "main")
