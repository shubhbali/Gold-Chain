...
var tempDir = func() string {
	dir, err := os.MkdirTemp("", "giltconsensus")
	if err != nil {
...
func initAppConfig() (string, interface{}) {
	...
	srvConf.API.Address = helper.DefaultGiltConsensusServerURL
	customAppConfig := helper.CustomAppConfig{
		Config: *srvConf,
		Custom: helper.GetDefaultGiltConsensusConfig(),
	}
...
func initRootCmd(
	...
	AddCommandsWithStartCmdOptions(rootCmd, app.DefaultNodeHome, newApp, appExport, server.StartCmdOptions{
		...
		PostSetup: func(svrCtx *server.Context, clientCtx client.Context, ctx context.Context, g *errgroup.Group) error {
			...
			go checkServerStatus(checkCtx, helper.GetGiltConsensusServerEndpoint(util.AccountParamsURL), resultChan)
...
func newApp(
	...
	return app.NewGiltConsensusApp(
...
// appExport creates a new giltconsensus app (optionally at a given height) and exports state.
func appExport(
	logger log.Logger,
	db dbm.DB,
	traceStore io.Writer,
	...
	var hApp *app.GiltConsensusApp
	if height != -1 {
		hApp = app.NewGiltConsensusApp(logger, db, traceStore, false, appOpts)
		defer func(hApp *app.GiltConsensusApp) {
			err := hApp.Close()
			if err != nil {
		...
		hApp = app.NewGiltConsensusApp(logger, db, traceStore, true, appOpts)
		defer func(hApp *app.GiltConsensusApp) {
			err := hApp.Close()
			if err != nil {
				fmt.Printf("error closing the app: %v\n", err)
...
func VerifyGenesis(ctx *server.Context, hApp *app.GiltConsensusApp) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "verify-genesis",
