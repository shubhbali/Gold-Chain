...
The `cmd` package is responsible for starting the giltconsensus application and provides the CLI framework (based on [cobra](https://github.com/spf13/cobra)).

## giltconsd

`giltconsd` is the service that is responsible for starting the giltconsensus application and also to interact with the giltconsensus application.
...
Available Commands:
  ...
  create-testnet         Initialize files for a GiltConsensus testnet
  debug                  Tool for helping with debugging your application
  export                 Export state to JSON
  generate-keystore      Generates keystore file
  generate-validator-key Generate validator key
  giltconsensus-bridge        GiltConsensus bridge daemon
...
Flags:
      ...
      --app string                        Override of GiltConsensus app config file (default <home>/config/config.json)
      --gilt_grpc_flag string              gRPC flag for gilt chain
      --gilt_grpc_url string               Set gRPC endpoint for gilt chain
      --gilt_rpc_url string                Set RPC endpoint for gilt chain
      --chain string                      Set one of the chains: [mainnet,gilttestnet-legacy,gilttestnet,local]
      ...
      --giltconsensus_rest_server string       Set GiltConsensus REST server endpoint
  -h, --help                              help for giltconsd
      --home string                       directory for config and data (default "/var/lib/gilt-consensus")
