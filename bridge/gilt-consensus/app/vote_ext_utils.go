...
import (
	...
	"github.com/giltchain/gilt-consensus/metrics/consensus"
...
// Since we are using a hard fork approach for the giltconsensus migration, VEs will be enabled from v2 genesis' initial height (v1 last height +1).
...
func validateCheckpointMsgData(ctx sdk.Context, extension []byte, chainManagerKeeper chainManagerKeeper.Keeper, checkpointKeeper checkpointKeeper.Keeper, contractCaller helper.IContractCaller) error {
	...
	if chainParams.ChainParams.GiltChainId != checkpointMsg.GiltChainId {
		return fmt.Errorf("invalid gilt chain id, expected %s, got %s", chainParams.ChainParams.GiltChainId, checkpointMsg.GiltChainId)
	}
