...
import (
	...
	clerktestutil "github.com/giltchain/gilt-consensus/x/clerk/testutil"
...
func TestExtendVoteHandler(t *testing.T) {
	...
	mockCaller.
		On("GetGiltChainBlock", mock.Anything, mock.Anything).
		Return(&ethTypes.Header{
			Number: big.NewInt(10),
		}, nil)
	mockCaller.
		On("GetGiltChainBlockInfoInBatch", mock.Anything, mock.AnythingOfType("int64"), mock.AnythingOfType("int64")).
	...
	app.GiltKeeper = giltKeeper.NewKeeper(
		app.AppCodec(),
		runtime.NewKVStoreService(app.GetKey(giltTypes.StoreKey)),
		authTypes.NewModuleAddress(govtypes.ModuleName).String(),
		app.ChainManagerKeeper,
		&app.StakeKeeper,
		nil,
	...
	app.GiltKeeper.SetContractCaller(mockCaller)
	...
	mockCaller.AssertCalled(t, "GetGiltChainBlockInfoInBatch", mock.Anything, mock.AnythingOfType("int64"), mock.AnythingOfType("int64"))
...
func TestSideTxsHappyPath(t *testing.T) {
	...
	mockGiltKeeper := giltKeeper.NewKeeper(
		app.AppCodec(),
		runtime.NewKVStoreService(app.GetKey(giltTypes.StoreKey)),
		authTypes.NewModuleAddress(govtypes.ModuleName).String(),
		app.ChainManagerKeeper,
	...
	app.ModuleManager.Modules[giltTypes.ModuleName] = gilt.NewAppModule(mockGiltKeeper, mockCaller)
	app.GiltKeeper.SetContractCaller(mockCaller)
	...
	testCases := []struct {
		...
		{
			name: "gilt [MsgProposeSpan]] happy path",
			msg: &giltTypes.MsgProposeSpan{
				SpanId:     2,
				Proposer:   validators[0].Signer,
				StartBlock: 26657,
...
func TestAllUnhappyPathGiltSideTxs(t *testing.T) {
	priv, app, ctx, validatorPrivKeys := SetupAppWithABCICtx(t)
	validators := app.StakeKeeper.GetAllValidators(ctx)
	...
	mockCaller.On("GetGiltChainBlockAuthor", mock.Anything).Return(&val1Addr, nil)

	mockCaller.On("GetGiltChainBlock", mock.Anything, mock.Anything).Return(&blockHeader1, nil)
	mockCaller.
		On("GetGiltChainBlockInfoInBatch", mock.Anything, mock.Anything, mock.Anything).
		Return([]*ethTypes.Header{&blockHeader1}, []uint64{1}, []common.Address{common.HexToAddress(vals[0].GetOperator())}, nil)

	for _, span := range spans {
		err := app.GiltKeeper.AddNewSpan(ctx, &span)
		require.NoError(t, err)
		err = app.GiltKeeper.StoreSeedProducer(ctx, span.Id, &val1Addr)
	}
...
func TestMilestoneHappyPath(t *testing.T) {
	...
	err := app.GiltKeeper.AddNewSpan(ctx, &giltTypes.Span{
		Id:         0,
		StartBlock: 0,
...
func TestPrepareProposal(t *testing.T) {
	...
	// Build a fake commit for height=3 with enough production validator power.
	extCommitBytes, extCommit, voteInfo1, err := buildExtensionCommits(
		t,
		app,
		common.Hex2Bytes("000000000000000000000000000000000000000000000000000000000001dead"),
		validators,
	...
	msgGilt := &giltTypes.MsgProposeSpan{
		// SpanId:     2,
		Proposer:   validators[0].Signer,
		StartBlock: 26657,
		EndBlock:   30000,
	...
	require.NoError(t, txBuilder.SetMsgs(msgGilt))
	require.NoError(t, err)
	require.NoError(t, txBuilder.SetSignatures(sigV2))

	txBytesGilt, err := txConfig.TxEncoder()(txBuilder.GetTx())
	require.NoError(t, err)
	err = app.StakeKeeper.SetLastBlockTxs(ctx, [][]byte{txBytesGilt})
	require.NoError(t, err)
	fmt.Println("#################################################################")
	fmt.Println(txBytesGilt)
	fmt.Println(app.StakeKeeper.GetLastBlockTxs(ctx))

	var txBytesGiltCmt cmtTypes.Tx = txBytesGilt
	...
	// Test FinalizeBlock handler
	finalizeReqGiltSideTx := abci.RequestFinalizeBlock{
		Txs:             [][]byte{extCommitBytes2, txBytesGilt},
		Height:          3,
		ProposerAddress: common.FromHex(validators[0].Signer),
	}
	_, err = app.PreBlocker(ctx, &finalizeReqGiltSideTx)
	require.NoError(t, err)
	...
	var txBytesClerkCmt cmtTypes.Tx = txBytesGilt

	extCommitBytes3, _, _, err := buildExtensionCommits(
		t,
		app,
	...
	extCommitBytes4, _, _, err := buildExtensionCommits(
		t,
		app,
...
func TestUpdateBlockProducerStatus(t *testing.T) {
	...
	err := app.GiltKeeper.UpdateLatestActiveProducer(ctx, initialActiveProducers)
	require.NoError(t, err)

	err = app.GiltKeeper.AddLatestFailedProducer(ctx, 3)
	require.NoError(t, err)
	err = app.GiltKeeper.AddLatestFailedProducer(ctx, 4)
	require.NoError(t, err)
	...
	latestActive, err := app.GiltKeeper.GetLatestActiveProducer(ctx)
	require.NoError(t, err)
	require.Equal(t, supportingProducerIDs, latestActive)

	latestFailed, err := app.GiltKeeper.GetLatestFailedProducer(ctx)
	require.NoError(t, err)
...
func TestCheckAndAddFutureSpan(t *testing.T) {
	...
	err := app.GiltKeeper.AddNewSpan(ctx, &lastSpan)
	require.NoError(t, err)
	...
	t.Run("condition false", func(t *testing.T) {
		...
		currentLastSpan, err := app.GiltKeeper.GetLastSpan(ctx)
		require.NoError(t, err)
	...
	t.Run("condition true", func(t *testing.T) {
		...
		mockCaller := new(helpermocks.IContractCaller)
		mockCaller.On("GetGiltChainBlockAuthor", mock.Anything, mock.Anything).Return([]common.Address{common.HexToAddress(validators[0].Signer)}, nil)
		app.GiltKeeper.SetContractCaller(mockCaller)
		...
		if len(validators) > 1 {
			...
			for _, val := range validators {
				...
				producerVotes := giltTypes.ProducerVotes{Votes: []uint64{consensusCandidateID}}
				err := app.GiltKeeper.SetProducerVotes(ctx, val.ValId, producerVotes)
				require.NoError(t, err)
			}

			// Set up producer performance scores
			err := app.GiltKeeper.UpdateValidatorPerformanceScore(ctx, allValidatorIDs, 1)
			require.NoError(t, err)
			...
			err = app.GiltKeeper.SetParams(ctx, params)
			require.NoError(t, err)
		}
...
func TestCheckAndRotateCurrentSpan(t *testing.T) {
	...
	t.Run("condition true", func(t *testing.T) {
		...
		err = app.GiltKeeper.UpdateLatestActiveProducer(ctx, initialActiveProducers)
		require.NoError(t, err)
		err = app.GiltKeeper.AddLatestFailedProducer(ctx, uint64(99)) // some other producer
		require.NoError(t, err)
		...
		if len(validators) > 0 {
			...
			for _, val := range validators {
				...
				producerVotes := giltTypes.ProducerVotes{Votes: votes}
				err := app.GiltKeeper.SetProducerVotes(ctx, val.ValId, producerVotes)
				require.NoError(t, err)
			...
			// Ensure gilt params allow for proper producer selection
			params, err := app.GiltKeeper.GetParams(ctx)
			require.NoError(t, err)
		...
		mockCaller.On("GetGiltChainBlockAuthor", mock.Anything, lastMilestone.EndBlock+1).Return(&producerSignerAddr, nil)
		app.GiltKeeper.SetContractCaller(mockCaller)
		...
		failedProducers, err := app.GiltKeeper.GetLatestFailedProducer(ctx)
		require.NoError(t, err)
...
func TestPreBlockerSpanRotationWithMinorityMilestone(t *testing.T) {
	...
	mockCaller.On("GetGiltChainBlockAuthor", mock.Anything, mock.Anything).Return(&producerSigner, nil)
	app.GiltKeeper.SetContractCaller(mockCaller)
	...
	currentSpan, err := app.GiltKeeper.GetLastSpan(ctx)
	require.NoError(t, err)
...
func TestPreBlockerSpanRotationWithMajorityMilestone(t *testing.T) {
	...
	_, err = app.GiltKeeper.GetLastSpan(ctx)
	require.NoError(t, err)
...
func TestPrepareProposal_MultipleSideTxsSameType(t *testing.T) {
	...
	t.Run("multiple gilt propose span messages in different txs", func(t *testing.T) {
		priv, app, ctx, validatorPrivKeys := SetupAppWithABCICtx(t)
		validators := app.StakeKeeper.GetAllValidators(ctx)

		// Create 3 gilt propose span side txs
		var proposedTxs [][]byte
		propAddr := sdk.AccAddress(priv.PubKey().Address())
		propAcc := app.AccountKeeper.GetAccount(ctx, propAddr)
		sequence := propAcc.GetSequence()
		...
		require.NoError(t, err)
		require.NotNil(t, res)
		require.Equal(t, 4, len(res.Txs), "Should have exactly 4 transactions (1 ExtendedCommitInfo + 3 gilt propose span txs)")
	})
}

func TestPrepareProposal_MultipleSideTxsDifferentTypes(t *testing.T) {
	t.Run("mix of checkpoint, gilt, clerk, stake, and topup side txs", func(t *testing.T) {
		priv, app, ctx, validatorPrivKeys := SetupAppWithABCICtx(t)
		validators := app.StakeKeeper.GetAllValidators(ctx)
		...
		// 2. Gilt propose span side tx
		giltMsg := &giltTypes.MsgProposeSpan{
			Proposer:   priv.PubKey().Address().String(),
			SpanId:     1,
			StartBlock: 0,
			EndBlock:   6399,
		...
		txBytes, err = buildSignedTxWithSequence(giltMsg, ctx, priv, app, sequence)
		require.NoError(t, err)
		proposedTxs = append(proposedTxs, txBytes)
		sequence++
...
func TestVerifyVoteExtension_AllRejectionScenarios(t *testing.T) {
	tests := []struct {
		name         string
		setupVE      func(t *testing.T, app *GiltConsensusApp, ctx sdk.Context, priv cryptotypes.PrivKey) *abci.RequestVerifyVoteExtension
...
func TestPreBlocker_MultipleApprovedSideTxs(t *testing.T) {
	t.Run("preBlocker with 5 approved side txs of different types", func(t *testing.T) {
		...
		txBytes, err = buildSignedTx(giltMsg, priv.PubKey().Address().String(), ctx, priv, app)
		require.NoError(t, err)
		txsForBlock = append(txsForBlock, txBytes)
	...
	t.Run("stress test with 100 blocks and mixed tx types", func(t *testing.T) {
		...
		for blockHeight := int64(3); blockHeight < 103; blockHeight++ {
			...
			for i := 0; i < 5; i++ {
				...
				case 4:
					msg = &giltTypes.MsgBackfillSpans{
						Proposer:         priv.PubKey().Address().String(),
						ChainId:          "1",
						LatestSpanId:     uint64(blockHeight),
						LatestGiltSpanId: uint64(blockHeight + 1),
					}
				}
...
func SetupAppWithABCICtx(t *testing.T) (cryptotypes.PrivKey, GiltConsensusApp, sdk.Context, []secp256k1.PrivKey) {
	return SetupAppWithABCICtxAndValidators(t, 1)
}

func SetupAppWithABCICtxAndValidators(t *testing.T, numValidators int) (cryptotypes.PrivKey, GiltConsensusApp, sdk.Context, []secp256k1.PrivKey) {
	priv, _, _ := testdata.KeyTestPubAddr()
...
func buildSignedTxWithSequence(msg sdk.Msg, ctx sdk.Context, priv cryptotypes.PrivKey, app GiltConsensusApp, sequence uint64) ([]byte, error) {
	propAddr := sdk.AccAddress(priv.PubKey().Address())
	propAcc := app.AccountKeeper.GetAccount(ctx, propAddr)
	if propAcc == nil {
...
func buildSignedTx(msg sdk.Msg, signer string, ctx sdk.Context, priv cryptotypes.PrivKey, app GiltConsensusApp) ([]byte, error) {
...
func buildExtensionCommits(
	...
	for _, validator := range validators {
		privKey, ok := privKeysBySigner[addressUtils.FormatAddress(validator.Signer)]
		require.Truef(t, ok, "missing private key for validator signer %s", validator.Signer)
		...
		votes = append(votes, setupExtendedVoteInfoWithNonRp(
			t,
			cmtproto.BlockIDFlagCommit,
			txHashBytes,
			...
			height,
			app,
			privKey.PubKey().(secp256k1.PubKey),
		))
	}
	...
	extCommitBytes, err := extCommit.Marshal()
	require.NoError(t, err)
	return extCommitBytes, extCommit, &votes[0], err
}

func buildExtensionCommitsWithMilestoneProposition(t *testing.T, app *GiltConsensusApp, txHashBytes []byte, validators []*stakeTypes.Validator, validatorPrivKeys []secp256k1.PrivKey, milestoneProp milestoneTypes.MilestoneProposition) ([]byte, *abci.ExtendedCommitInfo, *abci.ExtendedVoteInfo, error) {
	privKeysBySigner := validatorPrivKeysBySigner(validatorPrivKeys)
	votes := make([]abci.ExtendedVoteInfo, 0, len(validators))
	for _, validator := range validators {
		privKey, ok := privKeysBySigner[addressUtils.FormatAddress(validator.Signer)]
		...
		votes = append(votes, setupExtendedVoteInfoWithMilestoneProposition(
			t,
			cmtproto.BlockIDFlagCommit,
			txHashBytes,
			common.Hex2Bytes("000000000000000000000000000000000000000000000000000000000002dead"),
...
func validatorPrivKeysBySigner(privKeys []secp256k1.PrivKey) map[string]secp256k1.PrivKey {
	keysBySigner := make(map[string]secp256k1.PrivKey, len(privKeys))
	for _, privKey := range privKeys {
		keysBySigner[addressUtils.FormatAddress(privKey.PubKey().Address().String())] = privKey
	}
	return keysBySigner
}
