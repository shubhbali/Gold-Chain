...
func (app *GiltConsensusApp) NewPrepareProposalHandler() sdk.PrepareProposalHandler {
...
func (app *GiltConsensusApp) NewProcessProposalHandler() sdk.ProcessProposalHandler {
...
func (app *GiltConsensusApp) ExtendVoteHandler() sdk.ExtendVoteHandler {
	return func(ctx sdk.Context, req *abci.RequestExtendVote) (*abci.ResponseExtendVote, error) {
		...
		for _, rawTx := range txs {
			...
			// e.g. gilt, checkpoint, clerk, milestone, stake and topup
			messages := tx.GetMsgs()
...
func (app *GiltConsensusApp) VerifyVoteExtensionHandler() sdk.VerifyVoteExtensionHandler {
	return func(ctx sdk.Context, req *abci.RequestVerifyVoteExtension) (*abci.ResponseVerifyVoteExtension, error) {
		...
		if err := rejectUnknownVoteExtFields(req.VoteExtension); err != nil {
			logger.Error(giltconsensusTypes.ErrAlertVoteExtensionRejected+" Error while checking unknown fields in VoteExtension", "validator", valAddr, "error", err)
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		}

		var voteExtension sidetxs.VoteExtension
		if err := proto.Unmarshal(req.VoteExtension, &voteExtension); err != nil {
			logger.Error(giltconsensusTypes.ErrAlertVoteExtensionRejected+" Error while unmarshalling VoteExtension", "validator", valAddr, "error", err)
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		}

		// ensure block height and hash match
		if req.Height != voteExtension.Height {
			logger.Error(giltconsensusTypes.ErrAlertVoteExtensionRejected, "block height", req.Height, "consolidatedSideTxResponse height", voteExtension.Height, "validator", valAddr)
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		}

		if !bytes.Equal(req.Hash, voteExtension.BlockHash) {
			logger.Error(giltconsensusTypes.ErrAlertVoteExtensionRejected, "block hash", common.Bytes2Hex(req.Hash), "consolidatedSideTxResponse blockHash", common.Bytes2Hex(voteExtension.BlockHash), "validator", valAddr)
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		}
		...
		if err != nil {
			logger.Error(giltconsensusTypes.ErrAlertVoteExtensionRejected, "validator", valAddr, "tx hash", common.Bytes2Hex(txHash), "error", err)
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		}
...
func (app *GiltConsensusApp) PreBlocker(ctx sdk.Context, req *abci.RequestFinalizeBlock) (*sdk.ResponsePreBlock, error) {
	startTime := time.Now()
