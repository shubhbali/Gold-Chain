...
const (
	...
	LatestSpanURL           = "/gilt/spans/latest"
	SpanByIdURL             = "/gilt/spans/%v"
	NextSpanInfoURL         = "/gilt/spans/prepare"
	NextSpanSeedURL         = "/gilt/spans/seed/%v"
	ProducerVotesURL        = "/gilt/producer-votes/%v"
...
func IsProposer(cdc codec.Codec) (bool, error) {
	...
	result, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(fmt.Sprintf(ProposersURL, strconv.FormatUint(count, 10))))
	if err != nil {
...
func IsInProposerList(count uint64, cdc codec.Codec) (bool, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(fmt.Sprintf(ProposersURL, strconv.FormatUint(count+1, 10))))
	if err != nil {
...
func IsCurrentProposer(cdc codec.Codec) (bool, error) {
	...
	result, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(CurrentProposerURL))
	if err != nil {
...
func IsEventSender(validatorID uint64, cdc codec.Codec) bool {
	...
	result, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(fmt.Sprintf(ValidatorURL, strconv.FormatUint(validatorID, 10))))
	if err != nil {
...
// IsCatchingUp checks if the giltconsensus node you are connected to is fully synced or not
...
// GetAccount returns giltconsensus auth account
...
func GetAccountParamsURL(cdc codec.Codec) (*authtypes.Params, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(AccountParamsURL))
	if err != nil {
...
func GetChainmanagerParams(cdc codec.Codec) (*chainmanagertypes.Params, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(ChainManagerParamsURL))
	if err != nil {
...
func GetCheckpointParams(cdc codec.Codec) (*checkpointTypes.Params, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(CheckpointParamsURL))
	if err != nil {
...
func GetCheckpointAckCount(cdc codec.Codec) (uint64, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(CountCheckpointURL))
	if err != nil {
...
func GetBufferedCheckpoint(cdc codec.Codec) (*checkpointTypes.Checkpoint, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(BufferedCheckpointURL))
	if err != nil {
...
func GetLatestCheckpoint(cdc codec.Codec) (*checkpointTypes.Checkpoint, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(LatestCheckpointURL))
	if err != nil {
...
func AppendPrefix(signerPubKey []byte) []byte {
	// Append the prefix "0x04", because giltconsensus uses publicKey in an uncompressed format.
...
func GetValidatorSet(cdc codec.Codec) (*staketypes.ValidatorSet, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(CurrentValidatorSetURL))
	if err != nil {
...
func GetClerkEventRecord(stateId int64, cdc codec.Codec) (*clerktypes.EventRecord, error) {
	...
	response, err := helper.FetchFromAPI(helper.GetGiltConsensusServerEndpoint(fmt.Sprintf(ClerkEventRecordURL, stateId)))
	if err != nil {
...
func GetUnconfirmedTxnCount(event interface{}) int {
	...
	if err != nil {
		logger.Error("Error unmarshalling response received from GiltConsensus Server", "error", err)
		return 0
	}
