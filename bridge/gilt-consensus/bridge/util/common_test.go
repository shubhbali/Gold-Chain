...
func TestURLConstants_GiltURLs(t *testing.T) {
	// Test gilt-related URLs
	giltURLs := []string{
		util.LatestSpanURL,
		util.SpanByIdURL,
	...
	for _, url := range giltURLs {
		require.NotEmpty(t, url)
		// Should contain at least one of: gilt, span, producer
		containsExpected := strings.Contains(url, "gilt") ||
