...
const (
	...
	errMsgBroadcasting        = "StakingProcessor: error while broadcasting to giltconsensus"
	errMsgTxFailed            = "StakingProcessor: tx failed on giltconsensus"
	...
	// Info messages
	infoMsgIgnoringAlreadyProcessed = "StakingProcessor: ignoring task to send %s to giltconsensus as already processed"
	infoMsgIgnoringNonceOutOfOrder  = "StakingProcessor: ignoring task to send %s to giltconsensus as nonce is out of order"
	infoMsgAccountDoesNotExist      = "StakingProcessor: giltconsensus account doesn't exist. Retrying validator-join after 10 seconds"
...
func (sp *StakingProcessor) RegisterTasks() {
	...
	if err := sp.queueConnector.Server.RegisterTask("sendValidatorJoinToGiltConsensus", sp.sendValidatorJoinToGiltConsensus); err != nil {
		sp.Logger.Error("StakingProcessor | RegisterTasks | sendValidatorJoinToGiltConsensus", "error", err)
	}

	if err := sp.queueConnector.Server.RegisterTask("sendUnstakeInitToGiltConsensus", sp.sendUnstakeInitToGiltConsensus); err != nil {
		sp.Logger.Error("StakingProcessor | RegisterTasks | sendUnstakeInitToGiltConsensus", "error", err)
	}

	if err := sp.queueConnector.Server.RegisterTask("sendStakeUpdateToGiltConsensus", sp.sendStakeUpdateToGiltConsensus); err != nil {
		sp.Logger.Error("StakingProcessor | RegisterTasks | sendStakeUpdateToGiltConsensus", "error", err)
	}

	if err := sp.queueConnector.Server.RegisterTask("sendSignerChangeToGiltConsensus", sp.sendSignerChangeToGiltConsensus); err != nil {
		sp.Logger.Error("StakingProcessor | RegisterTasks | sendSignerChangeToGiltConsensus", "error", err)
	}
}

func (sp *StakingProcessor) sendValidatorJoinToGiltConsensus(eventName string, logBytes string) error {
	vLog := types.Log{}
