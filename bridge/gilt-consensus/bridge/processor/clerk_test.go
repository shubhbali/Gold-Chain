...
func BenchmarkSendStateSyncedToGiltConsensus(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		func() {
			...
			if err = cp.sendStateSyncedToGiltConsensus("StateSynced", dlb.String()); err != nil {
				b.Fatal(err)
			}

			b.StopTimer()

			b.Log("StateSynced sent to giltconsensus successfully")
		}()
	}
}
	...
	for i := 0; i < b.N; i++ {
		func() {
			...
			rcl.SendTaskWithDelay(
				"sendStateSyncedToGiltConsensus", "StateSynced",
				logs.Bytes(), ts[i], nil)
			b.StopTimer()
...
func BenchmarkCalculateTaskDelay(b *testing.B) {
	...
	helper.InitTestGiltConsensusConfig("")
