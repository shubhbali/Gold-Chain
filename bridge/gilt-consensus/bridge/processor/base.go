...
const errorUnmarshallingTxStatus = "BaseProcessor: error unmarshalling tx status received from giltconsensus server"

// Processor defines a block header listener for RootChain, GiltChain, GiltConsensus
...
func (bp *BaseProcessor) isOldTx(_ client.Context, txHash string, logIndex uint64, eventType util.BridgeEvent, event interface{}) (bool, error) {
	...
	case util.ClerkEvent:
		endpoint = helper.GetGiltConsensusServerEndpoint(util.ClerkTxStatusURL)
	default:
...
func (bp *BaseProcessor) checkTxAgainstMempool(msg types.Msg, event interface{}) (bool, error) {
	...
	if err != nil {
		bp.Logger.Error("BaseProcessor: error unmarshalling response received from GiltConsensus Server", "error", err)
		return false, err
	}
