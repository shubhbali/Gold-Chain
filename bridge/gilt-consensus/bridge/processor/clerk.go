...
import (
	...
	"github.com/giltchain/gilt-consensus/common/tracing"
...
const (
	...
	errMsgClerkBroadcasting               = "ClerkProcessor: error while broadcasting clerk Record to giltconsensus"
	...
	infoMsgClerkIgnoringAlreadyProcessed = "ClerkProcessor: ignoring task to send deposit to giltconsensus as already processed"
...
func (cp *ClerkProcessor) RegisterTasks() {
	...
	if err := cp.queueConnector.Server.RegisterTask("sendStateSyncedToGiltConsensus", cp.sendStateSyncedToGiltConsensus); err != nil {
		cp.Logger.Error("ClerkProcessor | RegisterTasks | sendStateSyncedToGiltConsensus", "error", err)
	}
}

// sendStateSyncedToGiltConsensus - handle state sync event from rootChain
// 1. Check if this deposit event has to be broadcast to giltconsensus
// 2. Create and broadcast record transaction to giltconsensus
func (cp *ClerkProcessor) sendStateSyncedToGiltConsensus(eventName string, logBytes string) error {
	tracingCtx := tracing.WithTracer(context.Background(), otel.Tracer("State-Sync"))
	// work begins
	sendStateSyncedToGiltConsensusCtx, sendStateSyncedToGiltConsensusSpan := tracing.StartSpan(tracingCtx, "sendStateSyncedToGiltConsensus")
	defer tracing.EndSpan(sendStateSyncedToGiltConsensusSpan)
	...
	if err = helper.UnpackLog(cp.stateSenderAbi, event, eventName, &vLog); err != nil {
		...
		defer util.LogElapsedTimeForStateSyncedEvent(event, "sendStateSyncedToGiltConsensus", start)

		tracing.SetAttributes(sendStateSyncedToGiltConsensusSpan, []attribute.KeyValue{
			attribute.String("event", eventName),
			attribute.Int64("id", event.Id.Int64()),
			attribute.String("contract", event.ContractAddress.String()),
		}...)

		_, isOldTxSpan := tracing.StartSpan(sendStateSyncedToGiltConsensusCtx, "isOldTx")
		...
		if isOld {
			cp.Logger.Info(infoMsgClerkIgnoringAlreadyProcessed,
				...
				"giltChainId", chainParams.GiltChainId,
		...
		_, maxStateSyncSizeCheckSpan := tracing.StartSpan(sendStateSyncedToGiltConsensusCtx, "maxStateSyncSizeCheck")
		...
		_, checkTxAgainstMempoolSpan := tracing.StartSpan(sendStateSyncedToGiltConsensusCtx, "checkTxAgainstMempool")
		...
		_, BroadcastToGiltConsensusSpan := tracing.StartSpan(sendStateSyncedToGiltConsensusCtx, "BroadcastToGiltConsensus")
		// return broadcast to giltconsensus
		_, err = cp.txBroadcaster.BroadcastToGiltConsensus(context.Background(), &msg, event)
		tracing.EndSpan(BroadcastToGiltConsensusSpan)
