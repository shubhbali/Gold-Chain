...
var (
	...
	giltconsensusAddress         = addressUtil.FormatAddress(common.BytesToAddress(address).String())
	giltconsensusAddressBytes, _ = addressCodec.NewHexCodec().StringToBytes(giltconsensusAddress)
	testChainId             = "testChainId"
	dummyCometBFTNodeUrl    = "http://localhost:26657"
	dummyGiltConsensusServerUrl  = "https://dummy-giltconsensus-api-testnet.giltchain.local"
	getAccountUrl           = dummyGiltConsensusServerUrl + "/cosmos/auth/v1beta1/accounts/" + giltconsensusAddress
	getAuthParamsUrl        = dummyGiltConsensusServerUrl + "/cosmos/auth/v1beta1/params"
...
func TestBroadcastToGiltConsensus(t *testing.T) {
	t.Parallel()
	...
	mockGetAccount = func(ctx context.Context, cliCtx client.Context, address string) (sdk.AccountI, error) {
		return authTypes.NewBaseAccount(giltconsensusAddressBytes, cosmosPrivKey.PubKey(), 0, 0), nil
	}

	srvConf := serverconfig.DefaultConfig()
	configuration := helper.GetDefaultGiltConsensusConfig()
	configuration.CometBFTRPCUrl = dummyCometBFTNodeUrl
	configuration.Chain = "testChain"
	srvConf.API.Enable = true
	srvConf.API.Address = dummyGiltConsensusServerUrl
	...
	giltconsensusApp, sdkCtx, _ := createTestApp(t)

	encodingConfig := moduletestutil.MakeTestEncodingConfig()
	txConfig := encodingConfig.TxConfig

	txBroadcaster := NewTxBroadcaster(giltconsensusApp.AppCodec(), context.Background(), client.Context{}, func(address string) sdk.AccountI {
		return authTypes.NewBaseAccount(giltconsensusAddressBytes, cosmosPrivKey.PubKey(), 1, 0)
	})

	// Create a mock account for testing.
	mockAccount := authTypes.NewBaseAccount(giltconsensusAddressBytes, cosmosPrivKey.PubKey(), 0, 0)
	mockGetAccount = func(ctx context.Context, cliCtx client.Context, address string) (sdk.AccountI, error) {
	...
	txBroadcaster.CliCtx.FromAddress = giltconsensusAddressBytes
	...
	mockAccountRetriever := &mockAccountRetriever{
		AccountKeeper: giltconsensusApp.AccountKeeper,
		Ctx:           sdkCtx,
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			txRes, err := txBroadcaster.testBroadcastToGiltConsensus(tc.msg, nil)
			if tc.expErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func createTestApp(t *testing.T) (*app.GiltConsensusApp, sdk.Context, client.Context) {
	t.Helper()
...
func (tb *TxBroadcaster) testBroadcastToGiltConsensus(msg sdk.Msg, event any) (*sdk.TxResponse, error) {
	tb.giltconsensusMutex.Lock()
	defer tb.giltconsensusMutex.Unlock()
	defer util.LogElapsedTimeForStateSyncedEvent(event, "BroadcastToGiltConsensus", time.Now())
	...
	if err != nil {
		tb.logger.Error("Error while broadcasting the giltconsensus transaction", "error", err)
	...
	tb.logger.Info("Tx sent on giltconsensus", "txHash", txHash, "accSeq", tb.lastSeqNo, "accNum", tb.accNum, "txResponse", txResponse)
