...
type TxBroadcaster struct {
	...
	giltMutex      sync.Mutex
	giltconsensusMutex sync.Mutex
...
// BroadcastToGiltConsensus broadcast to giltconsensus
func (tb *TxBroadcaster) BroadcastToGiltConsensus(ctx context.Context, msg sdk.Msg, event interface{}) (*sdk.TxResponse, error) {
...
func updateAccountSequence(tb *TxBroadcaster) error {
	...
	if errAcc != nil {
		tb.logger.Error("Error fetching account from rest-api", "url", helper.GetGiltConsensusServerEndpoint(fmt.Sprintf(util.AccountDetailsURL, address)))
		return errAcc
	}
...
// BroadcastToGiltChain broadcasts a msg to the gilt chain
func (tb *TxBroadcaster) BroadcastToGiltChain(msg ethereum.CallMsg) error {
	tb.giltMutex.Lock()
	defer tb.giltMutex.Unlock()

	// get gilt client
	giltClient := helper.GetGiltClient()

	// get auth
	auth, err := helper.GenerateAuthObj(giltClient, *msg.To, msg.Data)
	if err != nil {
	...
	tb.logger.Info("Sending transaction to gilt", "txHash", signedTx.Hash())

	// create a context with timeout
	ctx, cancel := context.WithTimeout(context.Background(), helper.GetConfig().GiltRPCTimeout)
	defer cancel()

	// broadcast transaction
	if err = giltClient.SendTransaction(ctx, signedTx); err != nil {
		tb.logger.Error("Error while broadcasting the transaction to gilt chain", "error", err)
		return err
	}
