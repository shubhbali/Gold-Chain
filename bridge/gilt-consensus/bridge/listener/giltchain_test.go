...
// giltChainListenerForTest wraps GiltChainListener for testing
type giltChainListenerForTest struct {
	GiltChainListener
	mockConnector *mockQueueConnector
}

func (b *giltChainListenerForTest) ProcessHeader(newHeader *blockHeader) {
	...
	b.sendTaskWithDelay("sendCheckpointToGiltConsensus", headerBytes, 0)
}

func (b *giltChainListenerForTest) sendTaskWithDelay(taskName string, headerBytes []byte, delay time.Duration) {
...
func TestGiltChainListener_Start(t *testing.T) {
	t.Parallel()
...
func TestGiltChainListener_ProcessHeader(t *testing.T) {
	t.Parallel()
	...
	t.Run("processes header and sends task successfully", func(t *testing.T) {
		...
		listener := &giltChainListenerForTest{
		...
		mockConnector.On("SendTask", mock.MatchedBy(func(sig *tasks.Signature) bool {
			return sig.Name == "sendCheckpointToGiltConsensus" &&
				len(sig.Args) == 1 &&
...
func TestGiltChainListener_sendTaskWithDelay(t *testing.T) {
	t.Parallel()
...
func TestGiltChainListener_Integration(t *testing.T) {
	t.Parallel()
...
func TestGiltChainListener_TaskSignatureStructure(t *testing.T) {
	t.Parallel()
	...
	t.Run("verifies task signature has correct structure", func(t *testing.T) {
		...
		signature := &tasks.Signature{
			Name: "sendCheckpointToGiltConsensus",
			Args: []tasks.Arg{
				{
					Type:  "string",
					Value: string(headerBytes),
		...
		// Verify structure
		require.Equal(t, "sendCheckpointToGiltConsensus", signature.Name)
...
func TestGiltChainListener_EdgeCases(t *testing.T) {
	t.Parallel()
