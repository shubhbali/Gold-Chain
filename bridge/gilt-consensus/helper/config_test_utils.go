...
// InitTestGiltConsensusConfig initializes test config for the unit tests
func InitTestGiltConsensusConfig(chain string) {
	customAppConf := CustomAppConfig{
		Config: *serverconfig.DefaultConfig(),
		Custom: GetDefaultGiltConsensusConfig(),
	}

	switch chain {
	case GiltTestnetChain:
		customAppConf.Custom.Chain = GiltTestnetChain
	case MainChain:
