# Checklist for GiltConsensus v1 to v2 containerized migration

This checklist is for users running GiltConsensus nodes in Docker or Kubernetes containers, or any other environment using docker images.  
...
## 1. Verify the environment
   ...
   - Identify the volume mount path for GiltConsensus `data`, `config`, and `bridge` (e.g., `-v /giltconsensus:/var/lib/gilt-consensus`).
...
## 2. Validate GiltConsensus v1 Config Files
...
If you plan to back up v1 `GILTCONSENSUS_HOME`,
ensure your system has at least 2× the current size of `GILTCONSENSUS_HOME` in available disk space
