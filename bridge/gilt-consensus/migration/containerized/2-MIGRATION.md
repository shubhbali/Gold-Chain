...
Once instructed by the Gilt team, please start the migration.  

## 1. Stop Existing GiltConsensus v1 Containers
- Gracefully shut down GiltConsensus v1, e.g., for docker:
  ```bash
  docker stop <container_id>
  ```

## 2. Prepare Backup
- Back up the `GILTCONSENSUS_HOME` (default `/var/lib/gilt-consensus`), containing `config/`, `data/` and `bridge/` folders outside the container.
- Example (for Docker):
  ```bash
  docker cp <container_id>:/var/lib/gilt-consensus /path/to/backup
  ```
  
You can start with the backup as soon as the halt height `24404500` has been reached.

## 3. Pull the GiltConsensus v2 Image

Download the v2 Docker image from Docker Hub:

```bash
docker pull giltchain/gilt-consensus:0.2.9
````
...
```bash
docker run --rm -v "<GILTCONSENSUS_HOME>:/var/lib/gilt-consensus" giltchain/gilt-consensus:0.2.9 init <MONIKER> --chain-id giltconsensusv2-137
```
...
In that case, place the default config files under `GILTCONSENSUS_HOME/config` manually.  
...
And ensure the `seeds` and `persistent_peers` match the [default values](https://github.com/chatzoneai-spec/Gold-Chain/blob/master/bridge/gilt-consensus/packaging/templates/config/mainnet/config.toml#L216). 
...
* [Mainnet genesis](https://storage.googleapis.com/mainnet-giltconsensusv2-genesis/migrated_dump-genesis.json) *(available after pilot migration)*

Save the file with name `genesis.json`.

For example, you can use this command:
```bash
wget -O genesis.json https://storage.googleapis.com/mainnet-giltconsensusv2-genesis/migrated_dump-genesis.json
```
...
Download the checksum file available [here](https://storage.googleapis.com/mainnet-giltconsensusv2-genesis/migrated_dump-genesis.json.sha512).
And name it `genesis.json.sha512`.  

For example, you can use this command:
```bash
wget -O genesis.json.sha512 https://storage.googleapis.com/mainnet-giltconsensusv2-genesis/migrated_dump-genesis.json.sha512
```
...
Place the previously downloaded genesis.json in your `GILTCONSENSUS_HOME/config` directory.
...
## 12. Start the GiltConsensus v2 Container
...
for non-validators:
```bash
docker run -d --name gilt-consensus \
  -v "$GILTCONSENSUS_HOME:/var/lib/gilt-consensus" \
  -p 26656:26656 -p 26657:26657 -p 1317:1317 \
  giltchain/gilt-consensus:0.2.9 \
  start
```
and for validators:
```bash
docker run -d --name gilt-consensus \
  ...
  -p 26656:26656 -p 26657:26657 -p 1317:1317 \
  giltchain/gilt-consensus:0.2.9 \
  start --bridge --all --chain=gilttestnet --rest-server
```
...
## 13. Configure WebSocket for Gilt ↔ GiltConsensus Communication
...
## 14. Restart Gilt (If Step 13 Was Applied)

Use your container orchestration tool to restart the Gilt container.  
