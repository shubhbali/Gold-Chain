...
Gilt team can guide you through the process if you reach out.  
...
1. **Stop the GiltConsensus v2 container** (if running)  
   Depending on the state of your migration, you may be running either GiltConsensus v1 or v2.

2. **Restore the v1 `GILTCONSENSUS_HOME` backup**

   Replace the existing `GILTCONSENSUS_HOME` directory (containing `/config`, `/data` and potentially `/bridge`)
   with your previously saved v1 backup

3. **Reinstall the previous GiltConsensus v1 image**

   Pull the correct version of the GiltConsensus v1 Docker image.

   ```bash
   docker pull giltchain/giltconsensus:1.6.0
   ```

4. **Start the GiltConsensus v1 container**

   Launch it using the original configuration and data directory.

5. **Check the logs**

   Monitor startup logs to ensure GiltConsensus v1 is running as expected.
