...
paths:
  ...
  - "bridge/processor/**/*.go"
  - "cmd/giltconsensusd/cmd/stake.go"
---

# Smart Contract Interaction Security Review

All Ethereum/Gilt contract interactions flow through `helper.IContractCaller`. This is the trust boundary between GiltConsensus consensus and external chains. Bugs here can forge validator joins, steal funds, or corrupt checkpoints.
...
- **L1 contract upgrader** (Gilt governance): if L1 contracts (RootChain, StakeManager) are upgraded via proxy, ABI bindings become stale. Mismatched ABIs decode silently with wrong values -- no error, just garbage data flowing into consensus.
- **Front-running attacker** (L1 MEV): observes GiltConsensus's `SendCheckpoint` or `StakeFor` transaction in the L1 mempool and front-runs with a same-nonce higher-gas replacement, or sandwich-attacks to manipulate gas pricing.
- **Gilt chain attacker**: if Gilt's gRPC endpoint is compromised or a malicious Gilt node is connected, it can feed false root hashes, block headers, or milestone data to GiltConsensus's side handlers.
...
- `GetRootHash()` and `GetVoteOnHash()` call Gilt RPC -- validate response length (root hash = 32 bytes) and non-nil
...
- **Side handlers** (run in `ExtendVote`): CAN make RPC calls to L1/Gilt. Each validator calls independently. RPC failures, timeouts, or different results across validators are expected -- they produce different vote extension opinions (YES/NO/UNSPECIFIED). This is by design.
...
## Gilt gRPC Client (`x/gilt/grpc/`)

- Direct gRPC client to Gilt bypasses the `IContractCaller` abstraction -- apply identical security standards
- `HeaderByNumber`, `BlockByNumber`: validate response is non-nil, block number matches request, chain ID matches expected Gilt chain
- `GetRootHash`: response must be exactly 32 bytes, non-zero
- `TransactionReceipt`: verify receipt status and that the block is finalized on Gilt
...
- All processors must validate event data before constructing GiltConsensus transactions
- Check tx status against GiltConsensus before re-submitting to prevent duplicate transactions
...
- Gilt gRPC calls without nil/error checking on responses
