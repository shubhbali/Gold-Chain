...
- **`ExtendVote`**: per-validator, MAY make external RPC calls (L1, Gilt), MAY be non-deterministic. This is where side-handlers run and produce validator-specific opinions.
...
- **`VoteExtensionsEnableHeight`**: giltconsensus-v2 launched with VEs always enabled (final v1 block + 1). This value MUST NEVER be changed -- modifying it would be catastrophic for consensus.
...
- Milestone propositions affect **gilt finality** -- incorrect milestones compromise the entire finality guarantee of the Gilt PoS stack
- `GenMilestoneProposition` makes external RPC calls to Gilt (allowed in ExtendVote)
