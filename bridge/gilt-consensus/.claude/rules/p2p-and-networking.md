...
GiltConsensus delegates P2P consensus to CometBFT, but configures seeds, peers, RPC endpoints, and runs bridge listeners/broadcasters that connect to L1, Gilt, and RabbitMQ. Compromised networking allows eclipse attacks, transaction censorship, or forged chain data.
...
- **MITM on RPC** (network-level): if RPC connections to L1/Gilt use plaintext HTTP, a network-level attacker can intercept and modify responses -- feeding fake finalized blocks, wrong root hashes, or fabricated receipts. Affects all bridge operations.
...
- Seed nodes and persistent peers must come from verified config for known networks (mainnet, gilttestnet) -- never accept seeds from unvalidated runtime input
...
- **Reference the repo's own config templates** for current defaults and param tuning: `packaging/templates/config/mainnet/config.toml` and `packaging/templates/config/gilttestnet/config.toml`. CometBFT params set programmatically in `cmd/giltconsensusd/cmd/commands.go` (`initCometBFTConfig`) may differ from template defaults.

## RPC Endpoint Security

- All RPC URLs (CometBFT, Ethereum, Gilt) must come from config, never hardcoded in source
...
- Validate L1/Gilt chain IDs on initial connection and periodically -- prevents connecting to wrong network (testnet vs mainnet)
...
- SubGraph queries for self-healing: validate the subgraph endpoint is the official Gilt subgraph, verify response schema, cross-validate results against L1 when possible
...
- `BroadcastToGiltConsensus()` signs Cosmos SDK txs using the keyring -- verify keyring backend is `file` or `os` in production, never `test`
- `BroadcastToGiltChain()` sends raw Ethereum txs -- nonce management must be serialized (mutex or channel) to prevent nonce collisions in concurrent sends
