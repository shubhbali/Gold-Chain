# GiltConsensus v2 Protocol Buffers

This directory contains Protocol Buffer definitions for GiltConsensus v2, the validator layer for Gilt's Proof-of-Stake network.

## Overview

The proto definitions are organized by module, with each module representing a core component of the GiltConsensus system:

- **gilt**: Manages spans and block producer selection for the Gilt (child) chain
- **checkpoint**: Handles checkpoint submissions and acknowledgments for state finality
- **clerk**: Manages event records from the root chain to the gilt child chain (state syncs)
