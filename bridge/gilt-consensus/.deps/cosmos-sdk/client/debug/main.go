...
$ %s debug addr cosmos1e0jnq2sun3dzjh8p2xq95kk0expwmd7shwjpfg
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			decodeFns := []func(text string) ([]byte, error){
				hex.DecodeString,
				func(text string) ([]byte, error) { return sdk.AccAddressFromHex(text) },
				func(text string) ([]byte, error) { return sdk.ValAddressFromHex(text) },
				func(text string) ([]byte, error) { return sdk.ConsAddressFromHex(text) },
			}
