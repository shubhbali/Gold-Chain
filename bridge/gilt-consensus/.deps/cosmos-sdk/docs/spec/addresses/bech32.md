...
# From Bech32 to Hex on GiltConsensus

GiltConsensus prefers to use the hex address format wherever users must handle binary data, to remain compatible with etherum based addresses.

In the GiltConsensus implementation, keys and addresses may refer to a number of different roles in the network like accounts, validators etc., but all of them have been standardized to be hex encoded.
