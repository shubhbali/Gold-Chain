...
The burn functionality is not supported by GiltConsensus, hence in any case the deposit is refunded to the proposer.  
Also, GiltConsensus does not support delegation, hence the inheritance is not to be considered.
...
* **Vote:** Once deposit reaches a certain value (`MinDeposit`), proposal is
  confirmed and vote opens. GiltChain PoS validators can then send `TxGovVote`
...
according to the final tally of the proposal. In GiltConsensus, burn is not enabled, hence all the deposits will be refunded.
...
GiltChain PoS network, participants are validators. Other holders and users do not get the right to participate in governance. 
...
*Weighted Votes are not supported in GiltConsensus*.
...
*Inheritance is not supported in GiltConsensus, as there's no concept of tokens delegation*.
...
* set expectations for delegators (not supported in GiltConsensus)
...
const (
    ...
    ProposalTypeSoftwareUpgrade = "SoftwareUpgrade" // currently not supported in GiltConsensus
)
...
GiltChain PoS validators are able to send `MsgVote` transactions to cast their
...
```bash
giltconsd query gov deposit [proposal-id] [depositer-addr] [flags]
```

Example:

```bash
giltconsd query gov deposit 1 0x...
```
...
```bash
giltconsd query gov deposits [proposal-id] [flags]
```

Example:

```bash
giltconsd query gov deposits 1
```
...
```bash
giltconsd query gov param [param-type] [flags]
```

Example:

```bash
giltconsd query gov param voting
```
...
```bash
giltconsd query gov params [flags]
```

Example:

```bash
giltconsd query gov params
```
...
```bash
giltconsd query gov proposal [proposal-id] [flags]
```

Example:

```bash
giltconsd query gov proposal 1
```
...
```bash
giltconsd query gov proposals [flags]
```

Example:

```bash
giltconsd query gov proposals
```
...
```bash
giltconsd query gov proposer [proposal-id] [flags]
```

Example:

```bash
giltconsd query gov proposer 1
```
...
```bash
giltconsd query gov tally [proposal-id] [flags]
```

Example:

```bash
giltconsd query gov tally 1
```
...
```bash
giltconsd query gov vote [proposal-id] [voter-addr] [flags]
```

Example:

```bash
giltconsd query gov vote 1 0x...
```
...
```bash
giltconsd query gov votes [proposal-id] [flags]
```

Example:

```bash
giltconsd query gov votes 1
```
...
```bash
giltconsd tx gov deposit [proposal-id] [deposit] [flags]
```

Example:

```bash
giltconsd tx gov deposit 1 10000000pol --from 0x...
```
...
```bash
giltconsd tx gov draft-proposal
```
...
```bash
giltconsd tx gov submit-proposal [path-to-proposal-json] [flags]
```

Example:

```bash
giltconsd tx gov submit-proposal /path/to/proposal.json --from 0x...
```
...
```bash
giltconsd tx gov submit-legacy-proposal [command] [flags]
```

Example:

```bash
giltconsd tx gov submit-legacy-proposal --title="Test Proposal" --description="testing" --type="Text" --deposit="100000000pol" --from 0x...
```

Example (`param-change`):

```bash
giltconsd tx gov submit-legacy-proposal param-change proposal.json --from 0x...
```
...
```bash
giltconsd tx gov cancel-proposal [proposal-id] [flags]
```

Example:

```bash
giltconsd tx gov cancel-proposal 1 --from 0x...
```
...
```bash
giltconsd tx gov vote [command] [flags]
```

Example:

```bash
giltconsd tx gov vote 1 yes --from 0x...
```

##### weighted-vote

*Currently not supported in GiltConsensus.*

The `weighted-vote` command allows users to submit a weighted vote for a given governance proposal.

```bash
giltconsd tx gov weighted-vote [proposal-id] [weighted-options] [flags]
```

Example:

```bash
giltconsd tx gov weighted-vote 1 yes=0.5,no=0.5 --from 0x...
```
...
These improvements are being discussed in the upstream cosmos-sdk, hence not really part of any giltconsensus future plan.  
