...
// HV2: vesting accounts are not used in giltconsensus
...
func (k BaseKeeper) DelegateCoins(_ context.Context, _, _ sdk.AccAddress, _ sdk.Coins) error {
	return fmt.Errorf("DelegateCoins not supported in GiltConsensus since vesting and delegation are disabled")
	/*
...
func (k BaseKeeper) UndelegateCoins(_ context.Context, _, _ sdk.AccAddress, _ sdk.Coins) error {
	return fmt.Errorf("UndelegateCoins not supported in GiltConsensus since vesting and delegation is disabled")
	/*
		moduleAcc := k.ak.GetAccount(ctx, moduleAccAddr)
		if moduleAcc == nil {
			return errorsmod.Wrapf(sdkerrors.ErrUnknownAddress, "module account %s does not exist", moduleAccAddr)
...
func (k BaseKeeper) DelegateCoinsFromAccountToModule(
	...
	return fmt.Errorf("DelegateCoinsFromAccountToModule not supported in GiltConsensus since vesting and delegation are disabled")
	/*
		recipientAcc := k.ak.GetModuleAccount(ctx, recipientModule)
		if recipientAcc == nil {
			panic(errorsmod.Wrapf(sdkerrors.ErrUnknownAddress, "module account %s does not exist", recipientModule))
...
func (k BaseKeeper) UndelegateCoinsFromModuleToAccount(
	...
	return fmt.Errorf("UndelegateCoinsFromModuleToAccount not supported in GiltConsensus since vesting and delegation are disabled")
	/*
