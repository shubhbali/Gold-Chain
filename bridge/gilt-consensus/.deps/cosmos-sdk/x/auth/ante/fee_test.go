...
func TestEnsureMempoolFees(t *testing.T) {
	...
	// called once more than vanilla cosmos-sdk as decorator will not throw an error for giltconsensus (CheckTx is disabled)
	...
	_, err = antehandler(s.ctx, tx, false)
	require.Nil(t, err, "Decorator will not throw an error for giltconsensus as CheckTx is disabled")
