...
This document specifies the auth module of the GiltChain's PoS Cosmos SDK fork.
...
`giltconsd start ... --minimum-gas-prices=0.00001stake;0.05photinos`
...
In GiltConsensus, a default fee of 10^15 pol (`DefaultFeeInPol`) is deducted from the tx sender for every tx.
...
GiltConsensus does not currently support vesting accounts, so they will be treated as base accounts.
...
Some of these decorators may be disabled due to GiltConsensus's specific requirements.
...
```bash
giltconsd query auth account [address] [flags]
```

Example:

```bash
giltconsd query auth account cosmos1...
```
...
```bash
giltconsd query auth accounts [flags]
```

Example:

```bash
giltconsd query auth accounts
```
...
```bash
giltconsd query auth params [flags]
```

Example:

```bash
giltconsd query auth params
```
...
```bash
giltconsd tx sign tx.json --from $ALICE > tx.signed.json
```

The result is a signed transaction that can be broadcasted to the network thanks to the broadcast command.

More information about the `sign` command can be found running `giltconsd tx sign --help`.
...
```bash
giltconsd tx sign txs.json --from $ALICE > tx.signed.json
```

or

```bash 
giltconsd tx sign tx1.json tx2.json tx3.json --from $ALICE > tx.signed.json
```
...
More information about the `sign-batch` command can be found running `giltconsd tx sign-batch --help`.

#### `multi-sign`

The `multi-sign` command allows users to sign transactions that was generated offline by a multisig account.  
GiltConsensus does not support the multi signature functionality.

```bash
giltconsd tx multisign transaction.json k1k2k3 k1sig.json k2sig.json k3sig.json
```
...
giltconsd tx multi-sign transaction.json ms1 ms1p1sig.json ms1p2sig.json --signature-only --skip-signature-verification > ms1sig.json

# Then use the aggregated signatures and the other signatures to sign the final transaction
giltconsd tx multi-sign transaction.json k1ms1 k1sig.json ms1sig.json --skip-signature-verification
```
...
More information about the `multi-sign` command can be found running `giltconsd tx multi-sign --help`.
...
The multi signature functionality is not supported by GiltConsensus.

More information about the `multisign-batch` command can be found running `giltconsd tx multisign-batch --help`.
...
```bash
$ giltconsd tx validate-signatures tx.signed.json
Signers:
  0: 0x...
...
More information about the `validate-signatures` command can be found running `giltconsd tx validate-signatures --help`.
...
```bash
giltconsd tx broadcast tx.signed.json
```

More information about the `broadcast` command can be found running `giltconsd tx broadcast --help`.
