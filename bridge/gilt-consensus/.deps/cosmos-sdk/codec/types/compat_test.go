...
func (s *Suite) SetupTest() {
	...
	a, err := types.NewAnyWithValue(s.spot)
	s.Require().NoError(err)
	s.b = testdata.HasAnimal{Animal: a}
}
...
func (s *Suite) TestNested() {
	...
	a, err := types.NewAnyWithValue(&s.b)
	s.Require().NoError(err)
	hha := testdata.HasHasAnimal{HasAnimal: a}
	a2, err := types.NewAnyWithValue(&hha)
	s.Require().NoError(err)
	hhha := testdata.HasHasHasAnimal{HasHasAnimal: a2}
