...
func TestAnyPackUnpack(t *testing.T) {
	...
	// with cache
	a, err := types.NewAnyWithValue(spot)
...
func TestNested(t *testing.T) {
	...
	ha := &testdata.HasAnimal{Animal: a}
	a2, err := types.NewAnyWithValue(ha)
	require.NoError(t, err)

	hha := &testdata.HasHasAnimal{HasAnimal: a2}
	a3, err := types.NewAnyWithValue(hha)
	require.NoError(t, err)

	hhha := testdata.HasHasHasAnimal{HasHasAnimal: a3}
...
func TestAny_ProtoJSON(t *testing.T) {
	...
	var a2 types.Any
	err = jum.Unmarshal(strings.NewReader(json), &a2)
	require.NoError(t, err)
	var animal testdata.Animal
	err = registry.UnpackAny(&a2, &animal)
	require.NoError(t, err)
	require.Equal(t, spot, animal)
