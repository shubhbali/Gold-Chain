...
func TestNewGiltGRPCClient_URLParsing(t *testing.T) {
	t.Parallel()
	...
	t.Run("rejects invalid URL", func(t *testing.T) {
		...
		client, err := NewGiltGRPCClient("://invalid", logger)
		require.Error(t, err)
	...
	t.Run("rejects https URL with empty host", func(t *testing.T) {
		...
		client, err := NewGiltGRPCClient("https://", logger)
		require.Error(t, err)
	...
	t.Run("rejects unix URL with empty path", func(t *testing.T) {
		...
		client, err := NewGiltGRPCClient("unix://", logger)
		require.Error(t, err)
	...
	t.Run("rejects unsupported URL scheme", func(t *testing.T) {
		...
		client, err := NewGiltGRPCClient("ftp://example.com", logger)
		require.Error(t, err)
	...
	t.Run("rejects non-local address without scheme", func(t *testing.T) {
		...
		client, err := NewGiltGRPCClient("example.com:8545", logger)
		require.Error(t, err)
...
func TestGiltGRPCClient_Close(t *testing.T) {
	t.Parallel()
	...
	t.Run("close client with nil connection", func(t *testing.T) {
		...
		client := &GiltGRPCClient{conn: nil}
		require.NotPanics(t, func() {
			client.Close(logger)
		})
	})
