...
func TestErrInvalidChainID(t *testing.T) {
	...
	require.Equal(t, "invalid gilt chain id", types.ErrInvalidChainID.Error())
}
...
func TestErrInvalidLastGiltSpanID(t *testing.T) {
	require.NotNil(t, types.ErrInvalidLastGiltSpanID)
	require.Equal(t, "invalid last gilt span id", types.ErrInvalidLastGiltSpanID.Error())
}
...
func TestErrFailedToQueryGilt(t *testing.T) {
	require.NotNil(t, types.ErrFailedToQueryGilt)
	require.Equal(t, "failed to query gilt", types.ErrFailedToQueryGilt.Error())
}
...
func TestAllErrorsAreErrors(t *testing.T) {
	...
	err = types.ErrInvalidLastGiltSpanID
	require.NotNil(t, err)

	err = types.ErrInvalidSeedLength
	require.NotNil(t, err)

	err = types.ErrFailedToQueryGilt
	require.NotNil(t, err)
...
func TestErrorUniqueness(t *testing.T) {
	...
	errorMessages := []string{
		...
		types.ErrInvalidLastGiltSpanID.Error(),
		types.ErrInvalidSeedLength.Error(),
		types.ErrFailedToQueryGilt.Error(),
...
func TestErrorsCanBeWrapped(t *testing.T) {
	...
	require.Contains(t, wrappedErr.Error(), "invalid gilt chain id")
...
func TestErrorMessagesNotEmpty(t *testing.T) {
	...
	require.NotEmpty(t, types.ErrInvalidLastGiltSpanID.Error())
	require.NotEmpty(t, types.ErrInvalidSeedLength.Error())
	require.NotEmpty(t, types.ErrFailedToQueryGilt.Error())
...
func TestErrorsAreDescriptive(t *testing.T) {
	...
	errs := map[string]error{
		...
		"gilt":       types.ErrInvalidLastGiltSpanID,
		"seed":      types.ErrInvalidSeedLength,
		"query":     types.ErrFailedToQueryGilt,
...
func TestInvalidSpanErrors(t *testing.T) {
	...
	require.Contains(t, types.ErrInvalidLastGiltSpanID.Error(), "span")
}

func TestQueryErrorContainsKeyword(t *testing.T) {
	require.Contains(t, types.ErrFailedToQueryGilt.Error(), "query")
	require.Contains(t, types.ErrFailedToQueryGilt.Error(), "gilt")
}
...
func TestErrorsAreLowercase(t *testing.T) {
	...
	errs := []error{
		...
		types.ErrInvalidLastGiltSpanID,
		types.ErrInvalidSeedLength,
		types.ErrFailedToQueryGilt,
