...
// DefaultParams returns default parameters for gilt module
...
// ValidateBasic checks that the gilt parameters have valid values.
...
func validatePositiveIntForParam(i interface{}, paramName string) error {
	v, ok := i.(uint64)
	if !ok {
		return fmt.Errorf("invalid type provided %T for gilt param %s", i, paramName)
	}

	if v == 0 {
		return fmt.Errorf("invalid value provided %d for gilt param %s", v, paramName)
	}
