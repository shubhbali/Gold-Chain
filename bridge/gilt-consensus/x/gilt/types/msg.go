...
// Type returns the type of the x/gilt MsgProposeSpan.
func (msg MsgProposeSpan) Type() string {
	return EventTypeProposeSpan
}

// Type returns the type of the x/gilt MsgBackfillSpans.
...
func NewMsgSetProducerDowntime(
	producer string,
	startGiltBlock uint64,
	endGiltBlock uint64,
) *MsgSetProducerDowntime {
	return &MsgSetProducerDowntime{
		Producer: util.FormatAddress(producer),
		DowntimeRange: BlockRange{
			StartBlock: startGiltBlock,
			EndBlock:   endGiltBlock,
		},
	}
}

// Route returns the message route for x/gilt MsgProposeSpan.
func (msg MsgProposeSpan) Route() string {
	return RouterKey
}

// Route returns the message route for x/gilt MsgBackfillSpans.
