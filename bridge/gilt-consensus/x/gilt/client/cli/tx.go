...
var logger = helper.Logger.With("module", "x/gilt/cli")

// NewTxCmd returns a root CLI command handler for all x/gilt transaction commands.
func NewTxCmd() *cobra.Command {
	txCmd := &cobra.Command{
		Use:                        types.ModuleName,
		Short:                      "Gilt transaction subcommands",
...
func NewSpanProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			giltChainID := viper.GetString(FlagGiltChainId)
			if giltChainID == "" {
				return fmt.Errorf("GiltChainID cannot be empty")
			}
			...
			res, err := queryClient.GetGiltParams(cmd.Context(), &types.QueryParamsRequest{})
			if err != nil {
				return err
			}
			...
			msg := types.NewMsgProposeSpan(spanID, proposer, startBlock, startBlock+spanDuration-1, giltChainID, seed.Bytes(), nextSpanSeedResponse.SeedAuthor)
	...
	cmd.Flags().String(FlagGiltChainId, "", "--gilt-chain-id=<gilt-chain-id>")
	cmd.Flags().String(FlagStartBlock, "", "--start-block=<start-block-number>")
	cmd.Flags().String(flags.FlagChainID, "", "--chain-id=<chain-id>")

	if err := cmd.MarkFlagRequired(FlagGiltChainId); err != nil {
		fmt.Printf("PostSendProposeSpanTx | MarkFlagRequired | FlagGiltChainId Error: %v", err)
	}
...
func NewProducerDowntimeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			block, err := giltClient.BlockByNumber(clientCtx.CmdContext, nil)
			if err != nil {
				return fmt.Errorf("failed to get latest gilt block: %w", err)
			}
...
func calculateAverageBlocktime(clientCtx client.Context) (uint64, error) {
	giltClient := helper.GetGiltClient()
	currentBlock, err := giltClient.BlockNumber(clientCtx.CmdContext)
	if err != nil {
		return 0, fmt.Errorf("failed to get latest gilt block number: %w", err)
	}
	if currentBlock == 0 {
	...
	for i := uint64(0); i < blockTimesToGet; i++ {
		...
		block, err := giltClient.BlockByNumber(clientCtx.CmdContext, blockNumber)
		if err != nil {
			return 0, err
		}

		prevBlockNumber := big.NewInt(int64(currentBlock - ((blocksInBetween * i) + 1)))
		prevBlock, err := giltClient.BlockByNumber(clientCtx.CmdContext, prevBlockNumber)
		if err != nil {
			return 0, err
		}
