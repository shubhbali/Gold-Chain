...
// Keeper stores all the gilt related data
type Keeper struct {
...
// NewKeeper creates a new instance of the gilt Keeper
func NewKeeper(
	cdc codec.BinaryCodec,
	storeService store.KVStoreService,
	authority string,
...
// AddNewSpan adds new span for gilt to store and updates last span
...
// AddNewRawSpan adds the new span for gilt to store
...
func (k *Keeper) FreezeSet(ctx sdk.Context, id uint64, startBlock uint64, endBlock uint64, giltChainID string, seed common.Hash) error {
	logger := k.Logger(ctx)
...
func (k *Keeper) FetchNextSpanSeed(ctx context.Context, id uint64) (common.Hash, common.Address, error) {
	...
	giltBlock, author, err := k.getGiltBlockForSpanSeed(ctx, &seedSpan, id)
	if err != nil {
		return common.Hash{}, common.Address{}, err
	}

	blockHeader, err := k.contractCaller.GetGiltChainBlock(ctx, big.NewInt(int64(giltBlock)))
	if err != nil {
		k.Logger(ctx).Error("Error fetching block header from gilt chain while calculating next span seed", "error", err, "block", giltBlock)
		return common.Hash{}, common.Address{}, err
	}
	...
	logger.Debug("Fetched block for seed", "block", giltBlock, "author", author, spandIdLog, id)
...
// SetParams sets the gilt module's parameters.
func (k *Keeper) SetParams(ctx context.Context, params types.Params) error {
	return k.Params.Set(ctx, params)
}

// FetchParams gets the gilt module's parameters.
...
// getGiltBlockForSpanSeed returns the gilt block number and its producer whose hash is used as seed for the next span
func (k *Keeper) getGiltBlockForSpanSeed(ctx context.Context, seedSpan *types.Span, proposedSpanID uint64) (uint64, *common.Address, error) {
	logger := k.Logger(ctx)
	...
	logger.Debug("Getting gilt block for span seed", spandIdLog, seedSpan.GetId(), "proposed span id", proposedSpanID)

	if proposedSpanID == 1 {
		giltBlock = 1
		author, err = k.contractCaller.GetGiltChainBlockAuthor(big.NewInt(int64(giltBlock)))
		if err != nil {
			logger.Error("Error fetching first block for span seed", "error", err, "block", giltBlock)
			return 0, nil, err
		}

		logger.Debug("Returning first block author", "author", author, "block", giltBlock)

		return giltBlock, author, nil
	}
	...
	giltParams, err := k.FetchParams(ctx)
	if err != nil {
		logger.Error("Error fetching gilt params while getting GiltBlockForSpanSeed")
		return 0, nil, err
	}

	for giltBlock = seedSpan.EndBlock; giltBlock >= seedSpan.StartBlock; giltBlock -= giltParams.SprintDuration {
		author, err = k.contractCaller.GetGiltChainBlockAuthor(big.NewInt(int64(giltBlock)))
		if err != nil {
			logger.Error("Error fetching block author from gilt chain while calculating next span seed", "error", err, "block", giltBlock)
			return 0, nil, err
		}

		if _, exists := uniqueAuthors[author.Hex()]; !exists || len(seedSpan.ValidatorSet.Validators) == 1 {
			logger.Debug("Got author", "author", author, "block", giltBlock)
			return giltBlock, author, nil
		}

		if firstDiffFromLast == 0 && lastAuthor != nil && author.Hex() != lastAuthor.Hex() {
			firstDiffFromLast = giltBlock
		}
	}

	// if no unique author found, return the first different block author
	giltBlock = firstDiffFromLast
	if giltBlock == 0 {
		giltBlock = seedSpan.EndBlock
	}

	author, err = k.contractCaller.GetGiltChainBlockAuthor(big.NewInt(int64(giltBlock)))
	if err != nil {
		logger.Error("Error fetching end block author from gilt chain while calculating next span seed", "error", err, "block", giltBlock)
		return 0, nil, err
	}

	logger.Debug("Returning first different block author", "author", author, "block", giltBlock)
