...
func (s *KeeperTestSuite) TestProposeSpan() {
	require, ctx, giltKeeper, cmKeeper, msgServer := s.Require(), s.ctx, s.giltKeeper, s.chainManagerKeeper, s.msgServer

	testChainParams := chainmanagertypes.DefaultParams()
	testSpan := s.genTestSpans(1)[0]
	err := giltKeeper.AddNewSpan(ctx, testSpan)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestMsgUpdateParams() {
	ctx, require, keeper, queryClient, msgServer, params := s.ctx, s.Require(), s.giltKeeper, s.queryClient, s.msgServer, types.DefaultParams()
	...
	testCases := []struct {
		...
		},
		{
			...
			expErr:    true,
			expErrMsg: "invalid value provided 0 for gilt param sprint duration",
		},
		{
			...
			expErr:    true,
			expErrMsg: "invalid value provided 0 for gilt param span duration",
		},
		{
			...
			expErr:    true,
			expErrMsg: "invalid value provided 0 for gilt param producer count",
		},
		{
			name: "all good",
			input: &types.MsgUpdateParams{
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expErr {
				...
				res, err := queryClient.GetGiltParams(ctx, &types.QueryParamsRequest{})
				require.NoError(err)
...
func (s *KeeperTestSuite) TestVoteProducers() {
	require, ctx, giltKeeper, skMock, msgServer := s.Require(), s.ctx, s.giltKeeper, s.stakeKeeper, s.msgServer
	...
	sampleVotes := types.ProducerVotes{
		Votes: []uint64{10, 20, 30}, // Slice of uint64 as per gilt.pb.go
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectError: false,
			verifyState: func(tc types.MsgVoteProducers) {
				storedVotes, err := giltKeeper.GetProducerVotes(ctx, tc.VoterId)
				require.NoError(err)
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			...
			_ = giltKeeper.SetProducerVotes(ctx, tc.msg.VoterId, types.ProducerVotes{})
...
func (s *KeeperTestSuite) TestBackfillSpans() {
	require, ctx, giltKeeper, milestoneKeeper, cmKeeper, msgServer := s.Require(), s.ctx, s.giltKeeper, s.milestoneKeeper, s.chainManagerKeeper, s.msgServer
	...
	testcases := []struct {
		...
		{
			name: "invalid last giltconsensus span id",
			backfillSpans: types.MsgBackfillSpans{
				Proposer:        common.HexToAddress("someProposer").String(),
				ChainId:         testChainParams.ChainParams.GiltChainId,
				LatestSpanId:    2,
		...
		{
			name: "invalid last gilt span id",
			backfillSpans: types.MsgBackfillSpans{
				Proposer:        common.HexToAddress("someProposer").String(),
				ChainId:         testChainParams.ChainParams.GiltChainId,
				LatestSpanId:    1,
			...
			expErr: "invalid last gilt span id",
		},
		{
...
func (s *KeeperTestSuite) TestCanVoteProducers() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			...
			err := giltKeeper.CanVoteProducers(ctx)
