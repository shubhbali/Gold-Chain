...
import (
	...
	"github.com/giltchain/gilt-consensus/x/gilt/keeper"
	gilttestutil "github.com/giltchain/gilt-consensus/x/gilt/testutil"
...
func (s *KeeperTestSuite) SetupTest() {
	...
	chainManagerKeeper := gilttestutil.NewMockChainManagerKeeper(ctrl)
	s.chainManagerKeeper = chainManagerKeeper

	stakeKeeper := gilttestutil.NewMockStakeKeeper(ctrl)
	s.stakeKeeper = stakeKeeper

	milestoneKeeper := gilttestutil.NewMockMilestoneKeeper(ctrl)
	s.milestoneKeeper = milestoneKeeper

	s.contractCaller = mocks.IContractCaller{}
	s.ctx = ctx

	s.giltKeeper = keeper.NewKeeper(
		encCfg.Codec,
		storeService,
	...
	s.giltKeeper.SetContractCaller(&s.contractCaller)
	types.RegisterInterfaces(encCfg.InterfaceRegistry)

	giltParams := types.DefaultParams()
	err := s.giltKeeper.SetParams(ctx, giltParams)
	s.Require().NoError(err)

	queryHelper := baseapp.NewQueryServerTestHelper(ctx, encCfg.InterfaceRegistry)
	types.RegisterQueryServer(queryHelper, keeper.NewQueryServer(&s.giltKeeper))
	queryClient := types.NewQueryClient(queryHelper)

	s.queryClient = queryClient
	s.msgServer = keeper.NewMsgServerImpl(s.giltKeeper)
	s.encCfg = encCfg

	msgServer := keeper.NewMsgServerImpl(s.giltKeeper)
	s.msgServer = msgServer

	sideMsgServer := keeper.NewSideMsgServerImpl(&s.giltKeeper)
	s.sideMsgServer = sideMsgServer
}

func (s *KeeperTestSuite) TestAddNewSpan() {
	require, ctx, giltKeeper := s.Require(), s.ctx, s.giltKeeper
	...
	for _, tc := range testcases {
		s.T().Run(tc.name, func(t *testing.T) {
			err := giltKeeper.AddNewSpan(ctx, &tc.span)
			require.NoError(err)

			hasSpan, err := giltKeeper.HasSpan(ctx, tc.span.Id)
			require.NoError(err)
			require.True(hasSpan)

			span, err := giltKeeper.GetSpan(ctx, tc.span.Id)
			require.NoError(err)
...
func (s *KeeperTestSuite) TestAddNewRawSpan() {
	...
	for _, tc := range testcases {
		s.T().Run(tc.name, func(t *testing.T) {
			err := giltKeeper.AddNewRawSpan(ctx, &tc.span)
			require.NoError(err)
...
func (s *KeeperTestSuite) TestGetAllSpans() {
	...
	resSpans, err := giltKeeper.GetAllSpans(ctx)
	require.NoError(err)
	require.Equal(spans, resSpans)
}

func (s *KeeperTestSuite) TestFreezeSet() {
	require, stakeKeeper, giltKeeper, ctx := s.Require(), s.stakeKeeper, s.giltKeeper, s.ctx
	...
	for _, tc := range testcases {
		...
		s.T().Run(tc.name, func(t *testing.T) {
			...
			err := giltKeeper.SetParams(ctx, params)
			require.NoError(err)
			err = giltKeeper.AddNewSpan(ctx, &types.Span{
				Id: tc.id,
			})
			require.NoError(err)
			err = giltKeeper.FreezeSet(ctx, tc.id, tc.startBlock, tc.endBlock, "test-chain", tc.seed)
			require.NoError(err)

			span, err := giltKeeper.GetSpan(ctx, tc.id)
			require.NoError(err)
...
func (s *KeeperTestSuite) TestUpdateLastSpan() {
	...
	for _, tc := range testcases {
		s.T().Run(tc.name, func(t *testing.T) {
			resLastSpan, err := giltKeeper.GetLastSpan(ctx)
			require.NoError(err)
			require.Equal(tc.expPrevLastSpan, &resLastSpan)

			err = giltKeeper.UpdateLastSpan(ctx, tc.expNewLastSpan.Id)
			require.NoError(err)
...
func (s *KeeperTestSuite) TestFetchNextSpanSeed() {
	...
	for block := spans[1].EndBlock - (2 * giltParams.SprintDuration); block >= spans[1].StartBlock; block -= giltParams.SprintDuration {
		s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(block))).Return(&val1Addr, nil)
	}

	seedBlock3 := spans[2].EndBlock - (2 * giltParams.SprintDuration)
	s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(spans[2].EndBlock))).Return(&val1Addr, nil)
	s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(spans[2].EndBlock-giltParams.SprintDuration))).Return(&val2Addr, nil)
	s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(seedBlock3))).Return(&val3Addr, nil)

	seedBlock4 := spans[3].EndBlock - giltParams.SprintDuration
	s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(spans[3].EndBlock))).Return(&val1Addr, nil)

	for block := spans[3].EndBlock; block >= spans[3].StartBlock; block -= giltParams.SprintDuration {
		s.contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(int64(block))).Return(&val2Addr, nil)
	}
	...
	for _, tc := range testcases {
		err := giltKeeper.StoreSeedProducer(ctx, tc.lastSpanId, tc.lastSeedProducer)
		require.NoError(err)
	...
	err = giltKeeper.StoreSeedProducer(ctx, lastSpanID+1, &v1A)
	require.NoError(err)

	for _, tc := range testcases {
		s.T().Run(tc.name, func(t *testing.T) {
			seed, seedAuthor, err := giltKeeper.FetchNextSpanSeed(ctx, tc.lastSpanId+2)
			require.NoError(err)
...
func (s *KeeperTestSuite) TestProposeSpanOne() {
	require, ctx, giltKeeper, contractCaller := s.Require(), s.ctx, s.giltKeeper, &s.contractCaller
	...
	seedBlock1 := int64(1)
	contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(seedBlock1)).Return(&val1Addr, nil)

	blockHeader1 := ethTypes.Header{Number: big.NewInt(seedBlock1)}
	blockHash1 := blockHeader1.Hash()
	contractCaller.On("GetGiltChainBlock", mock.Anything, big.NewInt(seedBlock1)).Return(&blockHeader1, nil)

	seed, seedAuthor, err := giltKeeper.FetchNextSpanSeed(ctx, 1)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestGetSeedProducer() {
	giltKeeper := s.giltKeeper

	producer := common.HexToAddress("0xc0ffee254729296a45a3885639AC7E10F9d54979")
	err := giltKeeper.StoreSeedProducer(s.ctx, 1, &producer)
	s.Require().NoError(err)

	author, err := giltKeeper.GetSeedProducer(s.ctx, 1)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestParamsGetterSetter() {
	...
	require.NoError(giltKeeper.SetParams(ctx, expParams))
	resParams, err := giltKeeper.FetchParams(ctx)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestSpanByBlockNumber() {
	...
	for i := range spans {
		err := giltKeeper.AddNewSpan(ctx, &spans[i])
		require.NoError(err)
	}
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			span, err := giltKeeper.SpanByBlockNumber(ctx, tc.blockNumber)
