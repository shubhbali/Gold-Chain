...
// NewSideMsgServerImpl returns an implementation of the x/gilt SideMsgServer interface for the provided Keeper.
...
func (srv sideMsgServer) SideHandleMsgSpan(ctx sdk.Context, msgI sdk.Msg) sidetxs.Vote {
	var err error
	start := time.Now()
	defer recordGiltMetric(api.SideHandleMsgSpanMethod, api.SideType, start, &err)
	...
	if err != nil {
		logger.Error(giltconsensusTypes.ErrMsgFailedToGetChainParams+" while executing gilt side handler", "error", err)
		return sidetxs.Vote_VOTE_NO
	}

	if chainParams.ChainParams.GiltChainId != msg.ChainId {
		logger.Error("Invalid gilt chain id in gilt side handler", "expected", chainParams.ChainParams.GiltChainId, "got", msg.ChainId)
		return sidetxs.Vote_VOTE_NO
	}

	// verify seed length
	if len(msg.Seed) != common.HashLength {
		logger.Error("Invalid seed length in gilt side handler", "expected", common.HashLength, "got", len(msg.Seed))
		return sidetxs.Vote_VOTE_NO
	}
	...
	childBlock, err := srv.k.contractCaller.GetGiltChainBlock(ctx, nil)
	if err != nil {
	...
	if lastSpan.Id+1 != msg.SpanId || msg.StartBlock != lastSpan.EndBlock+1 || msg.EndBlock <= msg.StartBlock {
		logger.Error(giltconsensusTypes.ErrMsgBlocksNotInContinuity+" in gilt side handler",
...
func (srv sideMsgServer) SideHandleSetProducerDowntime(ctx sdk.Context, msgI sdk.Msg) sidetxs.Vote {
	var err error
	start := time.Now()
	defer recordGiltMetric(api.SideHandleMsgSetProducerDowntimeMethod, api.SideType, start, &err)
	...
	if msg.DowntimeRange.StartBlock >= msg.DowntimeRange.EndBlock {
		logger.Error("Downtime range start block must be less than end block in gilt side handler",
	...
	if msg.DowntimeRange.EndBlock-msg.DowntimeRange.StartBlock < types.PlannedDowntimeMinRange {
		logger.Error("Time range for planned downtime is too small in gilt side handler",
			"startBlock", msg.DowntimeRange.StartBlock,
			"endBlock", msg.DowntimeRange.EndBlock,
		)
		return sidetxs.Vote_VOTE_NO
...
func (srv sideMsgServer) PostHandleMsgSpan(ctx sdk.Context, msgI sdk.Msg, sideTxResult sidetxs.Vote) error {
	var err error
	start := time.Now()
	defer recordGiltMetric(api.PostHandleMsgSpanMethod, api.PostType, start, &err)
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(giltconsensusTypes.AttributeKeyTxHash, common.BytesToHash(hash).Hex()), // tx hash
...
func (srv sideMsgServer) PostHandleMsgBackfillSpans(ctx sdk.Context, msgI sdk.Msg, sideTxResult sidetxs.Vote) error {
	var err error
	start := time.Now()
	defer recordGiltMetric(api.PostHandleMsgBackfillSpansMethod, api.PostType, start, &err)
	...
	giltSpans := types.GenerateGiltCommittedSpans(latestMilestone.EndBlock, &latestSpan)
	spansOverlap := 0
	for i := range giltSpans {
		if _, err := srv.k.GetSpan(ctx, giltSpans[i].Id); err == nil {
			spansOverlap++
		}
		if spansOverlap > 1 {
			logger.Error("More than one span overlap detected", spanIdLog, giltSpans[i].Id)
			return fmt.Errorf("more than one span overlap detected for span id: %d", giltSpans[i].Id)
		}
		if err = srv.k.AddNewSpan(ctx, &giltSpans[i]); err != nil {
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(giltconsensusTypes.AttributeKeyTxHash, common.BytesToHash(hash).Hex()),
			sdk.NewAttribute(giltconsensusTypes.AttributeKeySideTxResult, sideTxResult.String()),
			sdk.NewAttribute(types.AttributesKeyLatestSpanId, strconv.FormatUint(msg.LatestSpanId, 10)),
			sdk.NewAttribute(types.AttributesKeyLatestGiltSpanId, strconv.FormatUint(giltSpans[0].Id, 10)),
		),
	})
...
func (srv sideMsgServer) PostHandleSetProducerDowntime(ctx sdk.Context, msgI sdk.Msg, sideTxResult sidetxs.Vote) error {
	var err error
	start := time.Now()
	defer recordGiltMetric(api.PostHandleMsgSetProducerDowntimeMethod, api.PostType, start, &err)
	...
	if err := srv.k.AddNewVeBlopSpan(ctx, validatorId, msg.DowntimeRange.StartBlock, msg.DowntimeRange.StartBlock+params.SpanDuration, lastSpan.GiltChainId, latestActiveProducer, uint64(ctx.BlockHeight())); err != nil {
...
// recordGiltMetric records metrics for side and post-handlers.
func recordGiltMetric(method string, apiType string, start time.Time, err *error) {
	success := *err == nil
	api.RecordAPICallWithStart(api.GiltSubsystem, method, apiType, success, start)
}
