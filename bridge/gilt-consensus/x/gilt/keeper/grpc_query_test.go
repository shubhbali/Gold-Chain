...
func (s *KeeperTestSuite) TestGetLatestSpan() {
	...
	for _, span := range spans {
		err := s.giltKeeper.AddNewSpan(ctx, span)
		require.NoError(err)
	}
...
func (s *KeeperTestSuite) TestGetNextSpan() {
	...
	keeper, stakeKeeper := s.giltKeeper, s.stakeKeeper
	...
	contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(0)).Return(&valAddr, nil).Times(1)
	...
	req := &types.QueryNextSpanRequest{
		SpanId:     2,
		StartBlock: 102,
		GiltChainId: firstSpan[0].GiltChainId,
	}
...
func (s *KeeperTestSuite) TestGetNextSpanSeed() {
	...
	lastGiltBlockHeader := &ethTypes.Header{Number: big.NewInt(1)}
	contractCaller.On("GetGiltChainBlock", mock.Anything, big.NewInt(1)).Return(lastGiltBlockHeader, nil).Times(1)
	contractCaller.On("GetGiltChainBlockAuthor", big.NewInt(1)).Return(&valAddr, nil).Times(1)

	res, err := queryClient.GetNextSpanSeed(ctx, &types.QueryNextSpanSeedRequest{Id: 1})
	require.NoError(err)
	require.NotNil(res)
	require.Equal(&types.QueryNextSpanSeedResponse{Seed: lastGiltBlockHeader.Hash().String(), SeedAuthor: valAddr.Hex()}, res)
}

func (s *KeeperTestSuite) TestGetParams() {
	require, ctx, queryClient, keeper := s.Require(), s.ctx, s.queryClient, s.giltKeeper
...
func (s *KeeperTestSuite) TestGetSpanById() {
	require, ctx, keeper, queryClient := s.Require(), s.ctx, s.giltKeeper, s.queryClient
...
func (s *KeeperTestSuite) TestGetProducerVotes() {
	require, ctx, queryClient, stakeKeeper, keeper := s.Require(), s.ctx, s.queryClient, s.stakeKeeper, s.giltKeeper
