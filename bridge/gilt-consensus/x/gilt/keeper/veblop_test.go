...
func (s *KeeperTestSuite) TestCalculateProducerSet() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{prodB, prodA}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val2.ValId, types.ProducerVotes{Votes: []uint64{prodB, prodC}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val3.ValId, types.ProducerVotes{Votes: []uint64{prodA, prodC}}))
			},
		...
		},
		{
			...
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, 1, types.ProducerVotes{Votes: []uint64{prodA}}))
			},
			expectedCandidates: []uint64{},
			expectedError:      false,
		},
		{
			...
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{prodA}}))
			},
			expectedCandidates: []uint64{},
			expectedError:      false,
		},
		{
			...
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, 1, types.ProducerVotes{Votes: []uint64{prodA}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, 2, types.ProducerVotes{Votes: []uint64{prodB}}))
			},
			expectedCandidates: []uint64{},
			expectedError:      false,
		},
		{
			...
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{prodA}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val2.ValId, types.ProducerVotes{Votes: []uint64{prodA, prodB}}))
			},
		...
		},
		{
			...
			validatorDetails:   map[uint64]staketypes.Validator{1: val1, 2: val2, 3: val3},
			setupVotes: func() {
				require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{prodC, prodA}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val2.ValId, types.ProducerVotes{Votes: []uint64{prodC, prodB}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val3.ValId, types.ProducerVotes{Votes: []uint64{prodC, prodB}}))
			},
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			err := giltKeeper.ClearProducerVotes(ctx)
			require.NoError(err)

			require.NoError(giltKeeper.SetParams(ctx, tc.params))
			...
			candidates, err := giltKeeper.CalculateProducerSet(ctx, tc.params.ProducerCount)
...
func (s *KeeperTestSuite) TestSelectNextSpanProducer() {
	...
	err := s.giltKeeper.AddNewSpan(ctx, &types.Span{
		Id:         0,
		StartBlock: 0,
		EndBlock:   1001,
	...
	testCases := []struct {
		...
		},
		{
			...
			setupSpan: func() {
				...
				require.NoError(giltKeeper.AddNewSpan(ctx, &span))
			},
			setupProducerVotes: func() {},
			producerSetLimit:   3,
			setupValidatorSet:  func() {},
		...
		},
		{
			...
			setupProducerVotes: func() {
				...
				require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{2, 3}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val2.ValId, types.ProducerVotes{Votes: []uint64{2, 3}}))
				require.NoError(giltKeeper.SetProducerVotes(ctx, val3.ValId, types.ProducerVotes{Votes: []uint64{2, 3}}))
			},
			producerSetLimit: 2,
			setupValidatorSet: func() {
				valSet := staketypes.ValidatorSet{
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			params, err := giltKeeper.GetParams(ctx)
			require.NoError(err)

			startBlock := lastSpan.EndBlock + 1
			endBlock := lastSpan.EndBlock + params.SpanDuration

			result, err := giltKeeper.SelectNextSpanProducer(ctx, 1, tc.activeValidatorIDs, tc.producerSetLimit, startBlock, endBlock)
...
func (s *KeeperTestSuite) TestCalculateProducerSet_TotalPotentialProducersVoteCap() {
	...
	require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{prodA, prodB}}))

	// Call CalculateProducerSet
	candidates, err := giltKeeper.CalculateProducerSet(ctx, 2)
...
func (s *KeeperTestSuite) TestAddNewVeBlopSpan() {
	...
	require.NoError(giltKeeper.AddNewSpan(ctx, &types.Span{
		Id:         0,
		StartBlock: 0,
		EndBlock:   199,
		SelectedProducers: []staketypes.Validator{
	...
	require.NoError(giltKeeper.SetProducerVotes(ctx, val1.ValId, types.ProducerVotes{Votes: []uint64{2, 3}}))
	require.NoError(giltKeeper.SetProducerVotes(ctx, val2.ValId, types.ProducerVotes{Votes: []uint64{1, 3}}))
	require.NoError(giltKeeper.SetProducerVotes(ctx, val3.ValId, types.ProducerVotes{Votes: []uint64{3, 1}}))
	...
	endBlock := uint64(300)
	giltChainID := "137"
	giltconsensusBlock := uint64(5000)

	err := giltKeeper.AddNewVeBlopSpan(ctx, 1, startBlock, endBlock, giltChainID, active, giltconsensusBlock)
	require.NoError(err)

	span, err := giltKeeper.GetSpan(ctx, 1)
	require.NoError(err)
	require.Equal(uint64(1), span.Id)
	require.Equal(startBlock, span.StartBlock)
	require.Equal(endBlock, span.EndBlock)
	require.Equal(giltChainID, span.GiltChainId)
	...
	lastBlock, err := giltKeeper.GetLastSpanBlock(ctx)
	require.NoError(err)
	require.Equal(giltconsensusBlock, lastBlock)
}
