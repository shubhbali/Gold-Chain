...
func (k *Keeper) AddNewVeBlopSpan(ctx sdk.Context, currentProducer uint64, startBlock uint64, endBlock uint64, giltChainID string, activeValidatorIDs map[uint64]struct{}, giltconsensusBlock uint64) error {
	logger := k.Logger(ctx)
	...
	return k.SetLastSpanBlock(ctx, giltconsensusBlock)
}
...
func (k *Keeper) CalculateProducerSet(ctx context.Context, producerSetLimit uint64) ([]uint64, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s: %w", giltconsensusTypes.ErrMsgFailedToGetValidatorSet, err)
	}
