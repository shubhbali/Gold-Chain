...
// NewMsgServerImpl returns an implementation of the gilt MsgServer interface
...
func (srv msgServer) ProposeSpan(ctx context.Context, msg *types.MsgProposeSpan) (*types.MsgProposeSpanResponse, error) {
	var err error
	start := time.Now()
	defer recordGiltTransactionMetric(api.ProposeSpanMethod, start, &err)
	...
	if err != nil {
		logger.Error(giltconsensusTypes.ErrMsgInvalidProposerAddress, "error", err)
		return nil, errors.Wrapf(err, giltconsensusTypes.ErrMsgInvalidProposerAddress)
	}

	// verify chain id
	chainParams, err := srv.ck.GetParams(ctx)
	if err != nil {
		logger.Error(giltconsensusTypes.ErrMsgFailedToGetChainParams, "error", err)
		return nil, errors.Wrapf(err, giltconsensusTypes.ErrMsgFailedToGetChainParams)
	}

	if !helper.ValidateChainID(msg.ChainId, chainParams.ChainParams.GiltChainId, logger, "gilt") {
	...
	if lastSpan.Id+1 != msg.SpanId || msg.StartBlock != lastSpan.EndBlock+1 || msg.EndBlock <= msg.StartBlock {
		logger.Error(giltconsensusTypes.ErrMsgBlocksNotInContinuity,
...
// UpdateParams defines a method to update the params in x/gilt module.
func (srv msgServer) UpdateParams(ctx context.Context, msg *types.MsgUpdateParams) (*types.MsgUpdateParamsResponse, error) {
	var err error
	start := time.Now()
	defer recordGiltTransactionMetric(api.GiltUpdateParamsMethod, start, &err)
...
func (srv msgServer) VoteProducers(ctx context.Context, msg *types.MsgVoteProducers) (*types.MsgVoteProducersResponse, error) {
	var err error
	start := time.Now()
	defer recordGiltTransactionMetric(api.VoteProducersMethod, start, &err)
...
func (srv msgServer) BackfillSpans(ctx context.Context, msg *types.MsgBackfillSpans) (*types.MsgBackfillSpansResponse, error) {
	var err error
	start := time.Now()
	defer recordGiltTransactionMetric(api.BackfillSpansMethod, start, &err)
	...
	logger.Debug(helper.LogValidatingExternalCall("MsgSpanBackfill"),
		...
		"latestGiltSpanId", msg.LatestGiltSpanId,
		"chainId", msg.ChainId,
	)
	...
	if msg.LatestGiltSpanId <= msg.LatestSpanId {
		logger.Error("Invalid gilt span id, expected greater than latest span id",
			"latestSpanId", latestSpan.Id,
			"latestGiltSpanId", msg.LatestGiltSpanId,
		)
		return nil, types.ErrInvalidLastGiltSpanID
	}
	...
	giltLastUsedSpan, err := srv.GetSpan(ctx, msg.LatestSpanId)
	if err != nil {
		logger.Error("Failed to get last used gilt span", "error", err)
		return nil, errors.Wrapf(err, "failed to get last used gilt span")
	}

	giltSpanId, err := types.CalcCurrentGiltSpanId(latestMilestone.EndBlock, &giltLastUsedSpan)
	if err != nil {
		logger.Error("Failed to calculate gilt span id", "error", err)
		return nil, errors.Wrapf(err, "failed to calculate gilt span id")
	}

	if giltSpanId != msg.LatestGiltSpanId {
		logger.Error(
			"gilt span id mismatch",
			"calculatedGiltSpanId", giltSpanId,
			"msgLatestGiltSpanId", msg.LatestGiltSpanId,
...
func (srv msgServer) SetProducerDowntime(ctx context.Context, msg *types.MsgSetProducerDowntime) (*types.MsgSetProducerDowntimeResponse, error) {
	var err error
	start := time.Now()
	defer recordGiltTransactionMetric(api.ProducerDowntimeMethod, start, &err)
...
func recordGiltTransactionMetric(method string, start time.Time, err *error) {
	success := *err == nil
	api.RecordAPICallWithStart(api.GiltSubsystem, method, api.TxType, success, start)
}
