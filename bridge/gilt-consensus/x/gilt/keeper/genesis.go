...
// InitGenesis sets gilt information for genesis.
func (k Keeper) InitGenesis(ctx context.Context, data *types.GenesisState) {
	if err := k.SetParams(ctx, data.Params); err != nil {
		panic(fmt.Sprintf("error while setting gilt params during InitGenesis: %v", err))
	}
...
// ExportGenesis returns a GenesisState for gilt.
