...
* A `StateReceiver` is a contract deployed on L2 (Gilt) responsible for receiving state-sync events.
* A `EventRecord` is a record of the state-sync event stored in the giltconsensus state.
...
(Gilt in case of PoS) about the state changes in the L1. 
...
It's a mechanism for state-management between the Ethereum and Gilt chain.
...
* `giltChainID` is the chain id of the gilt chain.
* `recordTime` is the time at which the event was recorded in giltconsensus state.

The bridge will listen to the state-sync events from L1 and generate a txn with `MsgEventRecord` which is responsible for validating events from `StateSender` contract and storing the `EventRecord` on the giltconsensus state for gilt to use.

```protobuf
message MsgEventRecord {
  option (amino.name) = "giltconsensusv2/clerk/MsgEventRecord";
  option (gogoproto.equal) = false;
  option (gogoproto.goproto_getters) = false;
...
```bash
giltconsd tx clerk handle-msg-event-record [from] [tx-hash] [log-index] [block-number] [contract-address] [data] [id] [chain-id]
```
...
```bash
giltconsd query clerk record [record-id]
```

```bash
giltconsd query clerk record-list [page] [limit]
```

```bash
giltconsd query clerk record-list-with-time [from-id] [to-time]
```

```bash
giltconsd query clerk record-sequence [tx-hash] [log-index]
```

```bash
giltconsd query clerk is-old-tx [tx-hash] [log-index]
```

```bash
giltconsd query clerk latest-record-id
```

```bash
giltconsd query clerk record-count
```

### GRPC Endpoints

The endpoints and the params are defined in the [clerk/query.proto](/proto/giltconsensusv2/clerk/query.proto) file.
Please refer to them for more information about the optional params.

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.clerk.Query/GetRecordList
```

```bash
grpcurl -plaintext -d '{"record_id": <>}' localhost:9090 giltconsensusv2.clerk.Query/GetRecordById
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.clerk.Query/GetRecordListWithTime
```

```bash
grpcurl -plaintext -d '{"tx_hash": <>, "log_index": <>}' localhost:9090 giltconsensusv2.clerk.Query/GetRecordSequence
```

```bash
grpcurl -plaintext -d '{"tx_hash": <>, "log_index": <>}' localhost:9090 giltconsensusv2.clerk.Query/IsClerkTxOld
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.clerk.Query/GetLatestRecordId
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.clerk.Query/GetRecordCount
```
