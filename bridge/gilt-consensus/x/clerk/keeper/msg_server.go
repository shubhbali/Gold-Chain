...
func (srv msgServer) HandleMsgEventRecord(ctx context.Context, msg *types.MsgEventRecord) (*types.MsgEventRecordResponse, error) {
	...
	if err != nil {
		logger.Error(giltconsensusTypes.ErrMsgFailedToGetChainManagerParams, "error", err)
		return nil, err
	}
	...
	// check chain id
	if !helper.ValidateChainID(msg.ChainId, chainParams.GiltChainId, logger, "clerk") {
		return nil, errors.Wrapf(sdkerrors.ErrInvalidRequest, "invalid gilt chain id")
	}
	...
	if srv.HasRecordSequence(ctx, sequence) {
		logger.Error(giltconsensusTypes.ErrMsgEventAlreadyProcessed, giltconsensusTypes.LogKeySequence, sequence)
