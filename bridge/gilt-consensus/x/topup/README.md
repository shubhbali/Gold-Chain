...
GiltConsensus Topup is an amount that will be used to pay fees on the giltconsensus chain.
...
1. When new validator joins, they can mention a `topup` amount as top-up in addition to the staked amount, which will be
   moved as balance on the giltconsensus chain to pay fees on GiltConsensus.
2. A user can directly call the top-up function on the staking smart contract on Ethereum to increase top-up balance on
   GiltConsensus.

## Flow

The GiltConsensus Top-up Mechanism facilitates the management of validator fees on the GiltConsensus v2 chain by allowing deposits from the Ethereum (L1) root chain.  
This mechanism ensures validators have enough balances on GiltConsensus to cover operational fees.  
The system integrates staking smart contract on Ethereum with GiltConsensus custom x/topup and x/checkpoint modules and a bridge component to enable seamless cross-chain fee management.  

### Top-Up Funding Methods
There are two primary ways to fund a validator’s fee balance on GiltConsensus:  
- During Validator Initialization: When a new validator joins, they can specify a top-up amount in addition to the staked amount. This top-up is transferred to GiltConsensus as an initial balance.
- Direct Top-Up: Any user can invoke the top-up function on the Ethereum staking smart contract to increase the validators’ top-up balance on GiltConsensus.
...
- the Top-up Fee Processor task, upon detecting such an event, triggers the `sendTopUpFeeToGiltConsensus` execution.
The task:
— Decodes the Ethereum log.  
– Verifies the event hasn’t already been processed.  
— Broadcasts a `MsgTopupTx` to the GiltConsensus chain.  
In addition to the automated broadcasting of `MsgTopupTx` transactions, it is also possible to manually craft and submit these transactions at the GiltConsensus layer.  
This fallback mechanism is used in scenarios where issues arise in bridging or processing Ethereum events.  

### GiltConsensus x/topup Module Implementation
Two core messages are defined in the x/topup module for fee management:
- `MsgTopupTx`: Handles minting the top-up amount on GiltConsensus based on Ethereum events.  
...
For the top-up to be accepted, the `MsgTopupTx.Fee` must be at least equal to the `DefaultFeeWantedPerTx` amount. The top-up processing on GiltConsensus v2 involves:  
...
- `MsgWithdrawFeeTx`: Allows validators to withdraw fees from GiltConsensus back to Ethereum.
...
`MsgTopupTx` is responsible for minting balance to an address on GiltConsensus based on Ethereum chain's `TopUpEvent` on
...
message MsgTopupTx {
  ...
  option (amino.name) = "giltconsensusv2/topup/MsgTopupTx";
  string proposer = 1 [
...
`MsgWithdrawFeeTx` is responsible for withdrawing balance from GiltConsensus to the Ethereum chain.
A Validator can withdraw any amount from GiltConsensus.
...
message MsgWithdrawFeeTx {
  option (cosmos.msg.v1.signer) = "proposer";
  option (amino.name) = "giltconsensusv2/topup/MsgWithdrawFeeTx";
...
```bash
giltconsd tx topup handle-topup-tx [proposer] [user] [fee] [tx_hash] [log_index] [block_number]
```

#### Withdraw fee

```bash
giltconsd tx topup withdraw-fee [proposer] [amount]
```
...
```bash
giltconsd query topup topup-sequence [tx_hash] [log_index]
```

```bash
giltconsd query topup is-old-tx [tx_hash] [log_index]
```

```bash
giltconsd query topup dividend-account [address]
```

```bash
giltconsd query topup dividend-account-root
```

```bash
giltconsd query topup account-proof [address]
```

```bash
giltconsd query topup verify-account-proof [address] [proof]
```

### GRPC Endpoints

The endpoints and the params are defined in the [topup/query.proto](/proto/giltconsensusv2/topup/query.proto) file.
Please refer to them for more information about the optional params.

```bash
grpcurl -plaintext -d '{"tx_hash": <>, "log_index": <>}' localhost:9090 giltconsensusv2.topup.Query/IsTopupTxOld
```

```bash
grpcurl -plaintext -d '{"tx_hash": <>, "log_index": <>}' localhost:9090 giltconsensusv2.topup.Query/GetTopupTxSequence
```

```bash
grpcurl -plaintext -d '{"address": <>}' localhost:9090 giltconsensusv2.topup.Query/GetDividendAccountByAddress
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.topup.Query/GetDividendAccountRootHash
```

```bash
grpcurl -plaintext -d '{"address": <>, "proof": <>}' localhost:9090 giltconsensusv2.topup.Query/VerifyAccountProofByAddress
```

```bash
grpcurl -plaintext -d '{"address": <>}' localhost:9090 giltconsensusv2.topup.Query/GetAccountProofByAddress
```
