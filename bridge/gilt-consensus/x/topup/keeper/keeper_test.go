...
import (
	...
	"github.com/giltchain/gilt-consensus/x/topup/testutil"
...
func (s *KeeperTestSuite) TestDividendAccountTree() {
	...
	accountRoot, err := giltconsensusTypes.GetAccountRootHash(divAccounts)
	require.NotNil(accountRoot)
	require.NoError(err)

	accountProof, index, err := giltconsensusTypes.GetAccountProof(divAccounts, AccountHash)
