...
import (
	...
	"github.com/giltchain/gilt-consensus/x/topup/types"
)
...
func (s sideMsgServer) SideHandleTopupTx(ctx sdk.Context, msgI sdk.Msg) sidetxs.Vote {
	...
	if err != nil || eventLog == nil {
		logger.Error(giltconsensusTypes.ErrMsgErrorFetchingLog)
		return sidetxs.Vote_VOTE_NO
	}
	...
	if err != nil {
		logger.Error("Error converting msg.User to bytes", giltconsensusTypes.LogKeyError, err)
		return sidetxs.Vote_VOTE_NO
	}

	eventLogBytes, err := ac.StringToBytes(eventLog.User.String())
	if err != nil {
		logger.Error("Error converting eventLog.User to bytes", giltconsensusTypes.LogKeyError, err)
		return sidetxs.Vote_VOTE_NO
	}
...
func (s sideMsgServer) PostHandleTopupTx(ctx sdk.Context, msgI sdk.Msg, sideTxResult sidetxs.Vote) error {
	...
	if !helper.IsSideTxApproved(sideTxResult) {
		...
		return errors.New(giltconsensusTypes.ErrMsgSideTxRejected)
	}
	...
	if err != nil {
		logger.Error("Error while fetching older topup sequence",
			giltconsensusTypes.LogKeySequence, sequence,
			giltconsensusTypes.LogKeyLogIndex, msg.LogIndex,
			giltconsensusTypes.LogKeyBlockNumber, msg.BlockNumber,
			giltconsensusTypes.LogKeyError, err)
		return err
	}
	if exists {
	...
	if err != nil {
		logger.Error("Error while minting coins to x/topup module", "topupAmount", topupAmount, giltconsensusTypes.LogKeyError, err)
		return err
	}

	err = s.k.BankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, sdk.MustAccAddressFromHex(user), topupAmount)
	if err != nil {
		logger.Error("Error while sending coins from x/topup module to user", "user", user, "topupAmount", topupAmount, giltconsensusTypes.LogKeyError, err)
		return err
	}
	...
	if err != nil {
		logger.Error("Error while sending coins from user to proposer", "user", user, giltconsensusTypes.LogKeyProposer, msg.Proposer, "topupAmount", topupAmount, giltconsensusTypes.LogKeyError, err)
		return err
	}
	...
	if err != nil {
		logger.Error("Error while saving topup sequence", giltconsensusTypes.LogKeySequence, sequence, giltconsensusTypes.LogKeyError, err)
		return err
	}
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(giltconsensusTypes.AttributeKeyTxHash, common.Bytes2Hex(txBytes)),
