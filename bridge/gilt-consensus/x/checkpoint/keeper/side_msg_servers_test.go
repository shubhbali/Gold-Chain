...
import (
	...
	"github.com/giltchain/gilt-consensus/x/checkpoint/testutil"
	"github.com/giltchain/gilt-consensus/x/checkpoint/types"
	stakeSim "github.com/giltchain/gilt-consensus/x/stake/testutil"
)
...
func (s *KeeperTestSuite) TestSideHandleMsgCheckpoint() {
	...
	giltChainTxConfirmations := chainParams.GiltChainTxConfirmations
	...
	s.Run("Success", func() {
		...
		contractCaller.On("CheckIfBlocksExist", checkpoint.EndBlock+giltChainTxConfirmations).Return(true, nil)
	...
	s.Run("wrong giltChainId", func() {
		contractCaller.Mock = mock.Mock{}

		// create checkpoint msg with wrong giltChainId
		wrongGiltChainID := giltChainId + "-wrong"
		msgCheckpoint := types.NewMsgCheckpointBlock(
			checkpoint.Proposer,
			checkpoint.StartBlock,
			checkpoint.EndBlock,
		...
		result := sideHandler(ctx, msgCheckpoint)
		require.Equal(sidetxs.Vote_VOTE_NO, result, "should vote NO on giltChainId mismatch")
...
func (s *KeeperTestSuite) TestPostHandleMsgCheckpoint() {
	...
	s.Run("Success", func() {
		...
		require.Equal(bufCheckpoint.GiltChainId, checkpoint.GiltChainId)
