...
func (srv *sideMsgServer) SideHandleMsgCheckpoint(ctx sdk.Context, sdkMsg sdk.Msg) (result sidetxs.Vote) {
	...
	if !helper.ValidateChainID(msg.GiltChainId, chainParams.ChainParams.GiltChainId, logger, "checkpoint") {
		return sidetxs.Vote_VOTE_NO
	}
	...
	logger.Info("Validating checkpoint proposal",
		...
		"giltChainId", msg.GiltChainId,
	)

	// validate checkpoint
	validCheckpoint, err := types.IsValidCheckpoint(msg.StartBlock, msg.EndBlock, msg.RootHash, params.MaxCheckpointLength, contractCaller, giltChainTxConfirmations)
	if err != nil {
