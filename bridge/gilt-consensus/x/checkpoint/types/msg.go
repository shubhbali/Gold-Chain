...
import (
	...
	"github.com/giltchain/gilt-consensus/types"
)
...
func (msg MsgCheckpoint) GetSideSignBytes() []byte {
	// keccak256(abi.encoded(proposer, startBlock, endBlock, rootHash, accountRootHash, gilt chain id))
	giltChainID, _ := strconv.ParseUint(msg.GiltChainId, 10, 64)
	...
	return types.AppendBytes32(
		...
		msg.RootHash,
		msg.AccountRootHash,
		new(big.Int).SetUint64(giltChainID).Bytes(),
	)
}
...
func UnpackCheckpointSideSignBytes(data []byte) (*MsgCheckpoint, error) {
	...
	// Extract GiltChainId
	giltChainIDBytes := data[offset : offset+chunkSize]
	giltChainIDUint := new(big.Int).SetBytes(giltChainIDBytes).Uint64()
	giltChainIDStr := strconv.FormatUint(giltChainIDUint, 10)
