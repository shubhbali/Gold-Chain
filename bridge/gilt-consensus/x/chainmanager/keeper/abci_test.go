...
func (s *KeeperTestSuite) TestSetParamsAndRetrieval() {
	...
	s.Run("params can be updated", func() {
		...
		updatedParams.ChainParams.GiltChainId = "custom-chain-id-999"
		...
		require.NoError(err)
		require.Equal("custom-chain-id-999", retrieved.ChainParams.GiltChainId)
	})
	...
	s.Run("params update persists across gets", func() {
		...
		currentParams.GiltChainTxConfirmations = 999
		...
		retrieved, err := cmKeeper.GetParams(ctx)
		require.NoError(err)
		require.Equal(uint64(999), retrieved.GiltChainTxConfirmations)
	})
}
