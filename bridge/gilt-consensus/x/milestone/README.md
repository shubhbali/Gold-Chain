...
This module enables deterministic finality by leveraging Gilt PoS’s dual client architecture.  
...
After a certain number of blocks (minimum 1), a milestone is proposed and voted by GiltConsensus.  
...
Milestones are a lightweight alternative to checkpoints in GiltConsensus, used to finalize blocks more efficiently.  
With the introduction of milestones, finality is deterministic even before a checkpoint is submitted to L1.  
Unlike the original transaction-based design in GiltConsensus-v1, the current design operates without transactions, relying entirely on ABCI++ flow and validator vote extensions.  
...
message Milestone {
  ...
  string gilt_chain_id = 5 [ (amino.dont_omitempty) = true ];
...
```bash
giltconsd tx milestone milestone [proposer] [startBlock] [endBlock] [hash] [giltChainId] [milestoneId]
```
...
```bash
giltconsd query milestone get-params
```

```bash
giltconsd query milestone get-count
```

```bash
giltconsd query milestone get-latest-milestone
```

```bash
giltconsd query milestone get-milestone-by-number
```

```bash
giltconsd query milestone get-milestone-proposer
```

```bash
giltconsd query milestone get-latest-no-ack-milestone
```

```bash
giltconsd query milestone get-no-ack-milestone-by-id
```

### GRPC Endpoints

The endpoints and the params are defined in the [milestone/query.proto](/proto/giltconsensusv2/milestone/query.proto) file.
Please refer to them for more information about the optional params.

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.milestone.Query/GetMilestoneParams
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.milestone.Query/GetMilestoneCount
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.milestone.Query/GetLatestMilestone
```

```bash
grpcurl -plaintext -d '{}' localhost:9090 giltconsensusv2.milestone.Query/GetMilestoneByNumber
```
