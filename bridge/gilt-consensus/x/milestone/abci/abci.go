...
import (
	...
	"github.com/giltchain/gilt-consensus/x/milestone/keeper"
...
func GenMilestoneProposition(ctx sdk.Context, giltKeeper *giltKeeper.Keeper, milestoneKeeper *keeper.Keeper, contractCaller helper.IContractCaller, getBlockAuthor GetBlockAuthorFunc) (*types.MilestoneProposition, error) {
	...
	if milestone != nil {
		...
		latestHeader, err = contractCaller.GetGiltChainBlock(ctx, nil)
		if err != nil {
	...
	if err := giltKeeper.CanVoteProducers(ctx); err == nil {
		validIndex := 0
...
func getBlockInfo(ctx sdk.Context, contractCaller helper.IContractCaller, startBlockNum, maxBlocksInProposition uint64, latestHeader *ethTypes.Header, lastMilestoneHash []byte, lastMilestoneBlock uint64) ([]byte, [][]byte, []uint64, []common.Address, error) {
	...
	// If the cached latestHeader is stale (latestBlockNum < startBlockNum), refresh it once in case Gilt produced it in the meantime.
	// This handles the case where GiltConsensus blocks faster than Gilt.
	if latestBlockNum < startBlockNum {
		...
		latestBlockNum = latestHeader.Number.Uint64()
		// If still not available, return ErrNoNewHeadersFound since Gilt hasn't produced the block yet.
	...
	// Calculate how many blocks are actually available to fetch from the Gilt chain.
	...
	headers, tds, authors, err := contractCaller.GetGiltChainBlockInfoInBatch(ctx, int64(startBlockNum), int64(milestoneEnd))
	if err != nil {
	...
	if len(headers) > 0 && len(lastMilestoneHash) > 0 {
		...
		if startBlockNum-lastMilestoneBlock > 1 {
			header, err := contractCaller.GetGiltChainBlock(ctx, big.NewInt(int64(lastMilestoneBlock+1)))
			if err != nil {
