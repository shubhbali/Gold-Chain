...
export const initializeContracts = async () => {
  ...
  const MockCheckpointManager = await getContractInstance(
    'MockCheckpointManager',
    '0xMockCheckpointManagerAddress',
    rootProvider
  )
  const RootChainManager = await getContractInstance('RootChainManager', '0xRootChainManagerAddress', rootProvider)
  const RootChainManagerProxy = await getContractInstance(
    'RootChainManagerProxy',
    '0xRootChainManagerProxyAddress',
    rootProvider
  )
  const ExitPayloadReaderTest = await getContractInstance(
    'ExitPayloadReaderTest',
    '0xExitPayloadReaderTestAddress',
    rootProvider
  )
  ...
  const ERC20PredicateProxy = await getContractInstance(
    'ERC20PredicateProxy',
    '0xERC20PredicateProxyAddress',
    rootProvider
  )
  const MintableERC20Predicate = await getContractInstance(
    'MintableERC20Predicate',
    '0xMintableERC20PredicateAddress',
    rootProvider
  )
  const MintableERC20PredicateProxy = await getContractInstance(
    'MintableERC20PredicateProxy',
    '0xMintableERC20PredicateProxyAddress',
    rootProvider
  )
  const ERC721Predicate = await getContractInstance('ERC721Predicate', '0xERC721PredicateAddress', rootProvider)
  const ERC721PredicateProxy = await getContractInstance(
    'ERC721PredicateProxy',
    '0xERC721PredicateProxyAddress',
    rootProvider
  )
  const MintableERC721Predicate = await getContractInstance(
    'MintableERC721Predicate',
    '0xMintableERC721PredicateAddress',
    rootProvider
  )
  const MintableERC721PredicateProxy = await getContractInstance(
    'MintableERC721PredicateProxy',
    '0xMintableERC721PredicateProxyAddress',
    rootProvider
  )
  const ERC1155Predicate = await getContractInstance('ERC1155Predicate', '0xERC1155PredicateAddress', rootProvider)
  const ERC1155PredicateProxy = await getContractInstance(
    'ERC1155PredicateProxy',
    '0xERC1155PredicateProxyAddress',
    rootProvider
  )
  const MintableERC1155Predicate = await getContractInstance(
    'MintableERC1155Predicate',
    '0xMintableERC1155PredicateAddress',
    rootProvider
  )
  const MintableERC1155PredicateProxy = await getContractInstance(
    'MintableERC1155PredicateProxy',
    '0xMintableERC1155PredicateProxyAddress',
    rootProvider
  )
  const ChainExitERC1155Predicate = await getContractInstance(
    'ChainExitERC1155Predicate',
    '0xChainExitERC1155PredicateAddress',
    rootProvider
  )
  const ChainExitERC1155PredicateProxy = await getContractInstance(
    'ChainExitERC1155PredicateProxy',
    '0xChainExitERC1155PredicateProxyAddress',
    rootProvider
  )
  const EtherPredicate = await getContractInstance('EtherPredicate', '0xEtherPredicateAddress', rootProvider)
  const EtherPredicateProxy = await getContractInstance(
    'EtherPredicateProxy',
    '0xEtherPredicateProxyAddress',
    rootProvider
  )
  const DummyERC20 = await getContractInstance('DummyERC20', '0xDummyERC20Address', rootProvider)
  const DummyMintableERC20 = await getContractInstance(
    'DummyMintableERC20',
    '0xDummyMintableERC20Address',
    rootProvider
  )
  const DummyERC721 = await getContractInstance('DummyERC721', '0xDummyERC721Address', rootProvider)
  const DummyMintableERC721 = await getContractInstance(
    'DummyMintableERC721',
    '0xDummyMintableERC721Address',
    rootProvider
  )
  const DummyERC1155 = await getContractInstance('DummyERC1155', '0xDummyERC1155Address', rootProvider)
  const DummyMintableERC1155 = await getContractInstance(
    'DummyMintableERC1155',
    '0xDummyMintableERC1155Address',
    rootProvider
  )
  const TestRootTunnel = await getContractInstance('TestRootTunnel', '0xTestRootTunnelAddress', rootProvider)
  const RootPotatoMigrator = await getContractInstance(
    'RootPotatoMigrator',
    '0xRootPotatoMigratorAddress',
    rootProvider
  )
  ...
  const ChildChainManagerProxy = await getContractInstance(
    'ChildChainManagerProxy',
    '0xChildChainManagerProxyAddress',
    childProvider
  )
  const ChildERC20 = await getContractInstance('ChildERC20', '0xChildERC20Address', childProvider)
  const ChildMintableERC20 = await getContractInstance(
    'ChildMintableERC20',
    '0xChildMintableERC20Address',
    childProvider
  )
  ...
  const ChildMintableERC721 = await getContractInstance(
    'ChildMintableERC721',
    '0xChildMintableERC721Address',
    childProvider
  )
  const ChildERC1155 = await getContractInstance('ChildERC1155', '0xChildERC1155Address', childProvider)
  const ChildMintableERC1155 = await getContractInstance(
  ...
  const ChildPotatoMigrator = await getContractInstance(
    'ChildPotatoMigrator',
    '0xChildPotatoMigratorAddress',
    childProvider
  )
