...
const (
	...
	// In Gilt's PoS consensus, validators take turns producing blocks deterministically,
...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	veblopTimeout := time.Duration(w.chainConfig.Gilt.CalculatePeriod(w.chain.CurrentBlock().Number.Uint64())) * time.Second
	...
	for {
		...
		case <-veblopTimer.C:
			...
			veblopTimeout = time.Duration(w.chainConfig.Gilt.CalculatePeriod(currentBlock.Number.Uint64())) * time.Second
			if veblopTimeout < w.blockTime {
				veblopTimeout = w.blockTime
			}

			if w.chainConfig.Gilt == nil || !w.chainConfig.Gilt.IsRio(currentBlock.Number) {
				veblopTimer.Reset(veblopTimeout)
				continue
			}
...
func (w *worker) resultLoop() {
	...
	for {
		...
		case newSealedBlockEvent := <-w.resultCh:
			...
			// which emits read/update/hash/execution/gilt metrics before writeBlockAndSetHead.
			...
			if metrics.Enabled() {
				...
				workerGiltConsensusTimer.Update(task.state.GiltConsensusTime)
				...
				// Gilt consensus time, and IntermediateRoot time to isolate pure EVM execution time.
				...
				execTime := task.productionElapsed - trieRead - task.state.GiltConsensusTime - task.intermediateRootTime
				if execTime < 0 {
					execTime = 0
				}
	...
	if EnableMVHashMap && w.IsRunning() {
		...
		if deps != nil && len(env.mvReadMapList) > 0 {
			...
			for i := 1; i <= len(env.mvReadMapList)-1; i++ {
				...
				_, ok2 := reads[blockstm.NewSubpathKey(common.HexToAddress(w.chainConfig.Gilt.CalculateBurntContract(env.header.Number.Uint64())), state.BalancePath)]
				if ok1 || ok2 {
					delayFlag = false
					break
				}
...
func (w *worker) makeHeader(genParams *generateParams, waitOnPrepare bool) (*types.Header, common.Address, error) {
	...
	if w.chainConfig.Gilt != nil && w.chainConfig.Gilt.IsRio(newBlockNumber) {
		coinbase = common.HexToAddress(w.chainConfig.Gilt.CalculateCoinbase(newBlockNumber.Uint64()))
	...
	if err := w.engine.Prepare(w.chain, header, waitOnPrepare); err != nil {
		switch err.(type) {
		case *gilt.UnauthorizedSignerError:
...
func (w *worker) buildDefaultFilter(BaseFee *big.Int, Number *big.Int) txpool.PendingFilter {
	...
	isMadhugiri := w.chainConfig.Gilt != nil && w.chainConfig.Gilt.IsMadhugiri(Number)
...
func (w *worker) generateWork(params *generateParams, witness bool) *newPayloadResult {
	...
	// Gilt/gilt: EIP-6110, EIP-7002, and EIP-7251 are not supported
	// Collect consensus-layer requests if Prague is enabled and gilt consensus is not active.
	var requests [][]byte
	if w.chainConfig.IsPrague(work.header.Number) && w.chainConfig.Gilt == nil {
		// EIP-6110 deposits
...
func (w *worker) commitWork(interrupt *atomic.Int32, noempty bool, timestamp int64) {
	...
	newBlockNumber := new(big.Int).Add(parent.Number, common.Big1)
	if w.config.EnablePrefetch && w.chainConfig.Gilt != nil && w.chainConfig.Gilt.IsGiugliano(newBlockNumber) {
		go func() {
			defer func() {
				if r := recover(); r != nil {
...
func (w *worker) buildAndCommitBlock(interrupt *atomic.Int32, noempty bool, genParams *generateParams, interruptPrefetch *atomic.Bool) {
	...
	// Starts accounting time after prepareWork, since it includes the wait we have on Prepare phase of Gilt
	start := time.Now()
	...
	var isRio bool
	if w.chainConfig.Gilt != nil {
		isRio = w.chainConfig.Gilt.IsRio(work.header.Number)
	}
