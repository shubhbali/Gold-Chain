...
// nolint : paralleltest
func TestGenerateBlockAndImportGilt(t *testing.T) {
	testGenerateBlockAndImport(t, false, true)
}

//nolint:thelper
func testGenerateBlockAndImport(t *testing.T, isClique bool, isGilt bool) {
	var (
	...
	if isGilt {
		chainConfig = *params.GiltUnittestChainConfig

		engine, ctrl = getFakeGiltFromConfig(t, &chainConfig)
		defer engine.Close()
		defer ctrl.Finish()
	} else {
		if isClique {
...
// giltUnittestChainConfigWithGiugliano returns a shallow copy of GiltUnittestChainConfig
...
func giltUnittestChainConfigWithGiugliano() *params.ChainConfig {
	cfg := *params.GiltUnittestChainConfig
	giltCfg := *cfg.Gilt
	giltCfg.GiuglianoBlock = big.NewInt(0)
	cfg.Gilt = &giltCfg

	return &cfg
}

// setupGiltWorkerWithPrefetch sets up a worker with Gilt consensus engine and prefetch enabled.
// Returns worker, backend, consensus engine, and mock controller for cleanup.
// nolint:thelper
func setupGiltWorkerWithPrefetch(t *testing.T, gasPercent uint64, recommit time.Duration) (*worker, *testWorkerBackend, consensus.Engine, *gomock.Controller) {
	var (
		engine      consensus.Engine
		chainConfig = giltUnittestChainConfigWithGiugliano()
		db          = rawdb.NewMemoryDatabase()
		ctrl        *gomock.Controller
	)
...
func getFakeGiltFromConfig(t *testing.T, chainConfig *params.ChainConfig) (consensus.Engine, *gomock.Controller) {
	t.Helper()
	...
	spanner.EXPECT().GetCurrentValidatorsByHash(gomock.Any(), gomock.Any(), gomock.Any()).Return(giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(span0.ValidatorSet.Validators), nil).AnyTimes()

	giltconsensusClientMock := mocks.NewMockIGiltConsensusClient(ctrl)
	giltconsensusWSClient := mocks.NewMockIGiltConsensusWSClient(ctrl)

	giltconsensusClientMock.EXPECT().GetSpan(gomock.Any(), uint64(0)).Return(&span0, nil).AnyTimes()
	giltconsensusClientMock.EXPECT().GetLatestSpan(gomock.Any()).Return(&span0, nil).AnyTimes()
	giltconsensusClientMock.EXPECT().FetchMilestone(gomock.Any()).Return(&milestone.Milestone{}, nil).AnyTimes()
	giltconsensusClientMock.EXPECT().FetchStatus(gomock.Any()).Return(&ctypes.SyncInfo{CatchingUp: false}, nil).AnyTimes()
	giltconsensusClientMock.EXPECT().Close().AnyTimes()

	contractMock := gilt.NewMockGenesisContract(ctrl)

	db, _, _ := NewDBForFakes(t)

	engine := NewFakeGilt(t, db, chainConfig, ethAPIMock, spanner, giltconsensusClientMock, giltconsensusWSClient, contractMock)
...
// nolint:paralleltest
// TestCommitInterruptExperimentGilt_NormalFlow tests the commit interrupt experiment for gilt consensus by inducing
// an artificial delay at transaction level. It runs the normal mining flow triggered via new head.
func TestCommitInterruptExperimentGilt_NormalFlow(t *testing.T) {
	// with 1 sec block time and 200 millisec tx delay we should get up to 5 txs per block
	t.Run("200ms_delay", func(t *testing.T) {
		testCommitInterruptExperimentGilt(t, 200, 5)
	})
	...
	t.Run("100ms_delay", func(t *testing.T) {
		testCommitInterruptExperimentGilt(t, 100, 10)
	})
}

// nolint:thelper
// testCommitInterruptExperimentGilt is a helper function for testing the commit interrupt experiment for gilt consensus.
func testCommitInterruptExperimentGilt(t *testing.T, delay uint, txCount int) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		db          = rawdb.NewMemoryDatabase()
...
// TestCommitInterruptExperimentGilt_NewTxFlow tests the commit interrupt experiment for gilt consensus by inducing
...
func TestCommitInterruptExperimentGilt_NewTxFlow(t *testing.T) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		db          = rawdb.NewMemoryDatabase()
...
func BenchmarkGiltMining(b *testing.B) {
	chainConfig := params.GiltUnittestChainConfig
	...
	giltconsensusClientMock.EXPECT().Close().Times(1)

	contractMock := gilt.NewMockGenesisContract(ctrl)

	db, _, _ := NewDBForFakes(b)

	engine := NewFakeGilt(b, db, chainConfig, ethAPIMock, spanner, giltconsensusClientMock, giltconsensusWSClient, contractMock)
	defer engine.Close()
	...
	blockPeriod, ok := back.genesis.Config.Gilt.Period["0"]
	if !ok {
		blockPeriod = 1
	}
...
// params.GiltUnittestChainConfig contains the NapoliBlock as big.NewInt(5), so the first 4 blocks will not have metadata.
// nolint:gocognit
func BenchmarkGiltMiningBlockSTMMetadata(b *testing.B) {
	chainConfig := params.GiltUnittestChainConfig
...
func TestVeblopTimerTriggersStaleBlock(t *testing.T) {
	...
	chainConfig = &params.ChainConfig{}
	*chainConfig = *params.GiltUnittestChainConfig
	giltCfg := *chainConfig.Gilt
	chainConfig.Gilt = &giltCfg
	chainConfig.Gilt.RioBlock = big.NewInt(0)
...
func TestPrefetchFromPool_BasicExecution(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 100, 1*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchFromPool_GasLimitTracking(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 50, 1*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchFromPool_EarlyInterruption(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 100, 500*time.Millisecond)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchGasLimitPercent_EdgeValues(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, tc.gasPercent, 1*time.Second)
			defer engine.Close()
			defer ctrl.Finish()
			defer w.close()
...
func TestPrefetchFromPool_ActuallyProcessesTransactions(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 100, 2*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchFromPool_TransactionProcessingLoop(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 150, 3*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchFromPool_TxSelectionLogic(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 50, 3*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestPrefetchFromPool_IterativeLoops(t *testing.T) {
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 200, 5*time.Second)
	defer engine.Close()
	defer ctrl.Finish()
	defer w.close()
...
func TestRapidBlockProduction_WithoutWait(t *testing.T) {
	...
	w, b, engine, ctrl := setupGiltWorkerWithPrefetch(t, 100, 200*time.Millisecond)
	defer engine.Close()
	defer ctrl.Finish()
...
func BenchmarkBlockProductionLatency(b *testing.B) {
	b.Run("WithPrefetch", func(b *testing.B) {
		...
		engine, ctrl = getFakeGiltFromConfig(&testing.T{}, chainConfig)
		defer engine.Close()
		defer ctrl.Finish()
