...
var defaultGiltTraceEnabled = newBoolPtr(false)
...
type Backend interface {
	...
	// Gilt related APIs
	GetGiltBlockTransactionWithBlockHash(ctx context.Context, txHash common.Hash, blockHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error)
}
...
func (api *API) getAllBlockTransactions(ctx context.Context, block *types.Block) (types.Transactions, bool, common.Hash) {
	...
	isMadhugiri := api.backend.ChainConfig().Gilt != nil && api.backend.ChainConfig().Gilt.IsMadhugiri(block.Number())
	if isMadhugiri {
	...
	giltReceipt := rawdb.ReadGiltReceipt(api.backend.ChainDb(), block.Hash(), block.NumberU64(), api.backend.ChainConfig())
	if giltReceipt != nil {
		txHash := types.GetDerivedGiltTxHash(types.GiltReceiptKey(block.Number().Uint64(), block.Hash()))
		if txHash != (common.Hash{}) {
			giltTx, _, _, _, _ := api.backend.GetGiltBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
...
func deepCopyTraceConfig(config *TraceConfig) TraceConfig {
	...
	if config.GiltTx != nil {
		cpy.GiltTx = newBoolPtr(*config.GiltTx)
	}
	if config.GiltTraceEnabled != nil {
		cpy.GiltTraceEnabled = newBoolPtr(*config.GiltTraceEnabled)
	}
...
func (api *API) traceChain(start, end *types.Block, config *TraceConfig, closed <-chan error) chan *blockTraceResult {
	if config == nil {
		config = &TraceConfig{
			GiltTraceEnabled: defaultGiltTraceEnabled,
			GiltTx:           newBoolPtr(false),
		}
	}

	if config.GiltTraceEnabled == nil {
		config.GiltTraceEnabled = defaultGiltTraceEnabled
	}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range taskCh {
				...
				// Include state sync tx if canonical (post-Madhugiri) or GiltTraceEnabled (pre-Madhugiri).
				isMadhugiri := api.backend.ChainConfig().Gilt != nil && api.backend.ChainConfig().Gilt.IsMadhugiri(task.block.Number())
				includeStateSyncTx := isMadhugiri || *config.GiltTraceEnabled
				...
				for i, tx := range txs {
					...
					if stateSyncPresent && i == len(txs)-1 && includeStateSyncTx {
						txConfig.GiltTx = newBoolPtr(true)
					}
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	// Include state sync tx if canonical (post-Madhugiri) or GiltTraceEnabled (pre-Madhugiri).
	isMadhugiri := chainConfig.Gilt != nil && chainConfig.Gilt.IsMadhugiri(block.Number())
	includeStateSyncTx := isMadhugiri || *config.GiltTraceEnabled
	for i, tx := range txs {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				if stateSyncPresent && task.index == len(txs)-1 {
					isMadhugiri := api.backend.ChainConfig().Gilt != nil && api.backend.ChainConfig().Gilt.IsMadhugiri(block.Number())
					if isMadhugiri || *config.GiltTraceEnabled {
						txConfig.GiltTx = newBoolPtr(true)
					}
				}
...
txloop:
	for i, tx := range txs {
		...
		if !ioflag {
			//nolint:nestif
			if stateSyncPresent && i == len(txs)-1 {
				if *config.GiltTraceEnabled {
					callmsg := prepareCallMessage(*msg)
					// nolint : contextcheck
					if _, err := statefull.ApplyGiltMessage(evm, callmsg); err != nil {
						failed = err
						break txloop
					}
				} else {
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range txs {
		...
		if stateSyncPresent && i == len(txs)-1 && includeStateSyncTx {
			callmsg := prepareCallMessage(*msg)
			statedb.SetTxContext(stateSyncHash, i)
			_, err = statefull.ApplyGiltMessage(evm, callmsg)
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if !found {
		// For GiltTransaction, there will be no trace available
		tx, _, _, _ := rawdb.ReadGiltTransaction(api.backend.ChainDb(), hash)
		if tx != nil {
...
func (api *API) traceTx(ctx context.Context, tx *types.Transaction, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig, precompiles vm.PrecompiledContracts) (interface{}, error) {
	...
	if config.GiltTx == nil {
		config.GiltTx = newBoolPtr(false)
	}
