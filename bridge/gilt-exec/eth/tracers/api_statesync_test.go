...
// giltTestBackend extends testBackend with Gilt-specific configuration for testing
// state sync transaction handling across the Madhugiri hardfork.
type giltTestBackend struct {
	testBackend
...
func (b *giltTestBackend) BlockByNumber(ctx context.Context, number rpc.BlockNumber) (*types.Block, error) {
...
func (b *giltTestBackend) BlockByHash(ctx context.Context, hash common.Hash) (*types.Block, error) {
...
// newGiltChainConfig creates a chain config suitable for Gilt state sync testing.
func newGiltChainConfig(madhugiriBlock *big.Int) *params.ChainConfig {
...
// newGiltChainConfigForInsertion creates a chain config with Gilt settings but without Madhugiri
// for block insertion. This allows transaction execution (which needs GiltConfig for burnt contract)
// while avoiding state-sync validation during insertion.
func newGiltChainConfigForInsertion() *params.ChainConfig {
	return &params.ChainConfig{
		ChainID:             big.NewInt(137),
		HomesteadBlock:      big.NewInt(0),
		DAOForkBlock:        big.NewInt(0),
...
// newGiltTestBackend creates a test backend that:
// 1. Inserts blocks without Gilt validation (to avoid state-sync processing issues)
// 2. Exposes a Gilt chain config to the tracer API (to test Madhugiri logic)
// 3. Allows manual injection of state-sync txs into block bodies after insertion
func newGiltTestBackend(t *testing.T, n int, gspec *core.Genesis, madhugiriBlock *big.Int, generator func(i int, b *core.BlockGen)) *giltTestBackend {
	// Use config without Gilt for block insertion
	insertionConfig := newGiltChainConfigForInsertion()
	gspec.Config = insertionConfig

	backend := &giltTestBackend{
		testBackend: testBackend{
			// Use Gilt config for API queries (this is what the tracer API sees)
			chainConfig: newGiltChainConfig(madhugiriBlock),
	...
	// Generate blocks with insertion config (no Gilt)
	...
	// Create chain with insertion config (no Gilt validation)
...
func (b *giltTestBackend) injectStateSyncTx(blockNum uint64, stateSyncTx *types.Transaction) error {
...
// include state sync transactions in trace results regardless of GiltTraceEnabled.
...
func TestTraceBlockStateSyncPostMadhugiri(t *testing.T) {
	...
	backend := newGiltTestBackend(t, genBlocks, gspec, madhugiriBlock, func(i int, b *core.BlockGen) {
	...
	// Test post-Madhugiri block (block 10) with GiltTraceEnabled=false (default)
	t.Run("PostMadhugiri_GiltTraceDisabled", func(t *testing.T) {
		...
		// Trace with default config (GiltTraceEnabled=false)
	...
	// Test post-Madhugiri block with GiltTraceEnabled=true
	t.Run("PostMadhugiri_GiltTraceEnabled", func(t *testing.T) {
		block, _ := backend.BlockByNumber(context.Background(), rpc.BlockNumber(10))
		expectedTxCount := len(block.Transactions())

		giltTraceEnabled := true
		config := &TraceConfig{GiltTraceEnabled: &giltTraceEnabled}
		...
		if len(results) != expectedTxCount {
			t.Errorf("tx count mismatch with GiltTraceEnabled=true: block has %d txs, trace returned %d results",
				expectedTxCount, len(results))
		}
	})
}
...
func TestNoMadhugiriFork(t *testing.T) {
	...
	backend := newGiltTestBackend(t, genBlocks, gspec, nil, func(i int, b *core.BlockGen) {
		tx, _ := types.SignTx(types.NewTx(&types.LegacyTx{
			Nonce:    uint64(i),
			To:       &address,
			Value:    big.NewInt(1000),
