...
func (api *API) traceGiltBlock(ctx context.Context, block *types.Block, config *TraceConfig) (*BlockTraceResult, error) {
	...
	traceTxn := func(indx int, tx *types.Transaction, giltTx bool, stateSyncHash common.Hash) *TxTraceResult {
		...
		if giltTx {
			...
			execRes, err = statefull.ApplyGiltMessage(vmenv, callmsg)
		} else {
...
func (api *API) TraceGiltBlock(req *TraceBlockRequest) (*BlockTraceResult, error) {
	...
	log.Debug("Tracing Gilt Block", "block number", blockNumber)
	...
	return api.traceGiltBlock(ctx, block, req.Config)
}
