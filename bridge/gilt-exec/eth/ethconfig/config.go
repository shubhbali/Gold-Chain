...
var Defaults = Config{
	...
	TransactionHistory:    2350000, // Note: used in gilt cli
	LogHistory:            2350000, // Note: used in gilt cli
...
type Config struct {
	...
	// RPCBlockRangeLimit is the maximum block range allowed in eth_getLogs / gilt_getLogs (0 = unlimited)
	RPCBlockRangeLimit uint64

	// URL to connect to GiltConsensus node (comma-separated for failover: "url1,url2,url3")
...
func CreateConsensusEngine(chainConfig *params.ChainConfig, ethConfig *Config, db ethdb.Database, blockchainAPI *ethapi.BlockChainAPI) (consensus.Engine, error) {
	...
	} else if chainConfig.Gilt != nil && chainConfig.Gilt.ValidatorContract != "" {
		// If Gilt gilt consensus is requested, set it up
		// In order to pass the ethereum transaction tests, we need to set the burn contract which is in the gilt config
		// Then, gilt != nil will also be enabled for ethash and clique. Only enable Gilt for real if there is a validator contract present.
