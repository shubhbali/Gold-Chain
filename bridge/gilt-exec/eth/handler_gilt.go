...
var (
	...
	// latest checkpoint from the local giltconsensus.
	errCheckpoint = errors.New("failed to fetch latest checkpoint")

	// errMilestone is returned when we are unable to fetch the
	// latest milestone from the local giltconsensus.
	errMilestone = errors.New("failed to fetch latest milestone")
)

// fetchWhitelistCheckpoint fetches the latest checkpoint from it's local giltconsensus
// and verifies the data against gilt data.
func (h *ethHandler) fetchWhitelistCheckpoint(ctx context.Context, gilt *gilt.Gilt) (result *checkpoint.Checkpoint, err error) {
	defer func() {
		if err == nil && result != nil {
			log.Debug("Got new checkpoint from giltconsensus", "start", result.StartBlock, "end", result.EndBlock, "rootHash", result.RootHash.String())
		}
	}()

	// fetch the latest checkpoint from GiltConsensus
	result, err = gilt.GiltConsensusClient.FetchCheckpoint(ctx, -1)
...
func (h *ethHandler) handleWhitelistCheckpoint(ctx context.Context, checkpoint *checkpoint.Checkpoint, eth *Ethereum, verifier *giltVerifier, process bool) (common.Hash, error) {
...
// fetchWhitelistMilestone fetches the latest milestone from it's local giltconsensus
// and verifies the data against gilt data.
func (h *ethHandler) fetchWhitelistMilestone(ctx context.Context, gilt *gilt.Gilt) (result *milestone.Milestone, err error) {
	defer func() {
		if err == nil && result != nil {
			log.Debug("Got new milestone from giltconsensus", "start", result.StartBlock, "end", result.EndBlock, "hash", result.Hash.String())
		}
	}()

	// fetch latest milestone
	result, err = gilt.GiltConsensusClient.FetchMilestone(ctx)
...
func (h *ethHandler) handleMilestone(ctx context.Context, eth *Ethereum, milestone *milestone.Milestone, verifier *giltVerifier) error {
	...
	if errors.Is(err, giltconsensus.ErrServiceUnavailable) {
		return nil
	}
...
// reportCommonErrors reports common errors which can occur while fetching data from giltconsensus. It also
...
func reportCommonErrors(msg string, err error, wrapError error, ctx ...interface{}) error {
	...
	// We're skipping extra check to the `giltconsensus.ErrServiceUnavailable` error as it should not
	// occur post HF (in giltconsensus). If it does, we'll anyways warn below as a normal error.

	ctx = append(ctx, "err", err)

	if strings.Contains(err.Error(), "context deadline exceeded") {
		log.Warn(fmt.Sprintf("Failed to fetch %s, please check the giltconsensus endpoint and status of your giltconsensus node", msg), ctx...)
	} else {
