...
func ServiceGetReceiptsQuery69(chain *core.BlockChain, query GetReceiptsRequest) []rlp.RawValue {
	...
	for lookups, hash := range query {
		...
		if giltCfg != nil && giltCfg.IsMadhugiri(big.NewInt(int64(number))) {
		...
		giltReceipt := chain.GetGiltReceiptRLPByHash(hash)
		var giltReceiptDecoded types.ReceiptForStorage
		if giltReceipt != nil {
			if err := rlp.DecodeBytes(giltReceipt, &giltReceiptDecoded); err != nil {
		...
		if normalReceipts == nil && giltReceipt == nil {
		...
		// Track existence of gilt receipts for encoding
		var isGiltReceiptPresent bool
		...
		if giltReceipt != nil {
			isGiltReceiptPresent = true
			blockReceipts = append(blockReceipts, &giltReceiptDecoded)
		}

		// isStateSyncReceipt denotes whether a receipt belongs to state-sync transaction or not
		isStateSyncReceipt := func(index int) bool {
			// If gilt receipt is present, it will always be at the end of list
			if isGiltReceiptPresent && index == len(blockReceipts)-1 {
				return true
			}
			return false
		}
...
func EncodeReceiptsAndPrepareHasher(packet interface{}, giltCfg *params.GiltConfig) (ReceiptsRLPResponse, func(int, *big.Int) common.Hash) {
	...
	case []*ReceiptList68:
		receiptList := packet
		receipts, getReceiptListHashes = encodeReceiptsAndPrepareHasher(receiptList, giltCfg)
	case *[]*ReceiptList68:
		receiptList := packet
		receipts, getReceiptListHashes = encodeReceiptsAndPrepareHasher(*receiptList, giltCfg)
	case []*ReceiptList69:
...
func encodeReceiptsAndPrepareHasher[L ReceiptsList](receipts []L, giltCfg *params.GiltConfig) (ReceiptsRLPResponse, func(int, *big.Int) common.Hash) {
	...
	calculateReceiptHashes := func(index int, number *big.Int) common.Hash {
		// For non-gilt chains or post-Madhugiri blocks, include all receipts in hash
		if giltCfg == nil || giltCfg.IsMadhugiri(number) {
			return types.DeriveSha(receipts[index], hasher)
		}
		// Pre-Madhugiri gilt blocks: exclude state-sync receipt from hash calculation
