...
// peers are filtered out (due to their inability to serve gilt receipts).
...
func testEmptyShortCircuit(t *testing.T, protocol uint, mode SyncMode) {
	...
	// In eth/69, gilt receipts are also requested for sprint end blocks (because we're not aware during snap sync
	// if a block contains gilt transactions or not). Ensure those blocks are counted towards the receipt retrieval count.
	giltCfg := params.TestChainConfig.Gilt
	for _, block := range chain.blocks[1:] {
		if mode == SnapSync && (len(block.Transactions()) > 0 || types.IsSprintEndBlock(giltCfg, block.NumberU64())) {
			receiptsNeeded++
		}
	}
