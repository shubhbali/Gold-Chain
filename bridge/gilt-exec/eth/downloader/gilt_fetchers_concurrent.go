		...
		if queue.pending() == 0 {
			...
			for _, peer := range d.peers.AllPeers() {
				...
				if pending == nil && stale == nil {
					...
					// eth/69 handlers also sends gilt receipts via p2p. Skip peers
					// below that to avoid missing gilt receipts.
			...
			if isWitnessQueue {
				...
				// For receipt fetching, only count eth/69+ peers that include gilt receipts
				for _, peer := range d.peers.AllPeers() {
