...
func (r *resultStore) AddFetch(header *types.Header, syncMode SyncMode, giltCfg *params.GiltConfig) (stale, throttled bool, item *fetchResult, err error) {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if item == nil {
		item = newFetchResult(header, syncMode, giltCfg)
		r.items[index] = item
	}
