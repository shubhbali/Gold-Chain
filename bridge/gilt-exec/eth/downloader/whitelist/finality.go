...
type finality[T rawdb.BlockFinality[T]] struct {
	sync.RWMutex
	db       ethdb.Database
	Hash     common.Hash // Whitelisted Hash, populated by reaching out to giltconsensus
	Number   uint64      // Number , populated by reaching out to giltconsensus
...
// in terms of reorgs. We won't reorg beyond the last gilt finality submitted to mainchain.
...
func (f *finality[T]) reportWhitelist(block uint64, hash common.Hash) {
	msg := fmt.Sprintf("Whitelisting new %s from giltconsensus", f.name)
	if !f.doExist {
