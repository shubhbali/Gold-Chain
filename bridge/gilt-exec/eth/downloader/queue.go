...
func newFetchResult(header *types.Header, syncMode SyncMode, giltCfg *params.GiltConfig) *fetchResult {
	item := &fetchResult{
		Header: header,
	}
	if !header.EmptyBody() {
	...
	fetchReceipts := true
	if header.EmptyReceipts() && !types.IsSprintEndBlock(giltCfg, header.Number.Uint64()) {
		fetchReceipts = false
	}
...
func newQueue(blockCacheLimit int, thresholdInitialSize int, giltCfg *params.GiltConfig) *queue {
...
func (q *queue) Schedule(headers []*types.Header, hashes []common.Hash, from uint64) []*types.Header {
	...
	for i, header := range headers {
		...
		fetchReceipts := true
		if header.EmptyReceipts() && !types.IsSprintEndBlock(q.giltConfig, header.Number.Uint64()) {
			fetchReceipts = false
		}
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque[int64, *types.Header],
	...
	for proc := 0; len(send) < count && !taskQueue.Empty(); proc++ {
		...
		stale, throttle, item, err := q.resultCache.AddFetch(header, q.mode, q.giltConfig)
		if stale {
