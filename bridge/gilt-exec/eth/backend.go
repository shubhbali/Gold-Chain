...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if config.Miner.GasPrice == nil {
		log.Info("Miner gas price not set, using default", "value", params.GiltDefaultMinerGasPrice)
		config.Miner.GasPrice = big.NewInt(params.GiltDefaultMinerGasPrice)
	}
	// PIP-35: Enforce min gas price to 25 gwei only if gas tip override is not allowed
	if !config.Miner.AllowGasTipOverride && config.Miner.GasPrice.Cmp(big.NewInt(params.GiltDefaultMinerGasPrice)) != 0 {
	...
	// Wire MilestoneFetcher so verifyPendingHeaders queries GiltConsensus directly.
	if giltEngine, ok := eth.engine.(*gilt.Gilt); ok && giltEngine.GiltConsensusClient != nil {
		options.MilestoneFetcher = func(ctx context.Context) (uint64, error) {
			m, err := giltEngine.GiltConsensusClient.FetchMilestone(ctx)
			if err != nil {
				return 0, err
			}
	...
	// GILT changes
	// Blob pool is removed from Subpool for Gilt
	if eth.config.SyncMode != downloader.StatelessSync {
		...
		eth.txPool.SetGasTip(new(big.Int).SetUint64(params.GiltDefaultTxPoolPriceLimit))
	}
...
func (s *Ethereum) APIs() []rpc.API {
	...
	publicFilterAPI := filters.NewFilterAPI(filterSystem, s.config.GiltLogs)
	...
	return append(apis, []rpc.API{
		{
			...
			Namespace: "eth",
			Service:   publicFilterAPI, // GILT related change
		}, {
			Namespace: "admin",
...
func (s *Ethereum) StartMining() error {
	...
	if !s.IsMining() {
		...
		if !s.authorized {
			...
			if gilt, ok := s.engine.(*gilt.Gilt); ok {
				wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
				if wallet == nil || err != nil {
					log.Error("Etherbase account unavailable locally", "err", err)
...
func (s *Ethereum) startCheckpointWhitelistService() {
	...
	s.retryGiltConsensusHandler(s.fetchAndHandleWhitelistCheckpoint, tickerDuration, whitelistTimeout)
}
...
func (s *Ethereum) startMilestoneWhitelistService() {
	ethHandler, gilt, _ := s.getHandler()

	const (
		tickerDuration = 2 * time.Second
	)

	// If giltconsensus ws is available use WS subscription to new milestone events instead of polling
	if gilt != nil && gilt.GiltConsensusWSClient != nil {
		for {
			if err := s.subscribeAndHandleMilestone(context.Background(), ethHandler, gilt); err != nil {
	...
	s.retryGiltConsensusHandler(s.fetchAndHandleMilestone, tickerDuration, whitelistTimeout)
}

func (s *Ethereum) retryGiltConsensusHandler(fn giltconsensusHandler, tickerDuration time.Duration, timeout time.Duration) {
	retryGiltConsensusHandler(fn, tickerDuration, timeout, s.closeCh, s.getHandler)
}

func retryGiltConsensusHandler(fn giltconsensusHandler, tickerDuration time.Duration, timeout time.Duration, closeCh chan struct{}, getHandler func() (*ethHandler, *gilt.Gilt, error)) {
	...
	ethHandler, gilt, err := getHandler()
	if err != nil {
...
func (s *Ethereum) fetchAndHandleWhitelistCheckpoint(ctx context.Context, ethHandler *ethHandler, gilt *gilt.Gilt) error {
	// Create a new gilt verifier, which will be used to verify checkpoints and milestones
	verifier := newGiltVerifier()

	checkpoint, err := ethHandler.fetchWhitelistCheckpoint(ctx, gilt)
...
type giltconsensusHandler func(ctx context.Context, ethHandler *ethHandler, gilt *gilt.Gilt) error

// fetchAndHandleMilestone handles the milestone mechanism.
func (s *Ethereum) fetchAndHandleMilestone(ctx context.Context, ethHandler *ethHandler, gilt *gilt.Gilt) error {
	// Create a new gilt verifier, which will be used to verify checkpoints and milestones
	verifier := newGiltVerifier()
	milestone, err := ethHandler.fetchWhitelistMilestone(ctx, gilt)
	if err != nil {
		return err
	}
...
func (s *Ethereum) subscribeAndHandleMilestone(ctx context.Context, ethHandler *ethHandler, gilt *gilt.Gilt) error {
	milestoneEvents := gilt.GiltConsensusWSClient.SubscribeMilestoneEvents(ctx)
	...
	for {
		...
		case m, ok := <-milestoneEvents:
			...
			err := ethHandler.handleMilestone(ctx, s, m, newGiltVerifier())
			if err != nil {
		...
		case <-ticker.C:
			...
			err := ethHandler.handleMilestone(ctx, s, milestone, newGiltVerifier())
			if err != nil {
				log.Error("error handling milestone ws event", "err", err)
			}
...
func (s *Ethereum) getHandler() (*ethHandler, *gilt.Gilt, error) {
	ethHandler := (*ethHandler)(s.handler)

	gilt, ok := ethHandler.chain.Engine().(*gilt.Gilt)
	if !ok {
		return nil, nil, ErrNotGiltConsensus
	}
...
func (s *Ethereum) Stop() error {
	...
	// the giltconsensus is unresponsive and the syncing loop keeps waiting
