...
func NewFilterAPI(system *FilterSystem, giltLogs bool) *FilterAPI {
	api := &FilterAPI{
		sys:           system,
...
func (api *FilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	giltConfig := api.sys.backend.ChainConfig().Gilt
	...
	var giltLogsFilter *GiltBlockLogsFilter
	...
	if crit.BlockHash != nil {
		...
		// Block gilt filter
		if api.giltLogs {
			giltLogsFilter = NewGiltBlockLogsFilter(api.sys.backend, giltConfig, *crit.BlockHash, crit.Addresses, crit.Topics)
		}
	} else {
		...
		// Block gilt filter
		if api.giltLogs {
			giltLogsFilter = NewGiltBlockLogsRangeFilter(api.sys.backend, giltConfig, begin, end, crit.Addresses, crit.Topics)
		}
	}
	...
	if giltLogsFilter != nil {
		// Run the filter and return all the logs
		giltBlockLogs, err := giltLogsFilter.Logs(ctx)
		if err != nil {
			return nil, err
		}

		return returnLogs(types.MergeGiltLogs(logs, giltBlockLogs)), err
	}

	// merge gilt block logs and receipt logs and return it
...
func (api *FilterAPI) GetFilterLogs(ctx context.Context, id rpc.ID) ([]*types.Log, error) {
	...
	if f.crit.BlockHash != nil {
		...
		// Block gilt filter
		if api.giltLogs {
			giltLogsFilter = NewGiltBlockLogsFilter(api.sys.backend, giltConfig, *f.crit.BlockHash, f.crit.Addresses, f.crit.Topics)
		}
	} else {
		// Convert the RPC block numbers into internal representations
		begin := rpc.LatestBlockNumber.Int64()
		...
		if api.giltLogs {
			giltLogsFilter = NewGiltBlockLogsRangeFilter(api.sys.backend, giltConfig, begin, end, f.crit.Addresses, f.crit.Topics)
		}
	}
	// Run the filter and return all the logs
	logs, err := filter.Logs(ctx)
	...
	if giltLogsFilter != nil {
		...
		return returnLogs(types.MergeGiltLogs(logs, giltBlockLogs)), nil
	}
