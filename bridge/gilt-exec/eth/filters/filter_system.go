...
type Config struct {
	...
	RangeLimit    uint64        // maximum block range for eth_getLogs / gilt_getLogs (0 = unlimited)
}
...
type Backend interface {
	...
	GetGiltBlockReceipt(ctx context.Context, blockHash common.Hash) (*types.Receipt, error)
	GetGiltBlockLogs(ctx context.Context, blockHash common.Hash) ([]*types.Log, error)
...
type subscription struct {
	...
	stateSyncData chan *types.StateSyncData // channel for state sync data
}
...
type EventSystem struct {
	...
	// Gilt related subscription and channels
