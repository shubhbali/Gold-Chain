...
func (b *testBackend) GetGiltBlockReceipt(_ context.Context, blockHash common.Hash) (*types.Receipt, error) {
	number, ok := rawdb.ReadHeaderNumber(b.db, blockHash)
	if !ok {
		return &types.Receipt{}, nil
	}

	receipt := rawdb.ReadGiltReceipt(b.db, blockHash, number, nil)
	if receipt == nil {
		return &types.Receipt{}, nil
	}
...
func (b *testBackend) GetGiltBlockLogs(ctx context.Context, blockHash common.Hash) ([]*types.Log, error) {
	receipt, err := b.GetGiltBlockReceipt(ctx, blockHash)
	if err != nil {
...
func TestInvalidGetLogsRequest(t *testing.T) {
	...
	api.SetChainConfig(params.GiltUnittestChainConfig)
...
func TestInvalidGetRangeLogsRequest(t *testing.T) {
	...
	api.SetChainConfig(params.GiltTestChainConfig)
...
// TestRangeLimit verifies that RangeLimit is enforced for both GetLogs and GetGiltBlockLogs.
func TestRangeLimit(t *testing.T) {
	...
	for i, crit := range overLimitCases {
		...
		_, err = api.GetGiltBlockLogs(t.Context(), crit)
		if !errors.Is(err, errExceedBlockRangeLimit) {
			t.Errorf("GetGiltBlockLogs over-limit case %d: got %v, want errExceedBlockRangeLimit", i, err)
		}
	}
	...
	if _, err := api.GetGiltBlockLogs(t.Context(), latestCrit); !errors.Is(err, errExceedBlockRangeLimit) {
		t.Errorf("GetGiltBlockLogs with fromBlock=0 toBlock=latest on 200-block chain: got %v, want errExceedBlockRangeLimit", err)
	}
	...
	if _, err := api.GetGiltBlockLogs(t.Context(), atLimitCrit); errors.Is(err, errExceedBlockRangeLimit) {
		t.Error("GetGiltBlockLogs at exact limit should not return errExceedBlockRangeLimit")
	}
	...
	api2.SetChainConfig(params.GiltUnittestChainConfig)
	...
	if _, err := api2.GetGiltBlockLogs(t.Context(), bigRangeCrit); errors.Is(err, errExceedBlockRangeLimit) {
		t.Error("GetGiltBlockLogs with RangeLimit=0 should not return errExceedBlockRangeLimit")
	}
