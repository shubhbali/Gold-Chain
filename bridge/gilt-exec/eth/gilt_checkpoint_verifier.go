...
func newGiltVerifier() *giltVerifier {
	return &giltVerifier{giltVerify}
}

func giltVerify(ctx context.Context, eth *Ethereum, handler *ethHandler, start uint64, end uint64, hash string, isCheckpoint bool) (string, error) {
	str := "milestone"
	if isCheckpoint {
		str = "checkpoint"
	}
