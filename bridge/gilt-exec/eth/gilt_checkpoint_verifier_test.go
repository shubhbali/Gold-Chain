...
func TestGiltVerifyRewindLogic_Integration(t *testing.T) {
	...
	rewindTo := getGiltVerifyRewindBlock_Simulation(hasExistingMilestone, existingMilestoneBlock, start, end)
	...
	rewindTo = getGiltVerifyRewindBlock_Simulation(hasExistingMilestone, 0, start, end)
...
func getGiltVerifyRewindBlock_Simulation(hasExistingMilestone bool, existingBlock uint64, start uint64, end uint64) uint64 {
	// Simulate the logic from giltVerify function
...
func TestCheckpointMismatch_DoesNotRewind(t *testing.T) {
	...
	genesis := loadGiltTestGenesis(t)
	privKey := generateTestKey(t)

	stack, ethBackend := startGiltNode(t, genesis, privKey)
	defer stack.Close()
...
func loadGiltTestGenesis(t *testing.T) *core.Genesis {
	t.Helper()

	path := "../tests/gilt/testdata/genesis_2val.json"
...
func startGiltNode(t *testing.T, genesis *core.Genesis, privKey *ecdsa.PrivateKey) (*node.Node, *Ethereum) {
	t.Helper()
