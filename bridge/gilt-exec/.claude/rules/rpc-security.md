...
3. **Separate public and private APIs** — `debug_*`, `admin_*`, `personal_*` must never be exposed on public HTTP/WS. The `gilt_*` namespace (`gilt_getSnapshot`, `gilt_getSigners`, `gilt_getAuthor`) exposes validator-sensitive data and should be reviewed for public exposure. Verify API namespace configuration.

4. **JWT authentication on Engine API** — the consensus client (GiltConsensus) connection MUST require valid JWT. Verify in `node/jwt_handler.go`.
