...
paths:
  - "accounts/abi/**/*.go"
  - "accounts/abi/*.go"
  - "consensus/gilt/contract/**/*.go"
  - "consensus/gilt/statefull/**/*.go"
  - "consensus/gilt/abi/**/*.go"
  - "consensus/gilt/giltconsensus/span/**/*.go"
---
# Contract Interaction Security — accounts/abi/, consensus/gilt/contract/, consensus/gilt/statefull/

Gilt uses system contract calls for validator set management (`commitSpan`, `getGiltValidators`), state sync (`commitState`, `lastStateId`), and EIP-4788/7002 system calls. ABI encoding/decoding is the serialization boundary between Go and EVM. Bugs here can corrupt the validator set, inject arbitrary L1→L2 state, or silently drop system transactions.
...
3. **System call return values must be validated** — `commitSpan` returns no value (check `err` only), but `getGiltValidators` and `lastStateId` return critical data. An empty return (`len(ret) == 0`) must be treated as an error, not decoded.

4. **State must be finalised after system calls** — `statefull.ApplyMessage` calls `Finalise(true)` on error to clean up partial state. On success, the caller (`Gilt.Finalize`/`FinalizeAndAssemble`) is responsible for finalising after all system calls complete. Missing `Finalise` causes state changes to be lost. Note: `ApplyGiltMessage` (used for `commitSpan`) does NOT call `Finalise` on success — the caller must handle it.
...
```
Gilt.Finalize() / Gilt.FinalizeAndAssemble()
