...
func TestGiltWitnessAPI_Integration(t *testing.T) {
	t.Parallel()
	...
	giltApi := NewGiltAPI(backend)

	t.Run("GiltAPI_GetWitnessByNumber_WithStoredWitness", func(t *testing.T) {
		result, err := giltApi.GetWitnessByNumber(t.Context(), rpc.BlockNumber(1))
		require.NoError(t, err)
	...
	t.Run("GiltAPI_GetWitnessByHash_WithStoredWitness", func(t *testing.T) {
		result, err := giltApi.GetWitnessByHash(t.Context(), testBlockHash)
		require.NoError(t, err)
		require.NotNil(t, result, "Should find the stored witness")
		contextHeader := result["context"].(map[string]interface{})
		require.Equal(t, testBlockHash, contextHeader["hash"], "Witness should be for the correct block")
	...
	t.Run("GiltAPI_GetWitnessByBlockNumberOrHash_WithStoredWitness", func(t *testing.T) {
		result, err := giltApi.GetWitnessByBlockNumberOrHash(t.Context(), rpc.BlockNumberOrHashWithNumber(rpc.BlockNumber(1)))
		require.NoError(t, err)
		require.NotNil(t, result, "Should find the stored witness by number")

		result, err = giltApi.GetWitnessByBlockNumberOrHash(t.Context(), rpc.BlockNumberOrHashWithHash(testBlockHash, false))
		require.NoError(t, err)
	...
	t.Run("GiltAPI_GetWitnessByNumber_NoWitness", func(t *testing.T) {
		result, err := giltApi.GetWitnessByNumber(t.Context(), rpc.BlockNumber(2))
		require.NoError(t, err)
		require.Nil(t, result, "Should return nil for block without witness")
	})

	t.Run("GiltAPI_GetWitnessByHash_NonExistentBlock", func(t *testing.T) {
		fakeHash := common.HexToHash("0xdeadbeef")
		result, err := giltApi.GetWitnessByHash(t.Context(), fakeHash)
		require.NoError(t, err)
...
func TestGiltBlockNumber(t *testing.T) {
	t.Parallel()
...
func TestGiltGetHeaderByNumber(t *testing.T) {
	t.Parallel()
...
func TestGiltGetHeaderByHash(t *testing.T) {
	t.Parallel()
...
func TestGiltGetBlockReceiptsByBlockHash(t *testing.T) {
	t.Parallel()
	...
	// Test 8: Gilt state-sync receipt gets appended
	t.Run("gilt_state_sync_receipt_appended", func(t *testing.T) {
		block := backend.chain.GetBlockByNumber(1)
		if block == nil {
			t.Fatal("Could not get block 1")
		}
		...
		// Derive Gilt transaction hash
		giltTxHash := types.GetDerivedGiltTxHash(types.GiltReceiptKey(block.NumberU64(), block.Hash()))

		// Create a mock Gilt state-sync receipt
		giltReceipt := &types.Receipt{
		...
		// Write Gilt tx lookup entry using the block hash
		rawdb.WriteGiltTxLookupEntry(backend.chain.DB(), block.Hash(), block.NumberU64())

		// Create backend with Gilt receipt
		backendWithGilt := &testBackendWithGiltReceipt{
			testBackend: backend,
			giltReceipt:  giltReceipt,
		}
		apiWithGilt := NewGiltAPI(backendWithGilt)

		// Get receipts should append the state-sync receipt
		result, err := apiWithGilt.GetBlockReceiptsByBlockHash(context.Background(), block.Hash())
		if err != nil {
			t.Fatalf("GetBlockReceiptsByBlockHash() error = %v", err)
		}

		// Should have normal receipts plus the gilt state-sync receipt
		expectedCount := normalCount + 1
		if len(result) != expectedCount {
			t.Errorf("GetBlockReceiptsByBlockHash() returned %d receipts, want %d (txs + gilt)", len(result), expectedCount)
		}

		if len(result) <= normalCount {
			t.Error("Gilt state-sync receipt was NOT appended")
		}

		// The last receipt should be the gilt receipt
		if len(result) > normalCount {
			lastReceipt := result[len(result)-1]
			if txHash, ok := lastReceipt["transactionHash"].(common.Hash); ok {
				if txHash != giltTxHash {
					t.Errorf("Last receipt transactionHash = %s, want Gilt receipt %s", txHash, giltTxHash)
				}
			} else {
...
// testBackendWithGiltReceipt wraps testBackend and overrides GetGiltBlockReceipt
type testBackendWithGiltReceipt struct {
	*testBackend
	giltReceipt *types.Receipt
}

func (b *testBackendWithGiltReceipt) GetGiltBlockReceipt(_ context.Context, _ common.Hash) (*types.Receipt, error) {
	// Return mock Gilt receipt
	return b.giltReceipt, nil
}

func (b *testBackendWithGiltReceipt) ChainConfig() *params.ChainConfig {
	// Return config without Gilt to simulate pre-Madhugiri
	return params.TestChainConfig
}

func TestGiltForks(t *testing.T) {
	t.Parallel()
	...
	// Test 4: Gilt-specific forks are included
	t.Run("gilt_forks_included", func(t *testing.T) {
		var (
			accs = newAccounts(1)
			// Use a minimal config with just Gilt forks to test their inclusion
			config = &params.ChainConfig{
				ChainID:             big.NewInt(137), // Gilt mainnet
				HomesteadBlock:      big.NewInt(0),
		...
		// Verify Gilt forks are present
		expectedGiltForks := []uint64{50, 60, 70, 80}
		for _, expected := range expectedGiltForks {
			found := false
			for _, fork := range result.HeightForks {
				if fork == expected {
					found = true
			...
			if !found {
				t.Errorf("Expected Gilt fork at block %d not found in HeightForks", expected)
			}
		}
	})
...
func TestGiltGetLogsByHash(t *testing.T) {
	t.Parallel()
...
// testBackendWithPreMadhuguriGiltReceipt wraps testBackend and simulates pre-Madhugiri with Gilt receipts
type testBackendWithPreMadhuguriGiltReceipt struct {
	*testBackend
	giltReceipt *types.Receipt
}

func (b *testBackendWithPreMadhuguriGiltReceipt) GetGiltBlockReceipt(_ context.Context, _ common.Hash) (*types.Receipt, error) {
	return b.giltReceipt, nil
}

func (b *testBackendWithPreMadhuguriGiltReceipt) ChainConfig() *params.ChainConfig {
	// Return config with Gilt but MadhugiriBlock unset (nil) for pre-Madhuguri behavior
	cfg := *params.AllEthashProtocolChanges
	if cfg.Gilt == nil {
		cfg.Gilt = &params.GiltConfig{}
	}
	// Ensure MadhugiriBlock is nil so IsMadhugiri returns false
	cfg.Gilt.MadhugiriBlock = nil
	return &cfg
}

func TestGiltGetLogsByHashWithLogs(t *testing.T) {
	t.Parallel()
	...
	var (
		...
		config  = *params.AllEthashProtocolChanges // Use config without Gilt to avoid system transactions
	)
	...
	t.Run("transaction_with_actual_logs", func(t *testing.T) {
		...
		// Should have logs (may include Gilt system logs and contract logs)
		if len(result) == 0 {
	...
	t.Run("contract_deployment_logs", func(t *testing.T) {
		...
		// Should have at least one entry (deployment tx, possibly with Gilt system logs)
		if len(result) == 0 {
...
func TestGiltGetLogsByHashPreMadhugiri(t *testing.T) {
	t.Parallel()
	...
	t.Run("pre_madhugiri_empty_state_sync_logs", func(t *testing.T) {
		...
		backendWithGilt := &testBackendWithPreMadhuguriGiltReceipt{
			testBackend: backend,
			giltReceipt:  giltReceipt,
		}
		api := NewGiltAPI(backendWithGilt)
	...
	t.Run("pre_madhugiri_no_state_sync_receipt", func(t *testing.T) {
		backendWithGilt := &testBackendWithPreMadhuguriGiltReceipt{
			testBackend: backend,
			giltReceipt:  nil, // No state-sync receipt
		}
		api := NewGiltAPI(backendWithGilt)
...
func TestGiltGetBlockByTimestamp(t *testing.T) {
	t.Parallel()
	...
	t.Run("fullTx_parameter", func(t *testing.T) {
		...
		apiWithTx := NewGiltAPI(backendWithTx)
	...
	t.Run("repeated_timestamp_returns_earliest", func(t *testing.T) {
		...
		apiWithRepeated := NewGiltAPI(backendWithRepeated)
...
func TestGiltGetBalanceChangesInBlock(t *testing.T) {
	t.Parallel()
	...
	t.Run("contract_creation", func(t *testing.T) {
		...
		apiWithContract := NewGiltAPI(backendWithContract)
	...
	t.Run("require_canonical_with_non_canonical_hash", func(t *testing.T) {
		...
		wrappedAPI := NewGiltAPI(wrappedBackend)
...
func TestGiltGetLatestLogs(t *testing.T) {
	t.Parallel()
...
func TestGiltGetLogs(t *testing.T) {
	t.Parallel()
...
func TestGiltGetLatestLogs_TopicMatching(t *testing.T) {
	t.Parallel()
...
func TestGiltGetLogs_ErrorPropagation(t *testing.T) {
	t.Parallel()
...
func TestGiltGetLatestLogs_ErrorPropagation(t *testing.T) {
	t.Parallel()
...
// TestGetLogsBlockRangeLimit verifies that gilt_getLogs enforces GetLogsMaxBlockRange.
...
// TestGiltBlockNumberPendingTag verifies the pending tag returns the latest executed block.
func TestGiltBlockNumberPendingTag(t *testing.T) {
	t.Parallel()
...
// TestGiltBlockNumberNegativeUnknownTag verifies negative unknown tags fall through to the latest.
func TestGiltBlockNumberNegativeUnknownTag(t *testing.T) {
	t.Parallel()
...
// testBackendWithNilGiltTx simulates ReadGiltTransaction returning nil for the state-sync tx.
type testBackendWithNilGiltTx struct {
	*testBackend
}

func (b *testBackendWithNilGiltTx) GetGiltBlockReceipt(_ context.Context, _ common.Hash) (*types.Receipt, error) {
	// Return a receipt whose TxHash won't resolve to a gilt transaction in DB
...
func (b *testBackendWithNilGiltTx) ChainConfig() *params.ChainConfig {
	// Pre-Madhugiri so GetBlockReceiptsByBlockHash enters the state-sync path.
	// Deep-copy GiltConfig to avoid mutating the shared global.
	cfg := *params.AllEthashProtocolChanges
	giltCfg := params.GiltConfig{}
	if cfg.Gilt != nil {
		giltCfg = *cfg.Gilt
	}
	giltCfg.MadhugiriBlock = nil
	cfg.Gilt = &giltCfg
	return &cfg
}

// TestGetBlockReceiptsByBlockHashNilGiltTx verifies that a nil gilt transaction
// does not crash with a nil-pointer dereference.
func TestGetBlockReceiptsByBlockHashNilGiltTx(t *testing.T) {
	t.Parallel()
	...
	wrapped := &testBackendWithNilGiltTx{testBackend: base}
	api := NewGiltAPI(wrapped)

	block := base.chain.GetBlockByNumber(1)
	require.NotNil(t, block)

	// This should not panic even though ReadGiltTransaction returns nil
	result, err := api.GetBlockReceiptsByBlockHash(context.Background(), block.Hash())
	require.NoError(t, err)
	require.NotNil(t, result)

	// Should have only the normal tx receipts (1 per block), no gilt receipt appended
...
// newGasParamsTestAPI creates a GiltAPI backed by a testBackendWithGiuglianoExtra.
// headers are registered by both number and hash for lookup.
func newGasParamsTestAPI(t *testing.T, cfg *params.ChainConfig, headers ...*types.Header) *GiltAPI {
	t.Helper()
	genesis := &core.Genesis{
		Config: params.TestChainConfig,
		Alloc: types.GenesisAlloc{
	...
	return NewGiltAPI(backend)
}

// newPostGiuglianoGasParamsAPI creates a GiltAPI with a post-Giugliano header at block 10
// containing the given gas target and base fee change denominator.
func newPostGiuglianoGasParamsAPI(t *testing.T, gasTarget, bfcd uint64) (*GiltAPI, *types.Header) {
	t.Helper()
	header := &types.Header{
	...
	cfg := &params.ChainConfig{
		...
		Gilt:         &params.GiltConfig{GiuglianoBlock: big.NewInt(0)},
	}
...
func TestGetBlockGasParams_PreGiugliano(t *testing.T) {
	...
	cfg := &params.ChainConfig{
		ChainID:     big.NewInt(1),
		CancunBlock: big.NewInt(0),
		Gilt:         &params.GiltConfig{GiuglianoBlock: big.NewInt(100)},
	}
...
// makeBlockWithGiltExtra creates a block with RLP-encoded BlockExtraData in the header Extra field.
func makeBlockWithGiltExtra(number int64, bed *types.BlockExtraData) *types.Block {
	bedBytes, _ := rlp.EncodeToBytes(bed)
	extra := make([]byte, types.ExtraVanityLength)
	extra = append(extra, bedBytes...)
	extra = append(extra, make([]byte, types.ExtraSealLength)...)
...
func TestGetBlockByNumber_GiltExtraFlag_PostCancun(t *testing.T) {
	t.Parallel()
	gasTarget := uint64(15_000_000)
	bfcd := uint64(64)
	txDep := [][]uint64{{0}, {0, 1}, {}}

	block := makeBlockWithGiltExtra(10, &types.BlockExtraData{
		GasTarget:                &gasTarget,
		BaseFeeChangeDenominator: &bfcd,
		TxDependency:             txDep,
	})
...
func TestGetBlockByHash_GiltExtraFlag(t *testing.T) {
	t.Parallel()
	gasTarget := uint64(15_000_000)
	bfcd := uint64(64)
...
func TestGetBlockByNumber_GiltExtraFlag_Nil(t *testing.T) {
	t.Parallel()
	...
	require.False(t, ok, "decodedExtra should not be present when giltExtra is nil")
}

func TestGetBlockByNumber_GiltExtraFlag_False(t *testing.T) {
	t.Parallel()
	gasTarget := uint64(15_000_000)
	bfcd := uint64(64)
	...
	require.NotNil(t, result)
	_, ok := result["decodedExtra"]
	require.False(t, ok, "decodedExtra should not be present when giltExtra is false")
}

func TestGetBlockByNumber_GiltExtraFlag_PreCancun(t *testing.T) {
	t.Parallel()
