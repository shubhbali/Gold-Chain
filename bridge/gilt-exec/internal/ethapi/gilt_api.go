...
const (
	...
	// GetLogsMaxBlockRange is the maximum block range for gilt_getLogs
	GetLogsMaxBlockRange = 1000
)

// isGiltSystemTx checks if the tx is for gilt genesis contract addresses or not
func isGiltSystemTx(giltCfg *params.GiltConfig, to *common.Address) bool {
	if giltCfg == nil {
		return false
	}
	if to == nil {
	...
	validatorContract := common.HexToAddress(giltCfg.ValidatorContract)
	stateReceiverContract := common.HexToAddress(giltCfg.StateReceiverContract)
...
func (api *BlockChainAPI) GetGiltBlockReceipt(ctx context.Context, hash common.Hash) (*types.Receipt, error) {
	return api.b.GetGiltBlockReceipt(ctx, hash)
}
...
//
// Gilt transaction utils
//

func (api *BlockChainAPI) appendRPCMarshalGiltTransaction(ctx context.Context, block *types.Block, fields map[string]interface{}, fullTx bool) map[string]interface{} {
	if block != nil {
		txHash := types.GetDerivedGiltTxHash(types.GiltReceiptKey(block.Number().Uint64(), block.Hash()))

		giltTx, blockHash, blockNumber, txIndex, _ := api.b.GetGiltBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
		if giltTx != nil {
			formattedTxs := fields["transactions"].([]interface{})

			if fullTx {
				marshalledTx := newRPCTransaction(giltTx, blockHash, blockNumber, block.Time(), txIndex, block.BaseFee(), api.b.ChainConfig())
				// newRPCTransaction calculates hash based on RLP of the transaction data.
				// In the case of gilt block tx, we need simple derived tx hash (same as function argument) instead of RLP hash
...
// GiltAPI provides an API to access Gilt related information.
type GiltAPI struct {
	b Backend
}

// NewGiltAPI creates a new Gilt protocol API.
func NewGiltAPI(b Backend) *GiltAPI {
	return &GiltAPI{b}
}
...
func (api *GiltAPI) SendRawTransactionConditional(ctx context.Context, input hexutil.Bytes, options types.OptionsPIP15) (common.Hash, error) {
...
func (api *GiltAPI) GetVoteOnHash(ctx context.Context, starBlockNr uint64, endBlockNr uint64, hash string, milestoneId string) (bool, error) {
	return api.b.GetVoteOnHash(ctx, starBlockNr, endBlockNr, hash, milestoneId)
}

// GetWitnessByNumber returns the witness for the given block number.
func (api *GiltAPI) GetWitnessByNumber(ctx context.Context, number rpc.BlockNumber) (map[string]interface{}, error) {
...
func (api *GiltAPI) GetWitnessByHash(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
...
func (api *GiltAPI) GetWitnessByBlockNumberOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (map[string]interface{}, error) {
...
func (api *GiltAPI) GetBlockReceiptsByBlockHash(ctx context.Context, blockHash common.Hash) ([]map[string]interface{}, error) {
	...
	if chainConfig.Gilt != nil && chainConfig.Gilt.IsMadhugiri(block.Number()) {
		return result, nil
	}

	// Pre-Madhugiri: fetch state-sync receipt separately
	stateSyncReceipt, err := api.b.GetGiltBlockReceipt(ctx, blockHash)
...
func (api *GiltAPI) GetHeaderByHash(ctx context.Context, hash common.Hash) (*types.Header, error) {
...
//
// Returns the block header or error if not found
func (api *GiltAPI) GetHeaderByNumber(ctx context.Context, blockNumber rpc.BlockNumber) (*types.Header, error) {
...
// appendGiltExtraData adds the parsed block extra data to the response map if giltExtra is true.
func appendGiltExtraData(response map[string]interface{}, block *types.Block, giltExtra *bool, chainConfig *params.ChainConfig) {
	if giltExtra == nil || !*giltExtra || response == nil {
		return
	}
...
func (api *GiltAPI) GetBlockGasParams(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*BlockGasParamsResult, error) {
...
func (api *GiltAPI) BlockNumber(ctx context.Context, blockNrPtr *rpc.BlockNumber) (hexutil.Uint64, error) {
	...
	case rpc.FinalizedBlockNumber:
		// Get the finalized block using GiltConsensus's logic (milestone/checkpoint)
...
// Forks implements gilt_forks. Returns the genesis block hash and a sorted list of all forks block numbers.
//
// Returns:
//   - GenesisHash: The hash of the genesis block
//   - HeightForks: Sorted list of all block number forks
//   - TimeForks: Sorted list of all timestamp forks
func (api *GiltAPI) Forks(ctx context.Context) (Forks, error) {
	// Get genesis block
...
func (api *GiltAPI) GetBlockByTimestamp(ctx context.Context, timestamp rpc.Timestamp, fullTx bool) (map[string]interface{}, error) {
...
func (api *GiltAPI) GetBalanceChangesInBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (map[common.Address]*hexutil.Big, error) {
...
func (api *GiltAPI) getBalanceChangesForPending(ctx context.Context) (map[common.Address]*hexutil.Big, error) {
...
func (api *GiltAPI) GetLogsByHash(ctx context.Context, hash common.Hash) ([][]*types.Log, error) {
	// Get the block by hash
	block, err := api.b.BlockByHash(ctx, hash)
	if err != nil {
		return nil, err
	...
	// Handle Gilt state-sync logs (pre-Madhugiri)
	...
	stateSyncReceipt, err := api.b.GetGiltBlockReceipt(ctx, hash)
	if err != nil && !errors.Is(err, ethereum.NotFound) {
		return nil, fmt.Errorf("getReceipts error: %w", err)
	}
	if stateSyncReceipt != nil {
...
func (api *GiltAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
...
func (api *GiltAPI) GetLatestLogs(ctx context.Context, crit FilterCriteria, logOptions LogFilterOptions) ([]*types.Log, error) {
...
func (api *GiltAPI) determineBlockRange(ctx context.Context, crit ethereum.FilterQuery) (begin, end uint64, err error) {
...
func (api *GiltAPI) getBlockAndReceipts(ctx context.Context, blockNum uint64) (*types.Block, types.Receipts, error) {
