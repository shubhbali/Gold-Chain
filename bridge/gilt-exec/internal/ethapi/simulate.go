...
// SetStateSync implements core.GiltStateSyncer for Gilt consensus compatibility.
...
// SubscribeStateSyncEvent implements core.GiltStateSyncer for Gilt consensus compatibility.
...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, map[common.Hash]common.Address, error) {
	...
	if sim.chainConfig.IsPrague(header.Number) && sim.chainConfig.Gilt == nil {
		requests = [][]byte{}
		// EIP-6110
	...
	// For Gilt chains, Withdrawals will be nil, so we need to handle that
...
func (sim *simulator) sanitizeChain(blocks []simBlock) ([]simBlock, error) {
	...
	for _, block := range blocks {
		...
		// Only set withdrawals for non-Gilt chains (Ethereum mainnet and testnets)
		// Gilt/Gilt doesn't support withdrawals even post-Shanghai
		if block.BlockOverrides.Withdrawals == nil && sim.chainConfig.Gilt == nil {
		...
		if diff.Cmp(big.NewInt(1)) > 0 {
			...
			for i := uint64(0); i < gap.Uint64(); i++ {
				...
				// Only set withdrawals for non-Gilt chains
				if sim.chainConfig.Gilt == nil {
...
func (sim *simulator) makeHeaders(blocks []simBlock) ([]*types.Header, error) {
	...
	for bi, block := range blocks {
		...
		// Only set withdrawals hash for non-Gilt chains that have Shanghai fork
		if sim.chainConfig.IsShanghai(number) && sim.chainConfig.Gilt == nil {
