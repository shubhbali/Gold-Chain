...
func (b testBackend) GetGiltBlockTransaction(ctx context.Context, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	tx, blockHash, blockNumber, index := rawdb.ReadGiltTransaction(b.ChainDb(), hash)
	return tx, blockHash, blockNumber, index, nil
}

func (b testBackend) GetGiltBlockTransactionWithBlockHash(ctx context.Context, txHash common.Hash, blockHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	tx, blockHash, blockNumber, index := rawdb.ReadGiltTransactionWithBlockHash(b.ChainDb(), txHash, blockHash)
	return tx, blockHash, blockNumber, index, nil
}
...
func (b testBackend) GetGiltBlockLogs(ctx context.Context, hash common.Hash) ([]*types.Log, error) {
	receipt, err := b.GetGiltBlockReceipt(ctx, hash)
...
func (b testBackend) GetGiltBlockReceipt(ctx context.Context, hash common.Hash) (*types.Receipt, error) {
	receipt := b.chain.GetGiltReceiptByHash(hash)
	if receipt == nil {
...
func TestSimulateV1Disabled(t *testing.T) {
	...
	require.EqualError(t, err, "eth_simulateV1 is not supported on Gilt")
}
...
func TestSimulateV1ChainLinkage(t *testing.T) {
	t.Skip("gilt: skipping because we use total difficulty and beacon expects 0")
	var (
		acc          = newTestAccount()
		sender       = acc.addr
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	// that is commented out in gilt, we use the old genesis hash in the test files.
...
func setupTransactionsToApiTest(t *testing.T) (*TransactionAPI, []common.Hash, []struct {
	...
	backend.ChainConfig().Gilt.Sprint["0"] = uint64(genBlocks)
...
func mockStateSyncTxOnCurrentBlock(t *testing.T, backend *testBackend) common.Hash {
	...
	types.DeriveFieldsForGiltLogs(stateSyncLogs, block.Hash(), block.NumberU64(), 0, 0)

	// Write gilt receipt
	rawdb.WriteGiltReceipt(backend.ChainDb(), block.Hash(), block.NumberU64(), &types.ReceiptForStorage{
		Status: types.ReceiptStatusSuccessful, // make receipt status successful
		Logs:   stateSyncLogs,
	})

	// Write gilt tx reverse lookup
	rawdb.WriteGiltTxLookupEntry(backend.ChainDb(), block.Hash(), block.NumberU64())
	return types.GetDerivedGiltTxHash(types.GiltReceiptKey(block.NumberU64(), block.Hash()))
}
...
func setupBlocksToApiTest(t *testing.T) (*BlockChainAPI, rpc.BlockNumberOrHash, []struct {
	...
	backend.ChainConfig().Gilt.Sprint["0"] = 1
...
func TestGetWork(t *testing.T) {
	...
	// ethash.NewFaker() is a PoW engine, but Gilt's mining backend doesn't implement GetWork
...
func TestSubmitWork(t *testing.T) {
	...
	// Test SubmitWork with PoW consensus (gilt doesn't implement mining work)
...
func TestSubmitHashrate(t *testing.T) {
	...
	// Test SubmitHashrate with PoW consensus (gilt doesn't implement mining work)
