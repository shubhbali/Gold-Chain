...
func (c *PruneStateCommand) MarkDown() string {
	items := []string{
		`# Prune state`,
		`The ` + "```" + "gilt snapshot prune-state" + "```" + ` command will prune historical state data
...
func (c *PruneStateCommand) Help() string {
	return `Usage: gilt snapshot prune-state <datadir>
...
func (c *PruneStateCommand) Run(args []string) int {
	...
	chaindb, err := node.OpenDatabaseWithFreezer(chaindataPath, int(c.cache), dbHandles, c.datadirAncient, "", false, false, false, false, false, false)
...
func (c *PruneBlockCommand) MarkDown() string {
	items := []string{
		...
		"The ```gilt snapshot prune-block``` command will prune historical blockchain data stored in the ancientdb. The amount of blocks expected for remaining after prune can be specified via `block-amount-reserved` in this command, will prune and only remain the specified amount of old block data in ancientdb.",
		`
...
func (c *PruneBlockCommand) Help() string {
	return `Usage: gilt snapshot prune-block <datadir>
...
func (c *InspectAncientDbCommand) MarkDown() string {
	items := []string{
		...
		"The ```gilt snapshot inspect-ancient-db``` command will inspect few fields in the ancient datastore using the given datadir location.",
		`
...
func (c *InspectAncientDbCommand) Help() string {
	return `Usage: gilt snapshot inspect-ancient-db <datadir>
