...
func (c *DebugBlockCommand) MarkDown() string {
	items := []string{
		"# Debug trace",
		"The ```gilt debug block <number>``` command will create an archive containing traces of a gilt block.",
		c.Flags().MarkDown(),
	}
...
func (c *DebugBlockCommand) Help() string {
	return `Usage: gilt debug block <number>

  This command is used get traces of a gilt block`
}
...
func (c *DebugBlockCommand) Synopsis() string {
	return "Get trace of a gilt block"
}
...
func (c *DebugBlockCommand) Run(args []string) int {
	...
	stream, err := giltClt.DebugBlock(context.Background(), debugRequest)
	if err != nil {
		c.UI.Error(err.Error())
		return 1
	}
