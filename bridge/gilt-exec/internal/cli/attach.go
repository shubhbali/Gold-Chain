...
// AttachCommand is the command to Connect to remote Gilt IPC console
...
func (c *AttachCommand) MarkDown() string {
	items := []string{
		"# Attach",
		"Connect to remote Gilt IPC console.",
		c.Flags().MarkDown(),
	}
...
func (c *AttachCommand) Help() string {
	return `Usage: gilt attach <IPC FILE>
...
func (c *AttachCommand) Synopsis() string {
	return "Connect to Gilt via IPC"
}
...
// remoteConsole will connect to a remote gilt instance, attaching a JavaScript
// console to it.
// nolint:unparam
func (c *AttachCommand) remoteConsole() error {
	...
	if c.Endpoint == "" {
		...
		if path != "" {
			...
			path = filepath.Join(homeDir, "/.gilt/data")
		}

		c.Endpoint = fmt.Sprintf("%s/gilt.ipc", path)
		if _, err := os.Stat(c.Endpoint); err != nil {
			legacyEndpoint := fmt.Sprintf("%s/gilt.ipc", path)
			if _, legacyErr := os.Stat(legacyEndpoint); legacyErr == nil {
				c.Endpoint = legacyEndpoint
			}
		}
	}
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gilt: %v", err)
	}
...
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint("gilt")
