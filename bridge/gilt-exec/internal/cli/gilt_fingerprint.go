...
func formatFingerprint(giltFingerprint *GiltFingerprint) string {
	base := formatKV([]string{
		fmt.Sprintf("Gilt Version : %s", params.VersionWithMeta),
		fmt.Sprintf("CPU : %d cores", giltFingerprint.CoresCount),
		fmt.Sprintf("OS : %s %s ", giltFingerprint.OsName, giltFingerprint.OsVer),
		fmt.Sprintf("RAM :: total : %v GB, free : %v GB, used : %v GB", giltFingerprint.MemoryDetails.TotalMem, giltFingerprint.MemoryDetails.FreeMem, giltFingerprint.MemoryDetails.UsedMem),
		fmt.Sprintf("STORAGE :: total : %v GB, free : %v GB, used : %v GB", giltFingerprint.DiskDetails.TotalDisk, giltFingerprint.DiskDetails.FreeDisk, giltFingerprint.DiskDetails.UsedDisk),
	})
...
func (c *FingerprintCommand) Run(args []string) int {
	...
	giltFingerprint := &GiltFingerprint{
	...
	c.UI.Output(formatFingerprint(giltFingerprint))

	if giltFingerprint.OsName == "linux" {
		err = c.benchmark()
		if err != nil {
			c.UI.Error(err.Error())
			return 1
