...
func (c *PurgeWhitelistedEntriesCommand) MarkDown() string {
	items := []string{
		...
		"The ```gilt purge-whitelisted-entries``` command will remove locally whitelisted checkpoint and milestone entries from the database.",
		c.Flags().MarkDown(),
	}
...
func (c *PurgeWhitelistedEntriesCommand) Help() string {
	return `Usage: gilt purge-whitelisted-entries
