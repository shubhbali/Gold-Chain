...
type Config struct {
	...
	// GiltLogs enables gilt log retrieval
	GiltLogs bool `hcl:"gilt.logs,optional" toml:"gilt.logs,optional"`
	...
	// GiltConsensus has the giltconsensus connection related settings
	GiltConsensus *GiltConsensusConfig `hcl:"giltconsensus,block" toml:"giltconsensus,block"`
...
type HealthConfig struct {
	// MaxGoRoutineThreshold is the maximum number of goroutines before gilt health check fails.
	MaxGoRoutineThreshold int `hcl:"max_goroutine_threshold,optional" toml:"max_goroutine_threshold,optional"`

	// WarnGoRoutineThreshold is the maximum number of goroutines before gilt health check warns.
	WarnGoRoutineThreshold int `hcl:"warn_goroutine_threshold,optional" toml:"warn_goroutine_threshold,optional"`

	// MinPeerThreshold is the minimum number of peers before gilt health check fails.
	MinPeerThreshold int `hcl:"min_peer_threshold,optional" toml:"min_peer_threshold,optional"`

	// WarnPeerThreshold is the minimum number of peers before gilt health check warns.
...
type GiltConsensusConfig struct {
	// URL is the url of the giltconsensus server (comma-separated for failover: "url1,url2,url3")
...
type JsonRPCConfig struct {
	...
	// RangeLimit is the maximum block range allowed for eth_getLogs and gilt_getLogs (0 = no limit).
...
type AccountsConfig struct {
	...
	// DisableGiltWallet disables the personal wallet endpoints
	DisableGiltWallet bool `hcl:"disable-gilt-wallet,optional" toml:"disable-gilt-wallet,optional"`
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		TxPool: &TxPoolConfig{
			...
			PriceLimit:           params.GiltDefaultTxPoolPriceLimit, // gilt's default
			PriceBump:            10,
			AccountSlots:         16,
			GlobalSlots:          131072,
			AccountQueue:         64,
		...
		Gpo: &GpoConfig{
			...
			IgnorePrice:      new(big.Int).Set(gasprice.DefaultIgnorePrice), // gilt's default
		},
		JsonRPC: &JsonRPCConfig{
			...
			Http: &APIConfig{
				...
				Prefix:                      "",
				Host:                        "localhost",
				API:                         []string{"eth", "net", "web3", "txpool", "gilt"},
		...
		Cache: &CacheConfig{
			Cache:                1024, // geth's default (suitable for gilttestnet)
			PercDatabase:         50,
			PercTrie:             15,
			PercGc:               25,
			PercSnapshot:         10,
...
func (c *Config) buildEth(stack *node.Node, accountManager *accounts.Manager) (*ethconfig.Config, error) {
	...
	// Set runtime miner gas parameters in GiltConfig (if not in developer mode and Gilt chain)
	if !c.Developer.Enabled && n.Genesis != nil && n.Genesis.Config != nil && n.Genesis.Config.Gilt != nil {
		...
		if c.Sealer.TargetGasPercentage > 0 {
			...
			n.Genesis.Config.Gilt.TargetGasPercentage = &c.Sealer.TargetGasPercentage
		}
		if c.Sealer.BaseFeeChangeDenominator > 0 {
			n.Genesis.Config.Gilt.BaseFeeChangeDenominator = &c.Sealer.BaseFeeChangeDenominator
		}

		// Wire dynamic target gas percentage configuration to GiltConfig
		if c.Sealer.EnableDynamicTargetGas {
			n.Genesis.Config.Gilt.EnableDynamicTargetGas = &c.Sealer.EnableDynamicTargetGas
			n.Genesis.Config.Gilt.TargetGasMinPercentage = &c.Sealer.TargetGasMinPercentage
			n.Genesis.Config.Gilt.TargetGasMaxPercentage = &c.Sealer.TargetGasMaxPercentage
			n.Genesis.Config.Gilt.TargetBaseFee = &c.Sealer.TargetBaseFee
			n.Genesis.Config.Gilt.BaseFeeBuffer = &c.Sealer.BaseFeeBuffer
		}
	}
	...
	if c.Developer.Enabled {
		...
		// Set runtime miner gas parameters in GiltConfig for developer mode
		if n.Genesis != nil && n.Genesis.Config != nil && n.Genesis.Config.Gilt != nil {
			// Only set if non-zero (0 means not set via CLI, use defaults from consensus)
			if c.Sealer.TargetGasPercentage > 0 {
				if c.Sealer.TargetGasPercentage > 100 {
					return nil, fmt.Errorf("miner.targetGasPercentage must be between 1-100, got %d", c.Sealer.TargetGasPercentage)
...
func DefaultDataDir() string {
	...
	switch runtime.GOOS {
	case "darwin":
		return filepath.Join(home, "Library", "Gilt")
	case "windows":
		...
		return filepath.Join(appdata, "Gilt")
	default:
		return filepath.Join(home, ".gilt")
	}
}
