...
var ErrUnavailable = errors.New("gilt service is currently unavailable, try again later")
var ErrUnavailable2 = errors.New("gilt service unavailable even after waiting for 10 seconds, make sure gilt is running")

func sendStreamDebugFile(stream proto.Gilt_DebugPprofServer, headers map[string]string, data []byte) error {
...
func (s *Server) DebugPprof(req *proto.DebugPprofRequest, stream proto.Gilt_DebugPprofServer) error {
	var (
		payload []byte
...
func (s *Server) Status(ctx context.Context, in *proto.StatusRequest) (*proto.StatusResponse, error) {
	...
	resp := &proto.StatusResponse{
		...
		Forks: gatherForks(s.config.chain.Genesis.Config, s.config.chain.Genesis.Config.Gilt),
	}
...
func (s *Server) DebugBlock(req *proto.DebugBlockRequest, stream proto.Gilt_DebugBlockServer) error {
	...
	res, err := s.tracerAPI.TraceGiltBlock(traceReq)
	if err != nil {
		return err
	}
...
func (s *Server) ChainWatch(req *proto.ChainWatchRequest, reply proto.Gilt_ChainWatchServer) error {
