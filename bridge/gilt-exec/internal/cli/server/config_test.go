...
func TestConfigDefault(t *testing.T) {
	...
	assert.NoError(t, err)
	assertGiltDefaultGasPrice(t, ethConfig)
}

// assertGiltDefaultGasPrice asserts the gilt default gas price is set correctly.
func assertGiltDefaultGasPrice(t *testing.T, ethConfig *ethconfig.Config) {
	assert.NotNil(t, ethConfig)
	assert.Equal(t, ethConfig.Miner.GasPrice, big.NewInt(params.GiltDefaultMinerGasPrice))
}
...
func TestSealerTargetGasPercentageConfig(t *testing.T) {
	t.Run("Valid custom value sets GiltConfig", func(t *testing.T) {
		...
		for _, targetVal := range testCases {
			...
			assert.NotNil(t, ethConfig.Genesis.Config.Gilt.TargetGasPercentage)
			assert.Equal(t, targetVal, *ethConfig.Genesis.Config.Gilt.TargetGasPercentage)
		}
	})
...
func TestSealerBaseFeeChangeDenominatorConfig(t *testing.T) {
	t.Run("Valid custom value sets GiltConfig", func(t *testing.T) {
		...
		for _, denomVal := range testCases {
			...
			assert.NotNil(t, ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
			assert.Equal(t, denomVal, *ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
		}
	})
}
...
func TestSealerBothGasParametersConfig(t *testing.T) {
	t.Run("Both parameters set together", func(t *testing.T) {
		...
		assert.NotNil(t, ethConfig.Genesis.Config.Gilt.TargetGasPercentage)
		assert.Equal(t, uint64(75), *ethConfig.Genesis.Config.Gilt.TargetGasPercentage)

		assert.NotNil(t, ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
		assert.Equal(t, uint64(128), *ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
	})
	...
	t.Run("Only TargetGasPercentage set", func(t *testing.T) {
		...
		assert.NotNil(t, ethConfig.Genesis.Config.Gilt.TargetGasPercentage)
		assert.Equal(t, uint64(80), *ethConfig.Genesis.Config.Gilt.TargetGasPercentage)
	})
	...
	t.Run("Only BaseFeeChangeDenominator set", func(t *testing.T) {
		...
		assert.NotNil(t, ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
		assert.Equal(t, uint64(256), *ethConfig.Genesis.Config.Gilt.BaseFeeChangeDenominator)
	})
...
// and GiltConfig wiring introduced in buildEth.
func TestSealerDynamicTargetGasConfig(t *testing.T) {
	...
	t.Run("ValidConfig_WiresGiltConfig", func(t *testing.T) {
		config := validDynamicTargetGasConfig()

		ethConfig, err := buildConfig(t, config)
		assert.NoError(t, err)

		gilt := ethConfig.Genesis.Config.Gilt
		assert.NotNil(t, gilt.EnableDynamicTargetGas)
		assert.True(t, *gilt.EnableDynamicTargetGas)
		assert.NotNil(t, gilt.TargetGasMinPercentage)
		assert.Equal(t, uint64(50), *gilt.TargetGasMinPercentage)
		assert.NotNil(t, gilt.TargetGasMaxPercentage)
		assert.Equal(t, uint64(80), *gilt.TargetGasMaxPercentage)
		assert.NotNil(t, gilt.TargetBaseFee)
		assert.Equal(t, uint64(25_000_000_000), *gilt.TargetBaseFee)
		assert.NotNil(t, gilt.BaseFeeBuffer)
		assert.Equal(t, uint64(10_000_000_000), *gilt.BaseFeeBuffer)
	})
...
// The default config uses mainnet which has Gilt config, so these tests actually execute lines 1293-1304.
func TestDeveloperModeGasParameters(t *testing.T) {
	t.Run("Scenario 1: Both TargetGasPercentage > 0 AND BaseFeeChangeDenominator > 0", func(t *testing.T) {
		...
		// Mainnet config has Gilt, so lines 1295-1303 execute and set both values
		chainConfig := server.backend.BlockChain().GetChainConfig()
		assert.NotNil(t, chainConfig.Gilt)
		assert.NotNil(t, chainConfig.Gilt.TargetGasPercentage)
		assert.Equal(t, uint64(75), *chainConfig.Gilt.TargetGasPercentage)
		assert.NotNil(t, chainConfig.Gilt.BaseFeeChangeDenominator)
		assert.Equal(t, uint64(128), *chainConfig.Gilt.BaseFeeChangeDenominator)
	})
	...
	t.Run("Scenario 2: Only TargetGasPercentage > 0, BaseFeeChangeDenominator == 0", func(t *testing.T) {
		...
		assert.NotNil(t, chainConfig.Gilt.TargetGasPercentage)
		assert.Equal(t, uint64(80), *chainConfig.Gilt.TargetGasPercentage)
	...
	t.Run("Scenario 3: TargetGasPercentage == 0, only BaseFeeChangeDenominator > 0", func(t *testing.T) {
		...
		assert.NotNil(t, chainConfig.Gilt.BaseFeeChangeDenominator)
		assert.Equal(t, uint64(64), *chainConfig.Gilt.BaseFeeChangeDenominator)
	})
	...
	t.Run("Validation: TargetGasPercentage > 100 fails", func(t *testing.T) {
		...
		// With mainnet Gilt config, validation at line 1296-1298 executes and returns error
