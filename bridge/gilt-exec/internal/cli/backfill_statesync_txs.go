...
func (c *BackFillStateSyncTxsEntriesCommand) Help() string {
	return `Usage: gilt backfill-statesync-txs --datadir <path-to-datadir> --backfill-file <path-to-backfill-file>
...
var (
	// Receipt key byte prefix: "gilt-gilt-receipt-"
	giltReceiptPrefix = []byte("gilt-gilt-receipt-")
...
// Receipt key layout (bytes): prefix ("gilt-gilt-receipt-") + [8 bytes big-endian block number] + [32 bytes block hash]
func parseReceiptKey(keyHex string) (ok bool, blockNumber uint64, blockHash common.Hash, err error) {
	...
	if !bytes.HasPrefix(b, giltReceiptPrefix) {
		return false, 0, common.Hash{}, nil
	}
	suffix := b[len(giltReceiptPrefix):]
	if len(suffix) != 8+32 {
...
func (c *BackFillStateSyncTxsEntriesCommand) Run(args []string) int {
	...
	chaindb, err := node.OpenDatabase(datadir+"/gilt/chaindata", 1024, 2000, "", false)
	if err != nil {
