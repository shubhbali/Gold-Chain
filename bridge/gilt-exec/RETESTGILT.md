...
These integration tests are included in the gilt repo via using the git submodule  
...
## Retesteth - gilt on remote machine
...
- `ssh` into a VM running gilt 
- Change configs by replacing geth with gilt inside the docker container  
```
...
Modify the Dockerfile so that the eth client points to gilt  
...
to: `RUN git clone --depth 1 -b master https://github.com/chatzoneai-spec/Gold-Chain.git /gold-chain && ln -s /gold-chain/bridge/gilt-exec /geth`
...
```
this step is eventually replaced by adding the git submodule directly into gilt repo with   
``` 
...
If you want to run retestheth against a gilt client on localhost:8545 (using 8 threads), instead of isolating it into a docker image, run  
