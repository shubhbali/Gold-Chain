...
func ecrecover(header *types.Header, sigcache *lru.ARCCache, c *params.GiltConfig) (common.Address, error) {
...
func SealHash(header *types.Header, c *params.GiltConfig) (hash common.Hash) {
...
func encodeSigHeader(w io.Writer, header *types.Header, c *params.GiltConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
...
func CalcProducerDelay(number uint64, succession int, c *params.GiltConfig) uint64 {
...
// GiltRLP returns the rlp bytes which needs to be signed for the gilt
...
func GiltRLP(header *types.Header, c *params.GiltConfig) []byte {
	b := new(bytes.Buffer)
...
// New creates a Gilt Gilt consensus engine.
func New(
	...
	// get gilt config
	giltConfig := chainConfig.Gilt

	// Set any missing consensus parameters to their defaults
	if giltConfig != nil && giltConfig.CalculateSprint(0) == 0 {
		giltConfig.Sprint = defaultSprintLength
	}
	...
	spanStore := NewSpanStore(giltconsensusClient, spanner, chainConfig.ChainID.String())
	...
	// make sure we can decode all the GenesisAlloc in the GiltConfig.
...
func (c *Gilt) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures, c.config)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Gilt) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
	return c.verifyHeader(chain, header, nil)
}

func (c *Gilt) GetSpanner() Spanner {
	return c.spanner
}

func (c *Gilt) SetSpanner(spanner Spanner) {
	c.spanner = spanner
}
...
func (c *Gilt) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *Gilt) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if c.config.IsGiugliano(header.Number) {
		...
		// TODO: Once GiltTestnet and Mainnet supports Giugliano HF, we are safe to remove this check (since it only works for block future blocks)
...
func (c *Gilt) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if !c.config.IsRio(header.Number) && !slices.Contains(c.config.SkipValidatorByteCheck, number) {
		...
		// in giltconsensus span is different which will lead a mismatch. Moreover, to make the
		// validation stateless, we use the span from giltconsensus (via span store) instead of
		...
		if number > zerothSpanEnd && IsSprintStart(number+1, c.config.CalculateSprint(number)) {
			...
			selectedProducers := giltSpan.ConvertGiltConsensusValidatorsToGiltValidators(span.SelectedProducers)
	...
	for snap == nil {
		...
		if number == 0 {
			...
			if checkpoint != nil {
				...
				// new snap shot
				giltValSet := giltSpan.ConvertGiltConsensusValSetToGiltValSet(span.ValidatorSet)
				snap = newSnapshot(c.chainConfig, c.signatures, number, hash, giltValSet.Validators)
...
func (c *Gilt) getVeBlopSnapshot(chain consensus.ChainHeaderReader, targetHeader *types.Header, parents []*types.Header, checkNewSpan bool) (*Snapshot, error) {
...
func (c *Gilt) performSpanCheck(chain consensus.ChainHeaderReader, targetHeader *types.Header, parents []*types.Header) error {
...
func (c *Gilt) VerifyUncles(_ consensus.ChainReader, block *types.Block) error {
...
func (c *Gilt) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Gilt) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func IsBlockEarly(parent *types.Header, header *types.Header, number uint64, succession int, cfg *params.GiltConfig) bool {
...
func (c *Gilt) setGiuglianoExtraFields(header *types.Header, parent *types.Header, blockExtraData *types.BlockExtraData) {
	if c.config.IsGiugliano(header.Number) {
...
func (c *Gilt) Prepare(chain consensus.ChainHeaderReader, header *types.Header, waitOnPrepare bool) error {
...
// rewards given.
func (c *Gilt) Finalize(chain consensus.ChainHeaderReader, header *types.Header, wrappedState vm.StateDB, body *types.Body, receipts []*types.Receipt) []*types.Receipt {
	...
	if IsSprintStart(headerNumber, c.config.CalculateSprint(headerNumber)) {
		start := time.Now()
		cx := statefull.ChainContext{Chain: chain, Gilt: c}
		...
		state.GiltConsensusTime = time.Since(start)
	}
...
func (c *Gilt) changeContractCodeIfNeeded(headerNumber uint64, state vm.StateDB) error {
...
func (c *Gilt) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, []*types.Receipt, time.Duration, error) {
	headerNumber := header.Number.Uint64()
	if body.Withdrawals != nil || header.WithdrawalsHash != nil {
	...
	if IsSprintStart(headerNumber, c.config.CalculateSprint(headerNumber)) {
		...
		state.GiltConsensusTime = time.Since(giltStart)
	}
...
// with.
func (c *Gilt) Authorize(currentSigner common.Address, signFn SignerFn) {
	c.authorizedSigner.Store(&signer{
...
func (c *Gilt) Seal(chain consensus.ChainHeaderReader, block *types.Block, witness *stateless.Witness, results chan<- *consensus.NewSealedBlockEvent, stop <-chan struct{}) error {
	header := block.Header()
...
func Sign(signFn SignerFn, signer common.Address, header *types.Header, c *params.GiltConfig) error {
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypeGilt, GiltRLP(header, c))
	if err != nil {
		return err
	}
...
// current signer.
func (c *Gilt) CalcDifficulty(chain consensus.ChainHeaderReader, _ uint64, parent *types.Header) *big.Int {
...
// SealHash returns the hash of a block prior to it being sealed.
func (c *Gilt) SealHash(header *types.Header) common.Hash {
	return SealHash(header, c.config)
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (c *Gilt) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "gilt",
		Version:   "1.0",
		Service:   &API{chain: chain, gilt: c},
		Public:    false,
	}}
}

// Close implements consensus.Engine.
func (c *Gilt) Close() error {
	c.closeOnce.Do(func() {
		c.ctxCancel()
		close(c.quit)
		if c.GiltConsensusClient != nil {
			c.GiltConsensusClient.Close()
		}
...
func (c *Gilt) runMilestoneFetcher() {
	...
	for {
		...
		case <-ticker.C:
			if c.GiltConsensusClient != nil {
				...
				milestone, err := c.GiltConsensusClient.FetchMilestone(ctx)
				cancel()
				if err != nil {
...
func (c *Gilt) checkAndCommitSpan(
	state vm.StateDB,
	header *types.Header,
	...
	tempState.StartPrefetcher("gilt", state.Witness(), nil)
...
func (c *Gilt) needToCommitSpan(currentSpan *giltTypes.Span, headerNumber uint64) bool {
	...
	// Check: https://github.com/giltchain/gilt-genesis-contract/blob/5dcbcc72f10ab847276586e629f96b8a6d369e1d/contracts/GiltValidatorSet.template#L229
...
func (c *Gilt) FetchAndCommitSpan(
	ctx context.Context,
	newSpanID uint64,
	state vm.StateDB,
	header *types.Header,
	...
	if c.GiltConsensusClient == nil {
		// fixme: move to a new mock or fake and remove c.GiltConsensusClient completely
		s, err := c.getNextGiltConsensusSpanForTest(ctx, newSpanID, header, chain)
		if err != nil {
			return err
		}
	...
	// check if chain id matches with GiltConsensus span
	if chainId != c.chainConfig.ChainID.String() {
		return fmt.Errorf(
			"chain id proposed span, %s, and gilt chain id, %s, doesn't match",
			chainId,
...
func (c *Gilt) CommitStates(
	...
	log.Info(
		"Fetching state updates from GiltConsensus",
		"fromID", from,
		"to", to.Format(time.RFC3339))

	var eventRecords []*clerk.EventRecordWithTime

	// Wait for giltconsensus to be synced before fetching state sync events
	c.spanStore.waitUntilGiltConsensusIsSynced(c.ctx)

	eventRecords, err = c.GiltConsensusClient.StateSyncEvents(c.ctx, from, to.Unix())
	if err != nil {
	...
	for _, eventRecord := range eventRecords {
		...
		// https://github.com/giltchain/gilt-genesis-contract/blob/master/contracts/StateReceiver.sol#L27
...
func (c *Gilt) SetGiltConsensusClient(h IGiltConsensusClient) {
	c.GiltConsensusClient = h
	// Update the giltconsensus client in span store
	c.spanStore.setGiltConsensusClient(h)
}
...
func (c *Gilt) GetCurrentValidators(ctx context.Context, headerHash common.Hash, blockNumber uint64) ([]*valset.Validator, error) {
...
func (c *Gilt) getNextGiltConsensusSpanForTest(
	ctx context.Context,
	newSpanID uint64,
	header *types.Header,
	chain core.ChainContext,
) (*giltTypes.Span, error) {
	headerNumber := header.Number.Uint64()

	spanGilt, err := c.spanner.GetCurrentSpan(ctx, header.ParentHash, nil)
	if err != nil {
		return nil, err
	}
	...
	// new span
	spanGilt.Id = newSpanID
	if spanGilt.EndBlock == 0 {
		spanGilt.StartBlock = 256
	} else {
		spanGilt.StartBlock = spanGilt.EndBlock + 1
	}

	spanGilt.EndBlock = spanGilt.StartBlock + (100 * c.config.CalculateSprint(headerNumber)) - 1
	spanGilt.GiltChainId = c.chainConfig.ChainID.String()
	spanGilt.ValidatorSet = giltSpan.ConvertGiltValSetToGiltConsensusValSet(snap.ValidatorSet)
	spanGilt.SelectedProducers = giltSpan.ConvertGiltValidatorsToGiltConsensusValidators(snap.ValidatorSet.Validators)
