...
func TestWSClient_ConstructorSingleURL(t *testing.T) {
	client, err := NewGiltConsensusWSClient("ws://localhost:1234")
...
func TestWSClient_ConstructorMultipleURLs(t *testing.T) {
	client, err := NewGiltConsensusWSClient("ws://primary:1234", "ws://secondary:5678", "ws://tertiary:9999")
	require.NoError(t, err)
...
func TestWSClient_ConstructorFiltersEmpty(t *testing.T) {
	client, err := NewGiltConsensusWSClient("ws://primary:1234", "", "ws://tertiary:9999")
	require.NoError(t, err)
...
func TestWSClient_ConstructorNoURLs(t *testing.T) {
	_, err := NewGiltConsensusWSClient()
	require.Error(t, err)
}
...
func TestWSClient_SingleURL_ConnectsSuccessfully(t *testing.T) {
	...
	client, err := NewGiltConsensusWSClient(wsURL(server.URL))
	require.NoError(t, err)
	...
	case m := <-events:
		...
		assert.Equal(t, "137", m.GiltChainID)
...
func TestWSClient_DualURL_FailoverToSecondary(t *testing.T) {
	...
	client, err := NewGiltConsensusWSClient(wsURL(primary.URL), wsURL(secondary.URL))
	require.NoError(t, err)
...
func TestWSClient_ThreeURL_CascadeToTertiary(t *testing.T) {
	...
	client, err := NewGiltConsensusWSClient(wsURL(primary.URL), wsURL(secondary.URL), wsURL(tertiary.URL))
	require.NoError(t, err)
...
func TestWSClient_DualURL_PrimaryRecovery(t *testing.T) {
	...
	client, err := NewGiltConsensusWSClient(wsURL(primaryReject.URL), wsURL(secondary.URL))
	require.NoError(t, err)
