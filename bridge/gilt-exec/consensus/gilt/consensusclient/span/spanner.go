...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/gilt/abi"
...
func (c *ChainSpanner) GetCurrentSpan(ctx context.Context, headerHash common.Hash, state *state.StateDB) (*giltTypes.Span, error) {
	// block
...
func (c *ChainSpanner) GetCurrentValidatorsByBlockNrOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, blockNumber uint64) ([]*valset.Validator, error) {
	...
	valz, err := c.tryGetGiltValidatorsWithId(ctx, blockNrOrHash, blockNumber, toAddress, gas)
	if err != nil {
		return nil, err
	}
...
// tryGetGiltValidatorsWithId Try to get gilt validators with Id from ValidatorSet contract by querying each element on mapping(uint256 => Validator[]) public producers
// If fails then returns GetGiltValidators without id
func (c *ChainSpanner) tryGetGiltValidatorsWithId(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, blockNumber uint64, toAddress common.Address, gas hexutil.Uint64) ([]*valset.Validator, error) {
	...
	giltValidatorsWithoutId, err := c.getGiltValidatorsWithoutId(ctx, blockNrOrHash, blockNumber, toAddress)
	if err != nil {
		return nil, err
	}

	producersCount := len(giltValidatorsWithoutId)
	...
	for i := 0; i < producersCount; i++ {
		...
		if err != nil {
			return giltValidatorsWithoutId, nil
		}
...
func (c *ChainSpanner) getGiltValidatorsWithoutId(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, blockNumber uint64, toAddress common.Address) ([]*valset.Validator, error) {
	// method
	const method = "getGiltValidators"
...
func (c *ChainSpanner) CommitSpan(ctx context.Context, minimalSpan giltTypes.Span, validators, producers []stakeTypes.MinimalVal, state vm.StateDB, header *types.Header, chainContext core.ChainContext) error {
