...
func loadSnapshot(chainConfig *params.ChainConfig, config *params.GiltConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("gilt-"), hash[:]...))
	if err != nil {
		return nil, err
	}
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("gilt-"), s.Hash[:]...), blob)
}
...
func (s *Snapshot) apply(headers []*types.Header, c *Gilt) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if number >= s.chainConfig.Gilt.CalculateSprint(number) {
			delete(snap.Recents, number-s.chainConfig.Gilt.CalculateSprint(number))
		}

		// Resolve the authorization key and check against signers
		signer, err := ecrecover(header, s.sigcache, s.chainConfig.Gilt)
		if err != nil {
			return nil, err
		}
		...
		if number > 0 && (number+1)%s.chainConfig.Gilt.CalculateSprint(number) == 0 {
			...
			if v.CheckEmptyId() {
				...
				v.IncludeIds(giltSpan.ConvertGiltConsensusValSetToGiltValSet(span.ValidatorSet).Validators)
			}
			snap.ValidatorSet = v
		}
	}
...
func (s *Snapshot) isAllowedByValidatorSetOverride(addr common.Address, blockNumber uint64) bool {
	if s.chainConfig == nil || s.chainConfig.Gilt == nil {
		return false
	}

	overrides := s.chainConfig.Gilt.OverrideValidatorSetInRange
	if len(overrides) == 0 {
		return false
	}
