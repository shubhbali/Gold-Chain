...
func (h *GiltConsensusGRPCClient) FetchCheckpointCount(ctx context.Context) (int64, error) {
	...
	start := time.Now()
	ctx = giltconsensus.WithRequestType(ctx, giltconsensus.CheckpointCountRequest)
...
func (h *GiltConsensusGRPCClient) FetchCheckpoint(ctx context.Context, number int64) (*checkpoint.Checkpoint, error) {
	var fetchedCheckpoint checkpointTypes.Checkpoint
	var err error

	// Start the timer and set the request type on the context.
	start := time.Now()
	ctx = giltconsensus.WithRequestType(ctx, giltconsensus.CheckpointRequest)
	...
	checkpoint := &checkpoint.Checkpoint{
		...
		GiltChainID: fetchedCheckpoint.GiltChainId,
