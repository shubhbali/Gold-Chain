...
// Used to send validator information to gilt validator contract
...
// GiltConsensusToValSetValidators converts giltconsensus validators to valset validators
func GiltConsensusToValSetValidators(giltconsensusValidators []*stakeTypes.Validator) []*Validator {
	result := make([]*Validator, len(giltconsensusValidators))

	for i, giltconsensusValidator := range giltconsensusValidators {
		result[i] = GiltConsensusToValSetValidator(giltconsensusValidator)
	}

	return result
}

// GiltConsensusToValSetValidator converts giltconsensus validator to valset validator
func GiltConsensusToValSetValidator(giltconsensusValidator *stakeTypes.Validator) *Validator {
	return &Validator{
		ID:               giltconsensusValidator.ValId,
		Address:          common.HexToAddress(giltconsensusValidator.Signer),
		VotingPower:      giltconsensusValidator.VotingPower,
		ProposerPriority: giltconsensusValidator.ProposerPriority,
	}
}
