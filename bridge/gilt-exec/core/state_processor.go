...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config, author *common.Address, interruptCtx context.Context) (*ProcessResult, error) {
	...
	var requests [][]byte
	if p.chainConfig().IsPrague(block.Number()) && p.chainConfig().Gilt == nil {
		requests = [][]byte{}
		// EIP-6110
	...
	if p.chainConfig().Gilt != nil && p.chainConfig().Gilt.IsMadhugiri(block.Number()) {
		...
		if len(block.Transactions()) != len(receipts) {
			return nil, fmt.Errorf("err in gilt.Finalize: %w", ErrStateSyncProcessing)
		}
