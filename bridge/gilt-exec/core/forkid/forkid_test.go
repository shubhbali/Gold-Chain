...
func TestValidation(t *testing.T) {
	...
	tests := []struct {
		...
		// GILT
		// Gilt doesn't support timestamp based forks
...
func TestTimeBasedForkInGenesis(t *testing.T) {
	t.Skip("This test is not valid in gilt since gilt has block based hardforks")
	var (
...
// TestGatherForksGilt verifies that the exported GatherForks includes Gilt-specific fork blocks
// and correctly deduplicates/sorts them alongside the base chain forks.
func TestGatherForksGilt(t *testing.T) {
	// Config with known Gilt forks
	...
	// The Gilt forks must appear in the result
	giltForks := map[uint64]bool{100: false, 200: false, 300: false, 400: false}
	for _, f := range heightForks {
		if _, ok := giltForks[f]; ok {
			giltForks[f] = true
		}
	}
	for fork, found := range giltForks {
		if !found {
			t.Errorf("Gilt fork at block %d not found in GatherForks result", fork)
		}
	}
...
// TestGatherForksNilGilt verifies GatherForks works when Gilt config is nil.
func TestGatherForksNilGilt(t *testing.T) {
	cfg := &params.ChainConfig{
...
// TestGatherForksGiltZeroBlock verifies that Gilt forks at block 0 are filtered out.
func TestGatherForksGiltZeroBlock(t *testing.T) {
	cfg := &params.ChainConfig{
		HomesteadBlock: big.NewInt(0),
		Gilt: &params.GiltConfig{
