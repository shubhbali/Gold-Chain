...
// Returns heightForks (block number forks) and timeForks (timestamp forks, which are not supported in gilt).
// Returns nil for empty fork lists.
// This extends the internal gatherForks with Gilt-specific fork handling.
func GatherForks(config *params.ChainConfig, genesisTime uint64) (heightForks []uint64, timeForks []uint64) {
	...
	// Add Gilt-specific fork blocks (explicit list to avoid reflection fragility)
	if config.Gilt != nil {
		for _, fork := range []*big.Int{
			config.Gilt.JaipurBlock,
			config.Gilt.DelhiBlock,
			config.Gilt.IndoreBlock,
			config.Gilt.AhmedabadBlock,
			config.Gilt.BhilaiBlock,
			config.Gilt.RioBlock,
			config.Gilt.MadhugiriBlock,
			config.Gilt.MadhugiriProBlock,
			config.Gilt.DandeliBlock,
			config.Gilt.LisovoBlock,
			config.Gilt.LisovoProBlock,
			config.Gilt.GiuglianoBlock,
		} {
			if fork != nil {
		...
		// Re-sort and deduplicate after adding Gilt forks
