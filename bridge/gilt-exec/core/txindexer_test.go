...
func TestTxIndexer(t *testing.T) {
	...
	giltReceipts := make([]types.Receipts, len(receipts))

	for _, c := range cases {
		db, _ := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{})
		rawdb.WriteAncientBlocks(db, append([]*types.Block{gspec.ToBlock()}, blocks...), types.EncodeBlockReceiptLists(append([]types.Receipts{{}}, receipts...)), types.EncodeBlockReceiptLists(append([]types.Receipts{{}}, giltReceipts...)), big.NewInt(0))
...
func TestTxIndexerRepair(t *testing.T) {
	...
	for _, c := range cases {
		...
		encGiltReceipts := types.EncodeBlockReceiptLists(append([]types.Receipts{{}}, giltReceipts...))
		rawdb.WriteAncientBlocks(db, append([]*types.Block{gspec.ToBlock()}, blocks...), encReceipts, encGiltReceipts, big.NewInt(0))
