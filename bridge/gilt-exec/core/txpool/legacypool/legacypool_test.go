...
import (
	...
	crand2 "github.com/giltchain/crand"
...
func init() {
	...
	/*
		Given the introduction of `GiltDefaultTxPoolPriceLimit=25gwei`,
...
// TestTxPoolDefaultPriceLimit ensures the gilt default tx pool price limit is set correctly.
func TestTxPoolDefaultPriceLimit(t *testing.T) {
	...
	if have, want := pool.config.PriceLimit, uint64(params.GiltDefaultTxPoolPriceLimit); have != want {
...
func setupRebroadcastTest(t *testing.T, interval, maxAge time.Duration, batchSize int) (*LegacyPool, *ecdsa.PrivateKey, common.Address, *types.Transaction) {
	...
	tx := pricedTransaction(0, 100000, big.NewInt(params.GiltDefaultTxPoolPriceLimit), key)
	if err := pool.addRemoteSync(tx); err != nil {
		pool.Close()
		t.Fatalf("failed to add transaction: %v", err)
	}
...
func TestIdentifyStuckTransactions(t *testing.T) {
	...
	t.Run("BatchSizeLimit", func(t *testing.T) {
		...
		// Add 5 transactions
		for i := range 5 {
			tx := pricedTransaction(uint64(i), 100000, big.NewInt(params.GiltDefaultTxPoolPriceLimit), key)
			if err := pool.addRemoteSync(tx); err != nil {
