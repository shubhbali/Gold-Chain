...
var DefaultConfig = Config{
	...
	PriceLimit: params.GiltDefaultTxPoolPriceLimit,
	PriceBump:  10,
...
func (config *Config) sanitize() Config {
	...
	if conf.PriceLimit != params.GiltDefaultTxPoolPriceLimit {
...
func (pool *LegacyPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*SortedMap) {
	...
	if reset != nil {
		if reset.newHead != nil && reset.oldHead != nil {
			// Gilt: EIP-7825 at Madhugiri HF block
			isOsaka := pool.chainconfig.IsOsaka(reset.newHead.Number) && !pool.chainconfig.IsOsaka(reset.oldHead.Number)
			isMadhugiri := pool.chainconfig.Gilt != nil && (pool.chainconfig.Gilt.IsMadhugiri(reset.newHead.Number) && !pool.chainconfig.Gilt.IsMadhugiri(reset.oldHead.Number))
	...
	// Gilt: TxPool in order to maintain strict consistency acquires a lock for
...
func (pool *LegacyPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		// gilt: Drop all transactions that no longer have valid TxOptions
