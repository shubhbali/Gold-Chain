...
// GILT: if this test failed, it means you should include PrecompiledP256Verify in the PrecompiledContracts
...
func TestPrecompiledP256VerifyAlwaysAvailableInHFs(t *testing.T) {
	chainConfigs := []*params.ChainConfig{params.GiltMainnetChainConfig, params.GiltTestnetChainConfig}
...
//     The test must run on a multiclient network, including both Erigon and Gilt. The test is available in our e2e repository.
//
