...
//
// Note, as gilt receipts and tx lookup are also stored with prefix "gilt-gilt", it needs
// to be excluded from range deletion.
func DeleteStateHistoryIndex(db ethdb.KeyValueRangeDeleter) {
	// We split the delete range queries into 2 parts to ensure that gilt receipts related data isn't deleted.
	// 1. `StateHistoryIndexPrefix` (i.e. "m") to "gilt-gilt" key (exclusive)
	// 2. Immediate next key after "gilt-gilt" to next key after `StateHistoryIndexPrefix` (i.e. "n")
	start1 := StateHistoryIndexPrefix
	limit1 := []byte("gilt-gilt")
