...
// TestDeleteStateHistoryIndex_WithoutGiltReceipts tests if the deletion of all
// history indexing data works and gilt receipts aren't deleted despite having same
// prefix for deletion.
func TestDeleteStateHistoryIndex_WithoutGiltReceipts(t *testing.T) {
	...
	// Insert gilt receipt and tx lookup entry
	key = types.GiltReceiptKey(1, common.Hash{})
	db.Put(key, []byte{})
	key = giltTxLookupKey(common.Hash{})
	db.Put(key, []byte{})

	// Insert in the next key to "gilt-gilt"
	db.Put([]byte("gilt-bos"), []byte{})
	...
	tests := []Testcase{
		...
		// Gilt receipt related data
		{"checking 'gilt-gilt-receipts' key", types.GiltReceiptKey(1, common.Hash{}), true},
		{"checking 'gilt-gilt-tx-lookup' key", giltTxLookupKey(common.Hash{}), true},
		{"checking 'gilt-bos' key", []byte("gilt-bos"), false}, // this one should be deleted
