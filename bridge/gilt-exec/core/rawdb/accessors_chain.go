...
func WriteAncientBlocks(db ethdb.AncientWriter, blocks []*types.Block, receipts []rlp.RawValue, giltReceipts []rlp.RawValue, td *big.Int) (int64, error) {
	...
	return db.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i, block := range blocks {
			...
			if err := writeAncientBlock(op, block, header, receipts[i], giltReceipts[i], tdSum); err != nil {
				return err
			}
		}
...
func writeAncientBlock(op ethdb.AncientWriteOp, block *types.Block, header *types.Header, receipt, giltReceipt rlp.RawValue, td *big.Int) error {
	num := block.NumberU64()
	...
	if err := op.Append(freezerGiltReceiptTable, num, giltReceipt); err != nil {
		return fmt.Errorf("can't append block %d gilt receipts: %v", num, err)
	}
...
func DeleteBlock(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	...
	// delete gilt receipt
	DeleteGiltReceipt(db, hash, number)
}
