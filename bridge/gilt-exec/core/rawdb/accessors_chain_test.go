...
func TestHeadersRLPStorage(t *testing.T) {
	...
	var giltReceipts []types.Receipts = make([]types.Receipts, 100)

	// Write first half to ancients
	WriteAncientBlocks(db, chain[:50], types.EncodeBlockReceiptLists(receipts[:50]), types.EncodeBlockReceiptLists(giltReceipts[:50]), big.NewInt(100))
