...
func (f *chainFreezer) freezeRange(nfdb *nofreezedb, number, limit uint64) (hashes []common.Hash, err error) {
	...
	_, err = f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for ; number <= limit; number++ {
			...
			// gilt block receipt
			giltBlockReceipt := ReadGiltReceiptRLP(nfdb, hash, number)
			if err := op.AppendRaw(freezerGiltReceiptTable, number, giltBlockReceipt); err != nil {
				return fmt.Errorf("can't write gilt-receipt to freezer: %v", err)
			}
