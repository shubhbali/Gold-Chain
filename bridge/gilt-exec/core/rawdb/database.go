		...
		if frozen, _ := frdb.ItemAmountInAncient(); frozen > 0 {
			...
			if offset == 0 {
				...
				if ReadHeadHeaderHash(db) != common.BytesToHash(kvgenesis) {
					...
					// Gilt Change: Stateless client skip this check because of fast forward feature, which usually jumps #1 block
...
var knownMetadataKeys = [][]byte{
	...
	filterMapsRangeKey, headStateHistoryIndexKey, bytecodeSyncLastBlockKey, VerkleTransitionStatePrefix,
}
