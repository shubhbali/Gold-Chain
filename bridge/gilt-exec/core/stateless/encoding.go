...
// GiltWitness is the canonical 3-field RLP encoding used for network
// transmission in Gilt. The State field carries all proof data — both
// contract bytecodes and MPT state trie nodes — as a flat list of byte slices.
type GiltWitness struct {
...
// EncodeRLP serializes a witness as RLP using the canonical GiltWitness 3-field
...
// GiltWitness 3-field format. If that fails (e.g. the peer sent the legacy
// 5-field ExtWitness encoding), it falls back to ExtWitness for backward
// compatibility.
//
// When decoding GiltWitness, State items are placed into w.State and w.Codes
// is left empty, since codes are not part of the GiltWitness wire format.
func (w *Witness) DecodeRLP(s *rlp.Stream) error {
	...
	// Try GiltWitness first. A 5-field list will fail here with "too many
