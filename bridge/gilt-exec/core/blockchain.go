...
var (
	...
	giltConsensusTime = metrics.NewRegisteredTimer("chain/gilt/consensus", nil)
...
const (
	...
	// - Version 9
	//  The following incompatible database changes were added:
	//  * (not applicable for gilt) Total difficulty has been removed from both the key-value store and the ancient store.
...
type BlockChainConfig struct {
	...
	// MilestoneFetcher returns the latest milestone end block from GiltConsensus.
...
type BlockChain struct {
	...
	// Gilt related changes
	giltReceiptsCache    *lru.Cache[common.Hash, *types.Receipt]   // Cache for the most recent gilt receipt receipts per block
	stateSyncMu         sync.RWMutex                              // Mutex to protect the stateSyncData access
	giltReceiptsRLPCache *lru.Cache[common.Hash, rlp.RawValue]     // Cache for the most recent gilt receipt RLPs per block
	...
	chainSideFeed       event.Feed                                // Side chain data feed (removed from geth but needed in gilt)
	milestoneFetcher    func(ctx context.Context) (uint64, error) // Function to fetch the latest milestone end block from GiltConsensus.
}
...
func NewBlockChain(db ethdb.Database, genesis *Genesis, engine consensus.Engine, cfg *BlockChainConfig) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		engine:              engine,
		giltReceiptsCache:    lru.NewCache[common.Hash, *types.Receipt](receiptsCacheLimit),
		giltReceiptsRLPCache: lru.NewCache[common.Hash, rlp.RawValue](receiptsCacheLimit),
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			rawdb.DeleteGiltReceipt(db, hash, num)
			rawdb.DeleteGiltTxLookupEntry(db, hash, num)
		}
...
// and cumulative gas used for populating the gilt receipt.
...
func splitReceiptsAndDeriveFields(receipts rlp.RawValue, number uint64, hash common.Hash, giltCfg *params.GiltConfig) (rlp.RawValue, rlp.RawValue) {
	if receipts == nil {
		return nil, nil
	}
	...
	if giltCfg.IsMadhugiri(big.NewInt(int64(number))) {
		return receipts, nil
	}

	// Gilt receipts can only exist on sprint end blocks. Avoid decoding if possible.
	if !types.IsSprintEndBlock(giltCfg, number) {
		return receipts, nil
	}
	...
	if isStateSyncReceiptPresent(decoded) {
		giltReceipt := decoded[len(decoded)-1]

		// Derive rest of fields for gilt receipts before encoding back
		txIndex, logIndex, cumulativeGasUsed := getReceiptFields(decoded[:len(decoded)-1])
		types.DeriveFieldsForGiltLogs(giltReceipt.Logs, hash, number, uint(txIndex), uint(logIndex))
		giltReceipt.Status = types.ReceiptStatusSuccessful
		giltReceipt.CumulativeGasUsed = cumulativeGasUsed

		// Encode the state-sync transaction receipt separately
		encodedStateSyncReceipt, err := rlp.EncodeToBytes(giltReceipt)
		if err != nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []rlp.RawValue, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []rlp.RawValue) (int, error) {
		...
		// Separate out gilt receipts (i.e. receipts of state-sync transactions)
		giltReceipts := make([]rlp.RawValue, len(receiptChain))
		for i, receipts := range receiptChain {
			receiptChain[i], giltReceipts[i] = splitReceiptsAndDeriveFields(receipts, blockChain[i].NumberU64(), blockChain[i].Hash(), bc.chainConfig.Gilt)
		}
		...
		writeSize, err := rawdb.WriteAncientBlocks(bc.db, blockChain, receiptChain, giltReceipts, td)
		if err != nil {
		...
		for i, block := range blockChain {
			if bc.txIndexer == nil || bc.txIndexer.limit == 0 || ancientLimit <= bc.txIndexer.limit || block.NumberU64() >= ancientLimit-bc.txIndexer.limit {
				rawdb.WriteTxLookupEntriesByBlock(batch, block)
				if len(giltReceipts[i]) > 0 {
					rawdb.WriteGiltTxLookupEntry(batch, block.Hash(), block.NumberU64())
				}
	...
	writeLive := func(blockChain types.Blocks, receiptChain []rlp.RawValue) (int, error) {
		...
		for i, block := range blockChain {
			// Update the headers for gilt specific reorg check
			headers = append(headers, block.Header())
			...
			// Separate out gilt receipts (i.e. receipts of state-sync transactions)
			var giltReceiptRaw rlp.RawValue
			receiptChain[i], giltReceiptRaw = splitReceiptsAndDeriveFields(receiptChain[i], block.NumberU64(), block.Hash(), bc.chainConfig.Gilt)
			...
			var giltReceipt types.ReceiptForStorage
			if len(giltReceiptRaw) > 0 {
				if err := rlp.DecodeBytes(giltReceiptRaw, &giltReceipt); err == nil {
					rawdb.WriteGiltReceipt(batch, block.Hash(), block.NumberU64(), &giltReceipt)
					rawdb.WriteGiltTxLookupEntry(batch, block.Hash(), block.NumberU64())
				}
			}
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, statedb *state.StateDB) ([]*types.Log, error) {
	...
	if len(blockLogs) > 0 {
		// After Madhugiri HF we don't write gilt receipts separately
		if !(bc.chainConfig.Gilt != nil && bc.chainConfig.Gilt.IsMadhugiri(block.Number())) && len(blockLogs) > len(logs) {
			...
			// DeriveFieldsForGiltLogs will fill those fields for websocket subscriptions
			types.DeriveFieldsForGiltLogs(stateSyncLogs, block.Hash(), block.NumberU64(), uint(len(receipts)), uint(len(logs)))
			...
			// Write gilt receipt
			rawdb.WriteGiltReceipt(blockBatch, block.Hash(), block.NumberU64(), &types.ReceiptForStorage{
				Status:            types.ReceiptStatusSuccessful, // make receipt status successful
				Logs:              stateSyncLogs,
				CumulativeGasUsed: cumulativeGasUsed,
			})

			// Write gilt tx reverse lookup
			rawdb.WriteGiltTxLookupEntry(blockBatch, block.Hash(), block.NumberU64())
		}
	}
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool, stateless bool) (status WriteStatus, err error) {
	...
	if status == CanonStatTy {
		...
		if emitHeadEvent {
			...
			// GILT state sync feed related changes
...
func (bc *BlockChain) insertChainWithWitnesses(chain types.Blocks, setHead bool, makeWitness bool, witnesses []*stateless.Witness) (*stateless.Witness, int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		if bc.chainConfig.IsByzantium(block.Number()) {
			...
			// Gilt: We start the prefetcher in process block function called below
		...
		bc.stateSyncMu.RUnlock()
		// GILT
		ptime := time.Since(pstart) - vtime - statedb.GiltConsensusTime
		...
		giltConsensusTime.Update(statedb.GiltConsensusTime)               // The time spent on gilt consensus (span + state sync)
...
func (bc *BlockChain) collectReceiptsAndLogs(b *types.Block, removed bool) ([]*types.Receipt, []*types.Log) {
	...
	// Append gilt receipt
	giltReceipt := rawdb.ReadGiltReceipt(bc.db, b.Hash(), b.NumberU64(), bc.chainConfig)
	if giltReceipt != nil {
		receipts = append(receipts, giltReceipt)
	}
...
func (bc *BlockChain) reorg(oldHead *types.Header, newHead *types.Header) error {
	...
	// Deleted log emission on the API uses forward order, which is giltked, but
	...
	// TODO(karalabe): This should be nuked out, no idea how, deprecate some APIs?
	{
		for i := len(oldChain) - 1; i >= 0; i-- {
			...
			// the concept of side chains but we need them in gilt.
...
func (bc *BlockChain) ProcessBlockWithWitnesses(block *types.Block, witness *stateless.Witness) (*state.StateDB, *ProcessResult, error) {
	...
	// Gilt: Calculate EvmBlockContext with Root and ReceiptHash to properly get the author
	author := NewEVMBlockContext(block.Header(), bc.hc, nil).Coinbase
...
// verifyPendingHeaders fetches the latest milestone from GiltConsensus and verifies
...
func (bc *BlockChain) verifyPendingHeaders() {
	...
	if chainConfig.Gilt == nil || !chainConfig.Gilt.IsRio(currentHead.Number) {
		return
	}
	...
	if err != nil {
		log.Error("Failed to fetch milestone end block from GiltConsensus for header verification", "err", err)
		return
	}
