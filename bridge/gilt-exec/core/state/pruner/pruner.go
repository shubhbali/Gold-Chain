...
func (p *BlockPruner) backupOldDb(name string, cache, handles int, namespace string, readonly, interrupt bool) error {
	...
	writeBlock := func(blockNumber uint64) error {
		...
		giltReceipts := rawdb.ReadGiltReceiptRLP(chainDb, blockHash, blockNumber)
		...
		if _, err := rawdb.WriteAncientBlocks(frdbBack, []*types.Block{block}, []rlp.RawValue{receipts}, []rlp.RawValue{giltReceipts}, td); err != nil {
