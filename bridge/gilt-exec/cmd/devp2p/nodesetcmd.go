...
func ethFilter(args []string) (nodeFilter, error) {
	...
	case "gilt-gilttestnet-legacy":
		filter = forkid.NewStaticFilter(params.GiltTestnetChainConfig, core.DefaultGiltTestnetGenesisBlock().ToBlock())
	case "gilt-mainnet":
		filter = forkid.NewStaticFilter(params.GiltMainnetChainConfig, core.DefaultGiltMainnetGenesisBlock().ToBlock())
	case "gilt-gilttestnet":
		filter = forkid.NewStaticFilter(params.GiltTestnetChainConfig, core.DefaultGiltTestnetGenesisBlock().ToBlock())
	case "holesky":
