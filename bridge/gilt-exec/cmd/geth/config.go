...
func loadBaseConfig(ctx *cli.Context) gethConfig {
	...
	if ctx.IsSet(utils.GiltTestnetLegacyFlag.Name) {
		log.Warn("`--gilt-gilttestnet-legacy` is deprecated, using canonical `--gilt-gilttestnet`")
		setDefaultGiltTestnetGethConfig(ctx, &cfg)
	}

	if ctx.IsSet(utils.GiltTestnetFlag.Name) {
		setDefaultGiltTestnetGethConfig(ctx, &cfg)
	}

	if ctx.IsSet(utils.GiltMainnetFlag.Name) {
		setDefaultGiltMainnetGethConfig(ctx, &cfg)
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	// Set Gilt config flags
	utils.SetGiltConfig(ctx, &cfg.Eth)
...
func setDefaultGiltTestnetLegacyGethConfig(ctx *cli.Context, config *gethConfig) {
	...
	config.Node.IPCPath = utils.MakeDataDir(ctx) + "/gilt.ipc"
	config.Node.HTTPModules = []string{"eth", "net", "web3", "txpool", "gilt"}
...
// nolint : wsl
func setDefaultGiltTestnetGethConfig(ctx *cli.Context, config *gethConfig) {
	config.Node.P2P.ListenAddr = fmt.Sprintf(":%d", 30303)
	config.Node.HTTPHost = "0.0.0.0"
	config.Node.HTTPVirtualHosts = []string{"*"}
	config.Node.HTTPCors = []string{"*"}
...
func setDefaultGiltMainnetGethConfig(ctx *cli.Context, config *gethConfig) {
	config.Node.P2P.ListenAddr = fmt.Sprintf(":%d", 30303)
	config.Node.HTTPHost = "0.0.0.0"
	config.Node.HTTPVirtualHosts = []string{"*"}
	config.Node.HTTPCors = []string{"*"}
