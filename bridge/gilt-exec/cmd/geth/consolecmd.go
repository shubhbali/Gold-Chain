...
func remoteConsole(ctx *cli.Context) error {
	...
	if endpoint == "" {
		...
		if path != "" {
			if ctx.Bool(utils.GiltTestnetLegacyFlag.Name) || ctx.Bool(utils.GiltTestnetFlag.Name) || ctx.Bool(utils.GiltMainnetFlag.Name) {
				homeDir, _ := os.UserHomeDir()
				giltPath := filepath.Join(homeDir, "/.gilt/data")
				if _, err := os.Stat(giltPath); err == nil {
					path = giltPath
				} else {
		...
		endpoint = fmt.Sprintf("%s/gilt.ipc", path)
