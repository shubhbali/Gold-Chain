...
const (
	clientIdentifier     = "gilt" // Client identifier to advertise over the network
	repositoryIdentifier = "gilt-exec"
)
...
var (
	...
	nodeFlags = slices.Concat([]cli.Flag{
		...
		utils.GiltTestnetLegacyFlag,
		utils.GiltTestnetFlag,
		utils.GiltMainnetFlag,
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.GiltTestnetLegacyFlag.Name) || ctx.IsSet(utils.GiltTestnetFlag.Name):
		...
		log.Info("Starting Gilt Exec on Gilt testnet...")

	case ctx.IsSet(utils.GiltMainnetFlag.Name):
		log.Info("Starting Gilt Exec on Gilt mainnet...")
	...
	if !ctx.IsSet(utils.CacheFlag.Name) && !ctx.IsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.IsSet(utils.SepoliaFlag.Name) &&
			...
			!ctx.IsSet(utils.GiltTestnetFlag.Name) &&
			!ctx.IsSet(utils.GiltTestnetLegacyFlag.Name) &&
