...
- ```gilt.giltconsensusgRPC```: Address of GiltConsensus gRPC service (comma-separated for failover: "addr1,addr2")
...
- ```disable-gilt-wallet```: Disable the personal wallet endpoints (default: true)
...
- ```http.api```: API's offered over the HTTP-RPC interface (default: eth,net,web3,txpool,gilt)
...
- ```rpc.rangelimit```: Maximum block range allowed for eth_getLogs and gilt_getLogs (0 = no limit) (default: 0)
