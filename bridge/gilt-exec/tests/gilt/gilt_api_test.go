...
func testGetTransactionReceiptsByBlock(t *testing.T, publicBlockchainAPI *ethapi.BlockChainAPI) {
	...
	txHash := types.GetDerivedGiltTxHash(types.GiltReceiptKey(4, blockHash))
...
func TestAPIs(t *testing.T) {
	// Use the established pattern from other Gilt tests
	updateGenesis := func(gen *core.Genesis) {
		...
		gen.Config.Gilt.Period = map[string]uint64{
			"0": 1,
		}
		gen.Config.Gilt.BurntContract = map[string]string{"0": "0x000000000000000000000000000000000000dead"}
	}
	...
	testGiltConfig := init.genesis.Config.Gilt
	...
	for i, block := range chain {
		...
		if i%int(testGiltConfig.CalculateSprint(block.NumberU64())-1) != 0 {
			...
			if len(receipts[i]) > 0 {
				...
				types.DeriveFieldsForGiltLogs(receipts[i][1].Logs, block.Hash(), block.NumberU64(), uint(1), uint(1))
