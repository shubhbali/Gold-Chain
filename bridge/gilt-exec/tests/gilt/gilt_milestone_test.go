...
func TestRewinding(t *testing.T) {
	...
	for {
		...
		if blockHeaderVal0.Number.Uint64() == 15 && !step1 {
			_, _ = giltVerify(nodes[0], 8, 15, "LocalHash", "RootHash", 15, 7)
			step1 = true
		}
		...
		if blockHeaderVal0.Number.Uint64() == 270 && !step2 {
			_, _ = giltVerify(nodes[0], 8, 15, "LocalHash", "RootHash", 270, 7)
			step2 = true
		}
...
func giltVerify(eth *eth.Ethereum, start uint64, end uint64, rootHash string, localHash string, head uint64, lastMilestone uint64) (string, error) {
	//nolint
