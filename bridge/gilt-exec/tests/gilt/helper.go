...
func buildEthereumInstance(t *testing.T, db ethdb.Database, updateGenesis ...func(gen *core.Genesis)) *initializeData {
	...
	ethereum := utils.CreateGiltEthereum(ethConf)
	if err != nil {
	...
	ethereum.Engine().(*gilt.Gilt).Authorize(addr, func(account accounts.Account, s string, data []byte) ([]byte, error) {
...
func buildHeader(t *testing.T, chain *core.BlockChain, parentBlock *types.Block, signer []byte, giltConfig *params.GiltConfig, currentValidators []*valset.Validator, modifyHeader []modifyHeaderFunc) *types.Header {
	t.Helper()
	...
	// Similar to the logic in gilt consensus
	header.Time = parentBlock.Time() + gilt.CalcProducerDelay(header.Number.Uint64(), 0, giltConfig)
	// Keeping this causes some e2e tests to fail because they work under certain time assumptions
	// if header.Time < uint64(time.Now().Unix()) {
	// 	header.Time = uint64(time.Now().Unix())
	// }

	// Similar to logic in gilt consensus (prepare)
	...
	if (number+1)%chain.Config().Gilt.CalculateSprint(number) == 0 {
		isSprintEnd = true
	}
...
func buildNextBlock(t *testing.T, _gilt consensus.Engine, chain *core.BlockChain, parentBlock *types.Block, signer []byte, giltConfig *params.GiltConfig, txs []*types.Transaction, currentValidators []*valset.Validator, skipSealing bool, modifyHeader []modifyHeaderFunc, modifyBody []modifyBlockFunc) *types.Block {
	t.Helper()

	// Build a new header based on parent block
	header := buildHeader(t, chain, parentBlock, signer, giltConfig, currentValidators, modifyHeader)
	...
	block, b.receipts, _, err = _gilt.FinalizeAndAssemble(chain, b.header, state, &types.Body{
		Transactions: b.txs,
	}, b.receipts)
	if err != nil {
...
func sign(t *testing.T, header *types.Header, signer []byte, c *params.GiltConfig) {
	t.Helper()

	sig, err := secp256k1.Sign(crypto.Keccak256(gilt.GiltRLP(header, c)), signer)
	if err != nil {
		t.Fatalf("%s", err)
	}
...
//nolint:unused
func loadSpanFromFile(t *testing.T) *giltTypes.Span {
	t.Helper()
...
func getMockedGiltConsensusClient(t *testing.T, giltconsensusSpan *giltTypes.Span) (*mocks.MockIGiltConsensusClient, *gomock.Controller) {
	t.Helper()

	ctrl := gomock.NewController(t)
	h := mocks.NewMockIGiltConsensusClient(ctrl)

	h.EXPECT().GetSpan(gomock.Any(), uint64(1)).Return(giltconsensusSpan, nil).AnyTimes()
...
func createMockSpan(address common.Address, chainId string) giltTypes.Span {
...
func createMockGiltConsensus(ctrl *gomock.Controller, span0, span1 *giltTypes.Span) *mocks.MockIGiltConsensusClient {
	h := mocks.NewMockIGiltConsensusClient(ctrl)
...
func InitGenesis(t *testing.T, faucets []*ecdsa.PrivateKey, fileLocation string, sprintSize uint64) *core.Genesis {
	...
	genesis.Config.Gilt.Sprint["0"] = sprintSize
...
// This is the preferred way to test with mock GiltConsensus clients as it avoids caching issues.
func InitMinerWithGiltConsensus(genesis *core.Genesis, privKey *ecdsa.PrivateKey, giltconsensusClient gilt.IGiltConsensusClient) (*node.Node, *eth.Ethereum, error) {
	return InitMinerWithOptions(genesis, privKey, false, 0, giltconsensusClient)
}
