...
import (
	...
	giltMilestone "github.com/ethereum/go-ethereum/consensus/gilt/consensusclient/milestone"
...
func TestForkWithBlockTime(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			genesis := InitGenesis(t, faucets, "./testdata/genesis_2val.json", 8)
			genesis.Config.Gilt.Sprint = test.sprint
			genesis.Config.Gilt.Period = test.blockTime
			genesis.Config.Gilt.BackupMultiplier = test.blockTime
			genesis.Config.Gilt.ProducerDelay = test.producerDelay
...
func TestInsertingSpanSizeBlocks(t *testing.T) {
	...
	updateGenesis := func(gen *core.Genesis) {
		gen.Config.Gilt.StateSyncConfirmationDelay = map[string]uint64{"0": 128}
		gen.Config.Gilt.Sprint = map[string]uint64{"0": sprintSize}
	}
	init := buildEthereumInstance(t, rawdb.NewMemoryDatabase(), updateGenesis)
	chain := init.ethereum.BlockChain()
	engine := init.ethereum.Engine()
	_gilt := engine.(*gilt.Gilt)
	defer _gilt.Close()

	span0 := createMockSpan(addr, chain.Config().ChainID.String())
	res := loadSpanFromFile(t)

	// Create mock giltconsensus client
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	h := createMockGiltConsensus(ctrl, &span0, res)
	...
	_gilt.SetGiltConsensusClient(h)

	block := init.genesis.ToBlock()

	spanner := getMockedSpanner(t, giltSpan.ConvertGiltConsensusValSetToGiltValSet(span0.ValidatorSet).Validators)
	_gilt.SetSpanner(spanner)

	// Insert sprintSize # of blocks so that span is fetched at the start of a new sprint.
	for i := uint64(1); i <= spanSize; i++ {
		block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValSetToGiltValSet(span0.ValidatorSet).Validators, false, nil, nil)
		insertNewBlock(t, chain, block)
	}

	giltValSet := giltSpan.ConvertGiltConsensusValSetToGiltValSet(res.ValidatorSet)

	spanner = getMockedSpanner(t, giltValSet.Validators)
	_gilt.SetSpanner(spanner)

	// Check validator set at the first block of a new span.
	validators, err := _gilt.GetCurrentValidators(context.Background(), block.Hash(), spanSize)
	if err != nil {
		t.Fatalf("%s", err)
	}
	...
	for i, validator := range validators {
		require.Equal(t, validator.Address.Bytes(), giltValSet.Validators[i].Address.Bytes())
		require.Equal(t, validator.VotingPower, giltValSet.Validators[i].VotingPower)
	}
}
...
func TestFetchStateSyncEvents_PreMadhugiriHF(t *testing.T) {
	...
	updateGenesis := func(gen *core.Genesis) {
		gen.Config.Gilt.StateSyncConfirmationDelay = map[string]uint64{"0": uint64(stateSyncConfirmationDelay)}
		gen.Config.Gilt.Sprint = map[string]uint64{"0": sprintSize}
	}
	init := buildEthereumInstance(t, rawdb.NewMemoryDatabase(), updateGenesis)
	chain := init.ethereum.BlockChain()
	...
	span0 := createMockSpan(addr, chain.Config().ChainID.String())
	giltValSet := giltSpan.ConvertGiltConsensusValSetToGiltValSet(span0.ValidatorSet)
	currentValidators := giltValSet.Validators

	// Load mock span 0
	res := loadSpanFromFile(t)

	// reate mock gilt spanner
	...
	for i := uint64(1); i < sprintSize; i++ {
		...
		block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, currentValidators, false, nil, nil)
		insertNewBlock(t, chain, block)
	}

	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltValSet.Validators, false, nil, nil)
	...
	// TODO: Ideally gilt receipts should be present but as all state-sync events fails in this
	// test, no logs are generated and hence empty receipts aren't written. Fix this.

	// Ensure gilt receipts are stored correctly
	// giltReceipt := chain.GetGiltReceiptByHash(block.Hash())
	// t.Log(giltReceipt)
	// require.NotNil(t, giltReceipt, "gilt receipt expected but found nil")
	// require.Equal(t, uint8(0), giltReceipt.Type, "gilt receipt should have type 0")
...
func TestFetchStateSyncEvents_PostMadhugiriHF(t *testing.T) {
	...
	updateGenesis := func(gen *core.Genesis) {
		gen.Config.Gilt.StateSyncConfirmationDelay = map[string]uint64{"0": uint64(stateSyncConfirmationDelay)}
		gen.Config.Gilt.Sprint = map[string]uint64{"0": sprintSize}
		gen.Config.Gilt.MadhugiriBlock = big.NewInt(0) // Enable Madhugiri hardfork from genesis.
	}
	init := buildEthereumInstance(t, rawdb.NewMemoryDatabase(), updateGenesis)
	chain := init.ethereum.BlockChain()
	engine := init.ethereum.Engine()
	...
	// Confirm if gilt receipts are not stored separately
	giltReceipt := chain.GetGiltReceiptByHash(lastBlock.Hash())
	require.Nil(t, giltReceipt, "gilt receipt should not be stored separately")
}
...
func TestFetchStateSyncEvents_2(t *testing.T) {
	...
	lastStateID, _ := _gilt.GenesisContractsClient.LastStateId(nil, sprintSize, block.Hash())
	...
	for i := sprintSize + 1; i <= spanSize; i++ {
		...
		if IsSpanEnd(i) {
			...
			spanner := getMockedSpanner(t, giltSpan.ConvertGiltConsensusValSetToGiltValSet(res.ValidatorSet).Validators)
			_gilt.SetSpanner(spanner)
		...
		block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValSetToGiltValSet(res.ValidatorSet).Validators, false, nil, nil)
		insertNewBlock(t, chain, block)
	}

	lastStateID, _ = _gilt.GenesisContractsClient.LastStateId(nil, spanSize, block.Hash())
...
func TestOutOfTurnSigning(t *testing.T) {
	...
	for i := uint64(1); i < spanSize; i++ {
		...
		if i == chain.Config().Gilt.CalculateSprint(i)-1 {
			currentValidators = res.ValidatorSet.Validators
		...
		block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, []modifyHeaderFunc{setDifficulty}, nil)
		insertNewBlock(t, chain, block)
	}
	...
	block = buildNextBlock(t, _gilt, chain, block, signerKey, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(res.ValidatorSet.Validators), false, []modifyHeaderFunc{setParentTime, setDifficulty}, nil)
	_, err := chain.InsertChain([]*types.Block{block}, false)
	require.Equal(t,
		gilt.BlockTooSoonError{Number: spanSize, Succession: expectedSuccessionNumber},
		*err.(*gilt.BlockTooSoonError))

	expectedDifficulty := uint64(len(res.ValidatorSet.Validators) - expectedSuccessionNumber - turn) // len(validators) - succession
	header := block.Header()

	diff := gilt.CalcProducerDelay(header.Number.Uint64(), expectedSuccessionNumber, init.genesis.Config.Gilt)
	header.Time += diff

	sign(t, header, signerKey, init.genesis.Config.Gilt)
	...
	require.Equal(t,
		gilt.WrongDifficultyError{Number: spanSize, Expected: expectedDifficulty, Actual: 3, Signer: newAddr.Bytes()},
		*err.(*gilt.WrongDifficultyError))
...
func TestSignerNotFound(t *testing.T) {
	...
	_gilt.Authorize(newAddr, func(account accounts.Account, s string, data []byte) ([]byte, error) {
		return crypto.Sign(crypto.Keccak256(data), newKey)
	})

	block = buildNextBlock(t, _gilt, chain, block, signerKey, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(res.ValidatorSet.Validators), false, nil, nil)

	_, err := chain.InsertChain([]*types.Block{block}, false)
	require.Equal(t,
		*err.(*gilt.UnauthorizedSignerError),
		gilt.UnauthorizedSignerError{
			Number:         1,
			Signer:         newAddr.Bytes(),
			AllowedSigners: giltSpan.ConvertGiltConsensusValSetToGiltValSet(span0.ValidatorSet).Validators,
		})
}
...
func TestEIP1559Transition(t *testing.T) {
	...
	var (
		...
		gspec = &core.Genesis{
			Config: params.GiltUnittestChainConfig,
	...
	actual = state.GetBalance(common.HexToAddress(params.GiltUnittestChainConfig.Gilt.CalculateBurntContract(block.NumberU64()))).ToBig()
...
func TestBurnContract(t *testing.T) {
	...
	gspec.Config.BerlinBlock = common.Big0
	gspec.Config.LondonBlock = common.Big0
	gspec.Config.Gilt.BurntContract = map[string]string{
	...
	// check burnt contract balance
	actual = state.GetBalance(common.HexToAddress(gspec.Config.Gilt.CalculateBurntContract(block.NumberU64()))).ToBig()
	expected = new(big.Int).Mul(new(big.Int).SetUint64(block.GasUsed()), block.BaseFee())
	if actual.Cmp(expected) != 0 {
		t.Fatalf("burnt contract balance incorrect: expected %d, got %d", expected, actual)
	}
...
func TestBurnContractContractFetch(t *testing.T) {
	log.SetDefault(log.NewLogger(log.NewTerminalHandlerWithLevel(os.Stderr, log.LevelInfo, true)))
	config := params.GiltUnittestChainConfig
	config.Gilt.BurntContract = map[string]string{
		"10":  "0x000000000000000000000000000000000000aaab",
		"100": "0x000000000000000000000000000000000000aaad",
	}

	burnContractAddr10 := config.Gilt.CalculateBurntContract(10)
	burnContractAddr11 := config.Gilt.CalculateBurntContract(11)
	burnContractAddr99 := config.Gilt.CalculateBurntContract(99)
	burnContractAddr100 := config.Gilt.CalculateBurntContract(100)
	burnContractAddr101 := config.Gilt.CalculateBurntContract(101)
	...
	burnContractAddr100 = config.Gilt.CalculateBurntContract(100)
	burnContractAddr101 = config.Gilt.CalculateBurntContract(101)
	burnContractAddr999 := config.Gilt.CalculateBurntContract(999)
	burnContractAddr1000 := config.Gilt.CalculateBurntContract(1000)
	burnContractAddr1001 := config.Gilt.CalculateBurntContract(1001)
...
func TestJaipurFork(t *testing.T) {
	...
	spanner := getMockedSpanner(t, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(res.ValidatorSet.Validators))
	_gilt.SetSpanner(spanner)
	...
	for i := uint64(1); i < sprintSize; i++ {
		...
		block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, nil, nil)
		insertNewBlock(t, chain, block)

		if block.Number().Uint64() == init.genesis.Config.Gilt.JaipurBlock.Uint64()-1 {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Gilt), gilt.SealHash(block.Header(), init.genesis.Config.Gilt))
		}

		if block.Number().Uint64() == init.genesis.Config.Gilt.JaipurBlock.Uint64() {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Gilt), gilt.SealHash(block.Header(), init.genesis.Config.Gilt))
		}
	}
}
...
func testSealHash(header *types.Header, c *params.GiltConfig) (hash common.Hash) {
...
func testEncodeSigHeader(w io.Writer, header *types.Header, c *params.GiltConfig) {
	enc := []any{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
...
func TestEarlyBlockAnnouncementPostBhilai_Primary(t *testing.T) {
	...
	updateGenesis := func(gen *core.Genesis) {
		...
		gen.Config.Gilt.Period = map[string]uint64{"0": 2}
		gen.Config.Gilt.Sprint = map[string]uint64{"0": 16}
		...
		gen.Config.Gilt.JaipurBlock = common.Big0
		gen.Config.Gilt.DelhiBlock = common.Big0
		gen.Config.Gilt.IndoreBlock = common.Big0
		gen.Config.Gilt.BhilaiBlock = common.Big0
	}
	init := buildEthereumInstance(t, rawdb.NewMemoryDatabase(), updateGenesis)
	...
	spanner := getMockedSpanner(t, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators))
	_gilt.SetSpanner(spanner)
	...
	updateTime := func(header *types.Header) {
		...
		if header.Time < uint64(time.Now().Unix()) {
			...
			if chain.Config().Gilt.IsBhilai(header.Number) && getSuccession() == 0 {
				period := chain.Config().Gilt.CalculatePeriod(header.Number.Uint64())
	...
	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, []modifyHeaderFunc{updateTime}, nil)
	...
	// Capture the expected header time based on the logic used in gilt consensus
	headerTime := block.Time() + gilt.CalcProducerDelay(block.NumberU64(), getSuccession(), init.genesis.Config.Gilt)
	...
	// Block 5
	// Use signer to sign block instead of using `gilt.Seal` call. This is done to immediately
	// build the next block instead of waiting for the delay (using gilt.Seal will not lead
	...
	tempBlock := buildNextBlock(t, _gilt, chain, block, signer, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), true, []modifyHeaderFunc{updateTimeWithoutSleep}, nil)
	...
	setTime := func(header *types.Header) {
		header.Time = block.Time() + gilt.CalcProducerDelay(block.NumberU64(), getSuccession(), init.genesis.Config.Gilt) - 1
	}
	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, []modifyHeaderFunc{setTime}, nil)
	...
	i, err = chain.InsertChain([]*types.Block{block}, false)
	require.Equal(t, gilt.ErrInvalidTimestamp, err, "incorrect error while inserting block #5")
...
func TestEarlyBlockAnnouncementPostBhilai_NonPrimary(t *testing.T) {
	...
	updateGenesis := func(gen *core.Genesis) {
		...
		gen.Config.Gilt.Period = map[string]uint64{"0": 2}
		gen.Config.Gilt.Sprint = map[string]uint64{"0": 16}
		gen.Config.Gilt.ProducerDelay = map[string]uint64{"0": 4}
		gen.Config.Gilt.BackupMultiplier = map[string]uint64{"0": 2}
		gen.Config.Gilt.StateSyncConfirmationDelay = map[string]uint64{"0": 128}
		gen.Config.LondonBlock = common.Big0
		gen.Config.ShanghaiBlock = common.Big0
		gen.Config.CancunBlock = common.Big0
	...
	engine.(*gilt.Gilt).Authorize(addr2, func(account accounts.Account, s string, data []byte) ([]byte, error) {
	...
	h := createMockGiltConsensus(ctrl, res1, res2)
	h.EXPECT().StateSyncEvents(gomock.Any(), gomock.Any(), gomock.Any()).
		Return([]*clerk.EventRecordWithTime{getSampleEventRecord(t)}, nil).AnyTimes()
	_gilt.SetGiltConsensusClient(h)
	...
	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, []modifyHeaderFunc{updateTime, updateDiff}, nil)
	i, err := chain.InsertChain([]*types.Block{block}, false)
	require.NoError(t, err, "error inserting block #1")
	require.Equal(t, 1, i, "incorrect number of blocks inserted while inserting block #1")
	...
	engine.(*gilt.Gilt).Authorize(addr3, func(account accounts.Account, s string, data []byte) ([]byte, error) {
	...
	tempBlock := buildNextBlock(t, _gilt, chain, block, signer, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), true, []modifyHeaderFunc{updateTime, updateHeader}, nil)
	i, err = chain.InsertChain([]*types.Block{tempBlock}, false)
	require.Equal(t, gilt.ErrInvalidTimestamp, err, "incorrect error while inserting block #2")
	...
	updateHeader = func(header *types.Header) {
		...
		header.Time = block.Time() + gilt.CalcProducerDelay(block.NumberU64(), getSuccession(), init.genesis.Config.Gilt)
	}
	tempBlock = buildNextBlock(t, _gilt, chain, block, signer, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), true, []modifyHeaderFunc{updateTime, updateHeader}, nil)
	// Block is invalid according to consensus rules and should return appropriate error
	...
	start := time.Now()
	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), false, []modifyHeaderFunc{updateTime, updateHeader}, nil)
	blockAnnouncementTime := time.Since(start)
	...
	block = buildNextBlock(t, _gilt, chain, block, signer, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), true, []modifyHeaderFunc{updateTime, updateHeader}, nil)
	...
	timeToWait := time.Until(time.Unix(int64(block.Time()+gilt.CalcProducerDelay(block.NumberU64(), getSuccession(), init.genesis.Config.Gilt)), 0))
	block = buildNextBlock(t, _gilt, chain, block, signer, init.genesis.Config.Gilt, nil, giltSpan.ConvertGiltConsensusValidatorsToGiltValidatorsByRef(currentValidators), true, []modifyHeaderFunc{updateTime, updateHeader}, nil)
	...
	err = engine.VerifyHeader(chain, block.Header())
	require.Equal(t,
		gilt.BlockTooSoonError{Number: 4, Succession: 2},
		*err.(*gilt.BlockTooSoonError))
}
...
func TestCustomBlockTimeMining(t *testing.T) {
	...
	genesis.Config.Gilt.Period = map[string]uint64{"0": 1}        // Consensus period: 1s
	genesis.Config.Gilt.Sprint = map[string]uint64{"0": 16}       // Sprint size: 16 blocks
	genesis.Config.Gilt.ProducerDelay = map[string]uint64{"0": 0} // No producer delay
	genesis.Config.Gilt.BackupMultiplier = map[string]uint64{"0": 2}

	genesis.Config.Gilt.RioBlock = common.Big0
	...
	giltEngine := ethBackend.Engine().(*gilt.Gilt)
	giltEngine.Authorize(crypto.PubkeyToAddress(keys[0].PublicKey), func(account accounts.Account, s string, data []byte) ([]byte, error) {
...
func TestInvalidStateSyncInBlockBody(t *testing.T) {
	...
	block = buildNextBlock(t, _gilt, chain, block, nil, init.genesis.Config.Gilt, nil, giltValSet.Validators, false, nil, []modifyBlockFunc{createMaliciousBlock})
	txs := block.Transactions()
	require.Equal(t, 1, len(txs), "state-sync tx should be part of block body")
	require.Equal(t, uint8(types.StateSyncTxType), txs[0].Type(), "transaction should be of state-sync type")

	// Try inserting the malicious block. Due to mismatch in tx data and data from giltconsensus, receipt
...
func TestLateBlockNotEmpty(t *testing.T) {
	...
	genesis := InitGenesis(t, faucets, "./testdata/genesis_2val.json", 16)
	genesis.Config.Gilt.Period = map[string]uint64{"0": 2}
	genesis.Config.Gilt.Sprint = map[string]uint64{"0": 16}
	genesis.Config.Gilt.RioBlock = big.NewInt(0)
	...
	blockPeriod := time.Duration(genesis.Config.Gilt.Period["0"]) * time.Second
...
func TestVerifyPendingHeadersSpanRotationReorg(t *testing.T) {
	...
	genesis := InitGenesis(t, faucets, "./testdata/genesis_2val.json", 16)
	genesis.Config.Gilt.Period = map[string]uint64{"0": 1}
	genesis.Config.Gilt.Sprint = map[string]uint64{"0": 16}
	genesis.Config.Gilt.ProducerDelay = map[string]uint64{"0": 4}
	genesis.Config.Gilt.BackupMultiplier = map[string]uint64{"0": 2}
	genesis.Config.Gilt.StateSyncConfirmationDelay = map[string]uint64{"0": 128}
	...
	for i := 0; i < 2; i++ {
		h := mocks.NewMockIGiltConsensusClient(ctrl)
		...
		stack, ethBackend, err := InitMinerWithGiltConsensus(genesis, keys[i], h)
		if err != nil {
		...
		giltEng := ethBackend.Engine().(*gilt.Gilt)
		...
		nodes = append(nodes, ethBackend)
		enodes = append(enodes, stack.Server().Self())
		giltEngs = append(giltEngs, giltEng)
	}
	...
	log.Info("Simulating span rotation - updating GiltConsensus client to return span 1")

	h2 := mocks.NewMockIGiltConsensusClient(ctrl)
	...
	h2.EXPECT().FetchMilestone(gomock.Any()).Return(&giltMilestone.Milestone{EndBlock: 15}, nil).AnyTimes()
	...
	giltEngs[1].SetGiltConsensusClient(h2)
	...
	giltEngs[1].SetSpanner(spanner2)

	giltEngs[1].PurgeCache()
...
func createSpanWithProducer(producer common.Address, spanId, startBlock, endBlock uint64, chainId string) giltTypes.Span {
...
func getMockedSpannerWithSpanRotation(t *testing.T, validator1, validator2 common.Address, rotationBlock uint64) *gilt.MockSpanner {
	t.Helper()
	...
	span1Mock := &giltTypes.Span{
		Id:                1,
		StartBlock:        rotationBlock,
		EndBlock:          rotationBlock + 255,
		ValidatorSet:      giltSpan.ConvertGiltValSetToGiltConsensusValSet(&valset.ValidatorSet{Validators: []*valset.Validator{&validator2Val}, Proposer: &validator2Val}),
		SelectedProducers: giltSpan.ConvertGiltValidatorsToGiltConsensusValidators([]*valset.Validator{&validator2Val}),
	}
