		...
		if milestoneFlag == 1 {
			...
			if blockHeaderObserver.Number.Uint64() > lastRun {
				for _, nodeTemp := range nodes {
					_, _, err := giltVerifyTemP(nodeTemp, milestoneNum-milestoneLength+1, milestoneNum, milestoneHash.String())
					if err == nil {
...
func giltVerifyTemP(eth *eth.Ethereum, start uint64, end uint64, hash string) (string, uint64, error) {
