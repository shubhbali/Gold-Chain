...
func TestSprintLengths(t *testing.T) {
	...
	assert.Equal(t, testGiltConfig.CalculateSprint(0), uint64(16))
	assert.Equal(t, testGiltConfig.CalculateSprint(8), uint64(4))
	assert.Equal(t, testGiltConfig.CalculateSprint(9), uint64(4))
}

func TestProducerDelay(t *testing.T) {
	testGiltConfig := params.TestChainConfig.Gilt
	testGiltConfig.ProducerDelay = map[string]uint64{
		"0": 16,
		"8": 4,
	}
	assert.Equal(t, testGiltConfig.CalculateProducerDelay(0), uint64(16))
	assert.Equal(t, testGiltConfig.CalculateProducerDelay(8), uint64(4))
	assert.Equal(t, testGiltConfig.CalculateProducerDelay(9), uint64(4))
}
...
func InitMinerSprintLength(genesis *core.Genesis, privKey *ecdsa.PrivateKey, withoutGiltConsensus bool) (*node.Node, *eth.Ethereum, error) {
