...
func TestGiltFilters(t *testing.T) {
	...
	testGiltConfig.Sprint = map[string]uint64{
		"0": 4,
		"8": 2,
	}
	...
	for i, block := range chain {
		...
		if len(receipts[i]) > 0 {
			...
			// State sync logs don't have tx index, tx hash and other necessary fields, DeriveFieldsForGiltLogs will fill those fields for websocket subscriptions
			// DeriveFieldsForGiltLogs argurments:
			// 1. State-sync logs
			// 2. Block Hash
			// 3. Block Number
			...
			types.DeriveFieldsForGiltLogs(receipts[i][0].Logs, block.Hash(), block.NumberU64(), uint(0), uint(0))
		...
		if len(receipts[i]) > 0 {
			...
			storedReceipt := rawdb.ReadGiltReceipt(db, block.Hash(), block.NumberU64(), params.TestChainConfig)
	...
	filter := filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 0, -1, []common.Address{addr}, [][]common.Hash{{hash1, hash2, hash3, hash4, hash5}})
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 900, 999, []common.Address{addr}, [][]common.Hash{{hash3}})
	logs, err = filter.Logs(context.Background())
	if err != nil {
		t.Error("unexpected error:", err)
	}
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 992, -1, []common.Address{addr}, [][]common.Hash{{hash3}})
	logs, err = filter.Logs(context.Background())
	if err != nil {
		t.Error("unexpected error:", err)
	}
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 1, -1, []common.Address{addr}, [][]common.Hash{{hash1, hash2}})
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 0, -1, nil, [][]common.Hash{{failHash}})
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 0, -1, []common.Address{failAddr}, nil)
	...
	filter = filters.NewGiltBlockLogsRangeFilter(backend, testGiltConfig, 0, -1, nil, [][]common.Hash{{failHash}, {hash1}})
