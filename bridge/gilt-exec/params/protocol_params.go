...
const (
	...
	MaxCodeSizePostAhmedabad = 32768           // Maximum bytecode to permit for a contract post Ahmedabad hard fork (gilt / gilt pos) (32KB)
	...
	// GiltDefaultMinerGasPrice defines the minimum gas price to mine a transaction.
	GiltDefaultMinerGasPrice = 25 * GWei

	// GiltDefaultTxPoolPriceLimit defines the minimum gas price limit for gilt to enforce txs acceptance into the pool.
	GiltDefaultTxPoolPriceLimit = 25 * GWei

	// GiltDefaultGpoIgnorePrice defines the minimum gas price below which gilt gpo will ignore transactions.
	GiltDefaultGpoIgnorePrice = 25 * GWei
)
...
// - Post-Delhi: 16
// - Post-Bhilai: 64
// - Post-Lisovo: Configurable via GiltConfig.BaseFeeChangeDenominator (validated, falls back to Bhilai default if invalid)
// If giltConfig is nil, returns the default value of 8.
func BaseFeeChangeDenominator(giltConfig *GiltConfig, number *big.Int) uint64 {
	// Handle cases where gilt consensus isn't available to avoid panic
	if giltConfig == nil {
		return DefaultBaseFeeChangeDenominator
	}
	// If Lisovo is active and custom value is set, validate and use it
	if giltConfig.IsLisovo(number) && giltConfig.BaseFeeChangeDenominator != nil {
		val := *giltConfig.BaseFeeChangeDenominator
		// Validate: must be non-zero to prevent division by zero
		if val > 0 {
			return val
		}
		// Invalid value - log error and fall back to default
		log.Error("Invalid BaseFeeChangeDenominator in GiltConfig (must be > 0), falling back to default",
			"configured", val)
	}

	// Fall back to hard fork based values
	if giltConfig.IsBhilai(number) {
		return BaseFeeChangeDenominatorPostBhilai
	} else if giltConfig.IsDelhi(number) {
