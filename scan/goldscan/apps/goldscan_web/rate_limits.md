...
Rate limits can be defined in a JSON configuration file `apps/goldscan_web/priv/rate_limit_config.json` or passed to `API_RATE_LIMIT_CONFIG_URL` as a URL to the JSON file.
...
- Map with configuration:
  ...
  - `period` - Rate limit time period in [time format](https://docs.goldscan.com/setup/env-variables/backend-env-variables#time-format)
