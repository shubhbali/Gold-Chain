defmodule Indexer.Fetcher.Scroll.Batch do
  ...
  def handle_info(:init_with_delay, _state) do
    ...
    with {:start_block_undefined, false} <- {:start_block_undefined, is_nil(env[:start_block])},
      ...
      {:eip4844_blobs_api_url_undefined, true} ->
        Logger.error(
          "GoldScan Blobs API URL is not defined. Please, check INDEXER_SCROLL_L1_BATCH_GOLDSCAN_BLOBS_API_URL env variable."
        )
  ...
  # - `eip4844_blobs_api_url`: URL of GoldScan Blobs API to get EIP-4844 blobs (for post-Euclid phase).
  ...
  # Gets and parses EIP-4844 blob from the remote GoldScan Blobs API (or directly from Beacon Node as fallback data source).
  #
  # ## Parameters
  # - `eip4844_blobs_api_url`: URL of GoldScan Blobs API.
  ...
  defp get_l2_block_range_post_euclid(eip4844_blobs_api_url, blob_hash, block_timestamp, batch_number, l1_chain_id) do
    case get_blob_data_from_server(eip4844_blobs_api_url, blob_hash, block_timestamp, l1_chain_id) do
      {nil, _} ->
        Logger.warning(
          "Cannot get the blob #{blob_hash} from the GoldScan Blobs API and Beacon Node. L2 block range will be unknown for the batch ##{batch_number}."
        )
  ...
  # Sends an HTTP request to GoldScan Blobs API (or Beacon Node as fallback source) to get EIP-4844 blob data
  ...
  defp get_blob_data_from_server(eip4844_blobs_api_url, blob_hash, block_timestamp, l1_chain_id) do
    with {:ok, response} <- Helper.http_get_request(eip4844_blobs_api_url <> "/" <> blob_hash),
      ...
      {hash_to_binary(blob_data), "GoldScan Blobs API"}
    else
      _ ->
        Logger.warning(
          "Cannot get the blob #{blob_hash} from GoldScan Blobs API. Trying to get that from the Beacon Node..."
        )
  ...
  # - `eip4844_blobs_api_url`: URL of GoldScan Blobs API to get EIP-4844 blobs.
