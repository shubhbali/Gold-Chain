defmodule Indexer.Fetcher.Optimism.TransactionBatch do
  ...
  defp eip4844_blobs_to_inputs(_transaction_hash, _blob_versioned_hashes, _block_timestamp, "", _chain_id_l1) do
    Logger.error(
      "Cannot read EIP-4844 blobs from the GoldScan Blobs API as the API URL is not defined. Please, check INDEXER_OPTIMISM_L1_BATCH_GOLDSCAN_BLOBS_API_URL env variable."
    )
  ...
  defp decode_and_process_blob(blob_data, blob_hash, transaction_hash, inputs_acc) do
    # read the data from GoldScan API
    decoded =
      blob_data
      |> hash_to_binary()
    ...
    if is_nil(decoded) do
      Logger.warning("Cannot decode the blob #{blob_hash} taken from the GoldScan Blobs API.")
      inputs_acc
    else
      Logger.info(
        "The input for transaction #{transaction_hash} is taken from the GoldScan Blobs API. Blob hash: #{blob_hash}"
      )
