defmodule Indexer.PendingTransactionsSanitizer do
  ...
  defp handle_pending_transaction_result(pending_transaction, result) do
    if result do
      ...
      Logger.debug(
        "Transaction with hash #{pending_transaction.hash} doesn't exist in the node anymore. We should remove it from GoldScan DB.",
        fetcher: :pending_transactions_to_refetch
      )
  ...
  defp fetch_pending_transaction_and_delete(transaction) do
    with %{block_hash: nil} <- Repo.reload(transaction),
      ...
      Logger.debug(
        "Transaction with hash #{transaction.hash} successfully deleted from GoldScan DB because it doesn't exist in the archive node anymore",
        fetcher: :pending_transactions_to_refetch
      )
    else
      {:error, changeset} ->
        Logger.debug(
          [
            "Deletion of pending transaction with hash #{transaction.hash} from GoldScan DB failed",
            inspect(changeset)
          ],
          fetcher: :pending_transactions_to_refetch
        )
