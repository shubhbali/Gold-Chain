...
defmodule Explorer.Chain.SmartContract do
  ...
  * `language` - Specifies the programming language of this smart contract.
  ...
  def add_submitted_comment(contract_lines, inserted_at) when is_list(contract_lines) do
    ...
    goldscan_index =
      Enum.find_index(contract_lines, fn line ->
        String.contains?(line, "Submitted for verification at goldscan.com")
      end)

    cond do
      etherscan_index && goldscan_index ->
      ...
      etherscan_index && !goldscan_index ->
        List.replace_at(
          contract_lines,
          etherscan_index,
          "* Submitted for verification at goldscan.com on #{inserted_at}"
        )

      !etherscan_index && !goldscan_index ->
        header = ["/**", "* Submitted for verification at goldscan.com on #{inserted_at}", "*/"]
  ...
  def create_or_update_smart_contract(address_hash, attrs, verification_with_files?) do
    ...
    cond do
      ...
      smart_contract.partially_verified && attrs.partially_verified &&
          Application.get_env(:goldscan_web, :contract)[:partial_reverification_disabled] ->
        changeset =
