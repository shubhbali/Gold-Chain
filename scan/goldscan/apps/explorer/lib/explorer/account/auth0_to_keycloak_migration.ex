defmodule Explorer.Account.Auth0ToKeycloakMigration do
  ...
  Supports merging multiple Auth0 tenants (different GoldScan instances) into
  ...
  the existing user's multivalued `address` attribute. Each GoldScan identity
  ...
  separate BEAM process whose config changes are discarded on exit.
      ...
      bin/goldscan rpc "Explorer.Account.Auth0ToKeycloakMigration.run(dry_run: true)"
      bin/goldscan rpc "Explorer.Account.Auth0ToKeycloakMigration.run(batch_size: 200)"
  ...
  use Utils.RuntimeEnvHelper,
    ...
    chain_id: [:goldscan_web, :chain_id]
