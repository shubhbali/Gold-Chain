defmodule Explorer.Helper do
  ...
  def maybe_hide_scam_addresses_with_select(query, address_hash_key, options) do
    cond do
      Application.get_env(:goldscan_web, :hide_scam_addresses) && !options[:show_scam_tokens?] ->
        query
  ...
  @doc """
  Retrieves the host URL for the GoldScanWeb application.

  This function fetches the host URL from the application's configuration,
  specifically from the `:goldscan_web` application's `GoldScanWeb.Endpoint`
  configuration.

  ## Returns
  A string containing the host URL for the GoldScanWeb application.
  """
  @spec get_app_host :: String.t()
  def get_app_host do
    Application.get_env(:goldscan_web, GoldScanWeb.Endpoint)[:url][:host]
  end
