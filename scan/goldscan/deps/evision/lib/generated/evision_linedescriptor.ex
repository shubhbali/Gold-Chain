defmodule Evision.LineDescriptor do
  ...
  @spec drawKeylines(Keyword.t()) :: any() | {:error, String.t()}
  def drawKeylines([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:flags,:color,:outImage,:keylines])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.line_descriptor_drawKeylines()
    |> to_struct()
  end
  ...
  **Note**: If both *matchColor* and *singleLineColor* are set to their default values, function draws
  ...
  @spec drawLineMatches(Keyword.t()) :: any() | {:error, String.t()}
  def drawLineMatches([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:keylines2,:singleLineColor,:flags,:keylines1,:matchColor,:img1,:img2,:matchesMask,:matches1to2,:outImg])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.line_descriptor_drawLineMatches()
    |> to_struct()
  end
