defmodule Evision.Detail.BlocksGainCompensator do
  ...
  ```
  """
  @spec blocksGainCompensator(Keyword.t()) :: any() | {:error, String.t()}
  def blocksGainCompensator([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:bl_height,:bl_width,:nr_feeds])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_BlocksGainCompensator_BlocksGainCompensator()
    |> to_struct()
  end
  ...
  def apply([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_BlocksGainCompensator_apply()
    |> to_struct()
  end
  ...
  def getMatGains([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_BlocksGainCompensator_getMatGains()
    |> to_struct()
  end
  ...
  def setMatGains([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:umv])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_BlocksGainCompensator_setMatGains()
    |> to_struct()
  end
