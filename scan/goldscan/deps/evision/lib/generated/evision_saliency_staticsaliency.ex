defmodule Evision.Saliency.StaticSaliency do
  ...
  @spec computeBinaryMap(Keyword.t()) :: any() | {:error, String.t()}
  def computeBinaryMap([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_binaryMap,:_saliencyMap])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.saliency_saliency_StaticSaliency_computeBinaryMap()
    |> to_struct()
  end
