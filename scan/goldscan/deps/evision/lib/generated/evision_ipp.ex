defmodule Evision.Ipp do
  ...
  @spec getIppVersion(Keyword.t()) :: any() | {:error, String.t()}
  def getIppVersion([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ipp_getIppVersion()
    |> to_struct()
  end
  ...
  def setUseIPP([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:flag])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ipp_setUseIPP()
    |> to_struct()
  end
  ...
  @spec useIPP(Keyword.t()) :: any() | {:error, String.t()}
  def useIPP([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ipp_useIPP()
    |> to_struct()
  end
