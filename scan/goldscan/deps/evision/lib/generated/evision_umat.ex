defmodule Evision.UMat do
  ...
  def uMat([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:type,:rows,:rowRange,:cols,:m,:usageFlags,:colRange,:ranges,:roi,:size,:s])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.uMat_UMat()
    |> to_struct()
  end
  ...
  def context([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.uMat_context_static()
    |> to_struct()
  end
  ...
  def handle([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:accessFlags])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.uMat_handle()
    |> to_struct()
  end
  ...
  def isSubmatrix([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.uMat_isSubmatrix()
    |> to_struct()
  end
  ...
  def queue([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.uMat_queue_static()
    |> to_struct()
  end
