defmodule Evision.BOWKMeansTrainer do
  ...
  ```
  """
  @spec bowKMeansTrainer(Keyword.t()) :: any() | {:error, String.t()}
  def bowKMeansTrainer([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:flags,:clusterCount,:termcrit,:attempts])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.bowKMeansTrainer_BOWKMeansTrainer()
    |> to_struct()
  end
  ...
  def cluster([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.bowKMeansTrainer_cluster()
    |> to_struct()
  end
