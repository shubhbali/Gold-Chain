defmodule Evision.KinFu.VolumeParams do
  ...
  def coarseParams([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_volumeType])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.kinfu_kinfu_VolumeParams_coarseParams_static()
    |> to_struct()
  end
  ...
  def defaultParams([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_volumeType])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.kinfu_kinfu_VolumeParams_defaultParams_static()
    |> to_struct()
  end
