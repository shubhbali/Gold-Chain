defmodule Evision.Detail.Timelapser do
  ...
  def createDefault([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.detail_detail_Timelapser_createDefault_static()
    |> to_struct()
  end
  ...
  def getDst([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Timelapser_getDst()
    |> to_struct()
  end
  ...
  def initialize([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Timelapser_initialize()
    |> to_struct()
  end
  ...
  def process([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Timelapser_process()
    |> to_struct()
  end
