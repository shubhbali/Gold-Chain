defmodule Evision.DNN.Image2BlobParams do
  ...
  def image2BlobParams([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:datalayout,:mean,:ddepth,:borderValue,:swapRB,:mode,:size,:scalefactor])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Image2BlobParams_Image2BlobParams()
    |> to_struct()
  end
  ...
  @spec blobRectToImageRect(Keyword.t()) :: any() | {:error, String.t()}
  def blobRectToImageRect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:size,:rBlob])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Image2BlobParams_blobRectToImageRect()
    |> to_struct()
  end
  ...
  @spec blobRectsToImageRects(Keyword.t()) :: any() | {:error, String.t()}
  def blobRectsToImageRects([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:size,:rBlob,:rImg])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Image2BlobParams_blobRectsToImageRects()
    |> to_struct()
  end
