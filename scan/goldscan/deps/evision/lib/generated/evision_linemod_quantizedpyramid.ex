defmodule Evision.LineMod.QuantizedPyramid do
  ...
  ```
  """
  @spec extractTemplate(Keyword.t()) :: any() | {:error, String.t()}
  def extractTemplate([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:templ])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.linemod_linemod_QuantizedPyramid_extractTemplate()
    |> to_struct()
  end
  ...
  def pyrDown([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.linemod_linemod_QuantizedPyramid_pyrDown()
    |> to_struct()
  end
  ...
  def quantize([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.linemod_linemod_QuantizedPyramid_quantize()
    |> to_struct()
  end
