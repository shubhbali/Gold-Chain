defmodule Evision.Face.BIF do
  ...
  def compute([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:features,:image])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_BIF_compute()
    |> to_struct()
  end
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:num_rotations,:num_bands])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_BIF_create_static()
    |> to_struct()
  end
  ...
  def getDefaultName([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_BIF_getDefaultName()
    |> to_struct()
  end
  ...
  ```
  """
  @spec getNumBands(Keyword.t()) :: any() | {:error, String.t()}
  def getNumBands([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_BIF_getNumBands()
    |> to_struct()
  end
  ...
  ```
  """
  @spec getNumRotations(Keyword.t()) :: any() | {:error, String.t()}
  def getNumRotations([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_BIF_getNumRotations()
    |> to_struct()
  end
