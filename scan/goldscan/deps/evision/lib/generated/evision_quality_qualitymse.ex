defmodule Evision.Quality.QualityMSE do
  ...
  def clear([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.quality_quality_QualityMSE_clear()
    |> to_struct()
  end
  ...
  def compute([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:cmpImgs,:ref,:qualityMap,:cmp])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityMSE_compute()
    |> to_struct()
  end
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityMSE_create_static()
    |> to_struct()
  end
  ...
  def empty([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityMSE_empty()
    |> to_struct()
  end
