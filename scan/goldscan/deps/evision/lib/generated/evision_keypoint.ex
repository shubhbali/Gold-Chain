defmodule Evision.KeyPoint do
  ...
  @spec keyPoint(Keyword.t()) :: any() | {:error, String.t()}
  def keyPoint([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:octave,:class_id,:y,:response,:size,:x,:angle])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.keyPoint_KeyPoint()
    |> to_struct()
  end
  ...
  def convert([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:octave,:keypointIndexes,:keypoints,:class_id,:response,:size,:points2f])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.keyPoint_convert_static()
    |> to_struct()
  end
  ...
  @spec overlap(Keyword.t()) :: any() | {:error, String.t()}
  def overlap([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:kp1,:kp2])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.keyPoint_overlap_static()
    |> to_struct()
  end
