defmodule Evision.Detail.FeaturesMatcher do
  ...
  def apply([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.detail_detail_FeaturesMatcher_apply()
    |> to_struct()
  end
  ...
  def apply2([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_FeaturesMatcher_apply2()
    |> to_struct()
  end
  ...
  def collectGarbage([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_FeaturesMatcher_collectGarbage()
    |> to_struct()
  end
  ...
  def isThreadSafe([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_FeaturesMatcher_isThreadSafe()
    |> to_struct()
  end
