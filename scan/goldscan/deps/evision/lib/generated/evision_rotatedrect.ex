defmodule Evision.RotatedRect do
  ...
  @spec rotatedRect(Keyword.t()) :: any() | {:error, String.t()}
  def rotatedRect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:center,:point1,:point2,:point3,:size,:angle])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.rotatedRect_RotatedRect()
    |> to_struct()
  end
  ...
  def boundingRect([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.rotatedRect_boundingRect()
    |> to_struct()
  end
  ...
  def boundingRect2f([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.rotatedRect_boundingRect2f()
    |> to_struct()
  end
  ...
  **Note**: _Bottom_, _Top_, _Left_ and _Right_ sides refer to the original rectangle (angle is 0),
  ...
  def points([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:pts])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.rotatedRect_points()
    |> to_struct()
  end
