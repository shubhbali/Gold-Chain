defmodule Evision.Detail.HomographyBasedEstimator do
  ...
  def homographyBasedEstimator([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:is_focals_estimated])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_HomographyBasedEstimator_HomographyBasedEstimator()
    |> to_struct()
  end
