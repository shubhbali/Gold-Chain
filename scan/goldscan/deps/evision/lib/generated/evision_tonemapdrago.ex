defmodule Evision.TonemapDrago do
  ...
  ```
  """
  @spec getBias(Keyword.t()) :: any() | {:error, String.t()}
  def getBias([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapDrago_getBias()
    |> to_struct()
  end
  ...
  def getSaturation([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapDrago_getSaturation()
    |> to_struct()
  end
  ...
  @spec setBias(Keyword.t()) :: any() | {:error, String.t()}
  def setBias([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:bias])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapDrago_setBias()
    |> to_struct()
  end
  ...
  def setSaturation([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapDrago_setSaturation()
    |> to_struct()
  end
