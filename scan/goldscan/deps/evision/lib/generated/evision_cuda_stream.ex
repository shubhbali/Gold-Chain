defmodule Evision.CUDA.Stream do
  ...
  **Note**: The cudaFlags parameter is passed to the underlying api cudaStreamCreateWithFlags() and
  ...
  def stream([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:allocator,:cudaFlags])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_Stream()
    |> to_struct()
  end
  ...
  def null([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_Null_static()
    |> to_struct()
  end
  ...
  **Note**: Callbacks must not make any CUDA API calls. Callbacks must not perform any synchronization
  ...
  def cudaPtr([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_cudaPtr()
    |> to_struct()
  end
  ...
  def queryIfComplete([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_queryIfComplete()
    |> to_struct()
  end
  ...
  def waitEvent([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:event])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_waitEvent()
    |> to_struct()
  end
  ...
  def waitForCompletion([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Stream_waitForCompletion()
    |> to_struct()
  end
