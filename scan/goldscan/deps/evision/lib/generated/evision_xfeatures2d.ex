defmodule Evision.XFeatures2D do
  ...
  **Note**: 
  ...
  @spec matchGMS(Keyword.t()) :: any() | {:error, String.t()}
  def matchGMS([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:thresholdFactor,:size2,:size1,:withScale,:matchesGMS,:matches1to2,:keypoints2,:keypoints1,:withRotation])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.xfeatures2d_matchGMS()
    |> to_struct()
  end
  ...
  **Note**: 
  ...
  @spec matchLOGOS(Keyword.t()) :: any() | {:error, String.t()}
  def matchLOGOS([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:matches1to2,:nn1,:keypoints2,:nn2,:keypoints1])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.xfeatures2d_matchLOGOS()
    |> to_struct()
  end
