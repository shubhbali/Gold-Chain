defmodule Evision.CUDA.SURFCUDA do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_extended,:_nOctaveLayers,:_hessianThreshold,:_nOctaves,:_upright,:_keypointsRatio])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_create_static()
    |> to_struct()
  end
  ...
  def defaultNorm([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_defaultNorm()
    |> to_struct()
  end
  ...
  def descriptorSize([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_descriptorSize()
    |> to_struct()
  end
  ...
  def detect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:keypoints,:img,:mask])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_detect()
    |> to_struct()
  end
  ...
  @spec detectWithDescriptors(Keyword.t()) :: any() | {:error, String.t()}
  def detectWithDescriptors([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:img,:keypoints,:mask,:descriptors,:useProvidedKeypoints])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_detectWithDescriptors()
    |> to_struct()
  end
  ...
  @spec downloadKeypoints(Keyword.t()) :: any() | {:error, String.t()}
  def downloadKeypoints([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:keypoints,:keypointsGPU])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_SURF_CUDA_downloadKeypoints()
    |> to_struct()
  end
