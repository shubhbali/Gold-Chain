defmodule Evision.CUDA.Event do
  ...
  ```
  """
  @spec event(Keyword.t()) :: any() | {:error, String.t()}
  def event([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:flags])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Event_Event()
    |> to_struct()
  end
  ...
  @spec elapsedTime(Keyword.t()) :: any() | {:error, String.t()}
  def elapsedTime([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:end,:start])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Event_elapsedTime_static()
    |> to_struct()
  end
  ...
  def queryIfComplete([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Event_queryIfComplete()
    |> to_struct()
  end
  ...
  ```
  """
  @spec record(Keyword.t()) :: any() | {:error, String.t()}
  def record([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:stream])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Event_record()
    |> to_struct()
  end
  ...
  def waitForCompletion([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_Event_waitForCompletion()
    |> to_struct()
  end
