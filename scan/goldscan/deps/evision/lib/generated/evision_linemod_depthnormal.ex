defmodule Evision.LineMod.DepthNormal do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:distance_threshold,:num_features,:difference_threshold,:extract_threshold])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.linemod_linemod_DepthNormal_create_static()
    |> to_struct()
  end
