defmodule Evision.Signal do
  ...
  @spec resampleSignal(Keyword.t()) :: any() | {:error, String.t()}
  def resampleSignal([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:inputSignal,:outFreq,:outSignal,:inFreq])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.signal_resampleSignal()
    |> to_struct()
  end
