defmodule Evision.CUDA.BufferPool do
  ...
  def bufferPool([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:stream])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_BufferPool_BufferPool()
    |> to_struct()
  end
  ...
  def getAllocator([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_BufferPool_getAllocator()
    |> to_struct()
  end
  ...
  def getBuffer([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:type,:rows,:size,:cols])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.cuda_cuda_BufferPool_getBuffer()
    |> to_struct()
  end
