defmodule Evision.AsyncArray do
  ...
  @spec asyncArray(Keyword.t()) :: any() | {:error, String.t()}
  def asyncArray([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.asyncArray_AsyncArray()
    |> to_struct()
  end
  ...
  **Note**: Result or stored exception can be fetched only once.
  ...
  def get([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:timeoutNs,:dst])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.asyncArray_get()
    |> to_struct()
  end
  ...
  **Note**: Result or stored exception can be fetched only once.
  ...
  def release([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.asyncArray_release()
    |> to_struct()
  end
  ...
  def valid([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.asyncArray_valid()
    |> to_struct()
  end
  ...
  @spec wait_for(Keyword.t()) :: any() | {:error, String.t()}
  def wait_for([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:timeoutNs])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.asyncArray_wait_for()
    |> to_struct()
  end
