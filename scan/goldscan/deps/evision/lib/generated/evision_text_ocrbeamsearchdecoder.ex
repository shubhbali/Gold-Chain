defmodule Evision.Text.OCRBeamSearchDecoder do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:transition_probabilities_table,:classifier,:vocabulary,:mode,:emission_probabilities_table,:beam_size])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_OCRBeamSearchDecoder_create_static()
    |> to_struct()
  end
  ...
  def run([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_OCRBeamSearchDecoder_run()
    |> to_struct()
  end
