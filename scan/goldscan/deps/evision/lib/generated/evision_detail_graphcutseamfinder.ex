defmodule Evision.Detail.GraphCutSeamFinder do
  ...
  def graphCutSeamFinder([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:terminal_cost,:cost_type,:bad_region_penalty])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_GraphCutSeamFinder_GraphCutSeamFinder()
    |> to_struct()
  end
  ...
  def find([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:corners,:src,:masks])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_GraphCutSeamFinder_find()
    |> to_struct()
  end
