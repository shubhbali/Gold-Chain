defmodule Evision.Samples do
  ...
  def addSamplesDataSearchPath([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:path])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.samples_addSamplesDataSearchPath()
    |> to_struct()
  end
  ...
  def addSamplesDataSearchSubDirectory([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:subdir])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.samples_addSamplesDataSearchSubDirectory()
    |> to_struct()
  end
  ...
  def findFile([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:silentMode,:relative_path,:required])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.samples_findFile()
    |> to_struct()
  end
  ...
  def findFileOrKeep([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:silentMode,:relative_path])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.samples_findFileOrKeep()
    |> to_struct()
  end
