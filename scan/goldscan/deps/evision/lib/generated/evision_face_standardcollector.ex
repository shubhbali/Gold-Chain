defmodule Evision.Face.StandardCollector do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.face_face_StandardCollector_create_static()
    |> to_struct()
  end
  ...
  def getMinDist([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_StandardCollector_getMinDist()
    |> to_struct()
  end
  ...
  def getMinLabel([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_StandardCollector_getMinLabel()
    |> to_struct()
  end
  ...
  def getResults([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:sorted])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.face_face_StandardCollector_getResults()
    |> to_struct()
  end
