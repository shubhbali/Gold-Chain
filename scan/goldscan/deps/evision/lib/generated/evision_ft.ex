defmodule Evision.Ft do
  ...
  def createKernel([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:function,:chn,:radius,:kernel])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ft_createKernel()
    |> to_struct()
  end
  ...
  def createKernel1([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:chn,:B,:A,:kernel])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ft_createKernel1()
    |> to_struct()
  end
  ...
  def filter([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:output,:image,:kernel])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ft_filter()
    |> to_struct()
  end
  ...
  **Note**: 
  ...
  def inpaint([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:mask,:output,:algorithm,:radius,:function])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ft_inpaint()
    |> to_struct()
  end
