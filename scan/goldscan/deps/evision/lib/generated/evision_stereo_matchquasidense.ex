defmodule Evision.Stereo.MatchQuasiDense do
  ...
  def matchQuasiDense([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.stereo_stereo_MatchQuasiDense_MatchQuasiDense()
    |> to_struct()
  end
  ...
  def apply([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:rhs])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.stereo_stereo_MatchQuasiDense_apply()
    |> to_struct()
  end
