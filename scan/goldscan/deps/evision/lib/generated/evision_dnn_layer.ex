defmodule Evision.DNN.Layer do
  ...
  @spec finalize(Keyword.t()) :: any() | {:error, String.t()}
  def finalize([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:inputs,:outputs])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Layer_finalize()
    |> to_struct()
  end
  ...
  def getDefaultName([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_Layer_getDefaultName()
    |> to_struct()
  end
  ...
  @spec outputNameToIndex(Keyword.t()) :: any() | {:error, String.t()}
  def outputNameToIndex([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:outputName])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Layer_outputNameToIndex()
    |> to_struct()
  end
  ...
  def run([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:inputs,:outputs,:internals])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.dnn_dnn_Layer_run()
    |> to_struct()
  end
