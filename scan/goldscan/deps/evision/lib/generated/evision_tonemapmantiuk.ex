defmodule Evision.TonemapMantiuk do
  ...
  def getSaturation([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.tonemapMantiuk_getSaturation()
    |> to_struct()
  end
  ...
  def getScale([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapMantiuk_getScale()
    |> to_struct()
  end
  ...
  def setSaturation([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapMantiuk_setSaturation()
    |> to_struct()
  end
  ...
  def setScale([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.tonemapMantiuk_setScale()
    |> to_struct()
  end
