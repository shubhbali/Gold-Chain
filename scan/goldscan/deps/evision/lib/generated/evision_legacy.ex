defmodule Evision.Legacy do
  ...
  ```
  """
  @spec upgradeTrackingAPI(Keyword.t()) :: any() | {:error, String.t()}
  def upgradeTrackingAPI([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:legacy_tracker])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.legacy_upgradeTrackingAPI()
    |> to_struct()
  end
