defmodule Evision.Reg.MapperGradSimilar do
  ...
  @spec mapperGradSimilar(Keyword.t()) :: any() | {:error, String.t()}
  def mapperGradSimilar([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.reg_reg_MapperGradSimilar_MapperGradSimilar()
    |> to_struct()
  end
  ...
  def calculate([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.reg_reg_MapperGradSimilar_calculate()
    |> to_struct()
  end
  ...
  def getMap([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.reg_reg_MapperGradSimilar_getMap()
    |> to_struct()
  end
