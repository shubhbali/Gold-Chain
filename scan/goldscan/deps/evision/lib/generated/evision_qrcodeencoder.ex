defmodule Evision.QRCodeEncoder do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.qrCodeEncoder_create_static()
    |> to_struct()
  end
  ...
  def encode([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:qrcode,:encoded_info])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.qrCodeEncoder_encode()
    |> to_struct()
  end
  ...
  @spec encodeStructuredAppend(Keyword.t()) :: any() | {:error, String.t()}
  def encodeStructuredAppend([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:qrcodes,:encoded_info])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.qrCodeEncoder_encodeStructuredAppend()
    |> to_struct()
  end
