defmodule Evision.ArUco.GridBoard do
  ...
  @spec gridBoard(Keyword.t()) :: any() | {:error, String.t()}
  def gridBoard([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:ids,:markerSeparation,:markerLength,:dictionary,:size])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_GridBoard_GridBoard()
    |> to_struct()
  end
  ...
  def getGridSize([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_GridBoard_getGridSize()
    |> to_struct()
  end
  ...
  def getMarkerLength([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_GridBoard_getMarkerLength()
    |> to_struct()
  end
  ...
  def getMarkerSeparation([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_GridBoard_getMarkerSeparation()
    |> to_struct()
  end
