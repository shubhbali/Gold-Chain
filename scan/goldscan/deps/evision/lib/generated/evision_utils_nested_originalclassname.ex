defmodule Evision.Utils.Nested.OriginalClassName do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.utils_nested_utils_nested_OriginalClassName_create_static()
    |> to_struct()
  end
  ...
  ```
  """
  @spec getFloatParam(Keyword.t()) :: any() | {:error, String.t()}
  def getFloatParam([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.utils_nested_utils_nested_OriginalClassName_getFloatParam()
    |> to_struct()
  end
  ...
  ```
  """
  @spec getIntParam(Keyword.t()) :: any() | {:error, String.t()}
  def getIntParam([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.utils_nested_utils_nested_OriginalClassName_getIntParam()
    |> to_struct()
  end
  ...
  @spec originalName(Keyword.t()) :: any() | {:error, String.t()}
  def originalName([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.utils_nested_utils_nested_OriginalClassName_originalName_static()
    |> to_struct()
  end
