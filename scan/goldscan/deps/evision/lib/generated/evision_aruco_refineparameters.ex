defmodule Evision.ArUco.RefineParameters do
  ...
  def refineParameters([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:minRepDistance,:errorCorrectionRate,:checkAllOrders])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_RefineParameters_RefineParameters()
    |> to_struct()
  end
  ...
  def readRefineParameters([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:fn])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_RefineParameters_readRefineParameters()
    |> to_struct()
  end
  ...
  def writeRefineParameters([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:fs,:name])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.aruco_aruco_RefineParameters_writeRefineParameters()
    |> to_struct()
  end
