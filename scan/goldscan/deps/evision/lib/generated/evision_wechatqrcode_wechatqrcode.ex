defmodule Evision.WeChatQRCode.WeChatQRCode do
  ...
  def weChatQRCode([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:super_resolution_caffe_model_path,:super_resolution_prototxt_path,:detector_caffe_model_path,:detector_prototxt_path])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.wechat_qrcode_wechat_qrcode_WeChatQRCode_WeChatQRCode()
    |> to_struct()
  end
  ...
  def detectAndDecode([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.wechat_qrcode_wechat_qrcode_WeChatQRCode_detectAndDecode()
    |> to_struct()
  end
  ...
  def getScaleFactor([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.wechat_qrcode_wechat_qrcode_WeChatQRCode_getScaleFactor()
    |> to_struct()
  end
  ...
  def setScaleFactor([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_scalingFactor])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.wechat_qrcode_wechat_qrcode_WeChatQRCode_setScaleFactor()
    |> to_struct()
  end
