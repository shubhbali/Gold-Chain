defmodule Evision.Text.TextDetectorCNN do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:modelWeightsFilename,:modelArchFilename])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_TextDetectorCNN_create_static()
    |> to_struct()
  end
  ...
  def detect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:confidence,:Bbox,:inputImage])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_TextDetectorCNN_detect()
    |> to_struct()
  end
