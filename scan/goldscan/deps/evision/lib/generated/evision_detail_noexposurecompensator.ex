defmodule Evision.Detail.NoExposureCompensator do
  ...
  def apply([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:arg1,:arg2,:arg4,:arg3])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_NoExposureCompensator_apply()
    |> to_struct()
  end
  ...
  def getMatGains([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_NoExposureCompensator_getMatGains()
    |> to_struct()
  end
  ...
  def setMatGains([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:umv])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_NoExposureCompensator_setMatGains()
    |> to_struct()
  end
