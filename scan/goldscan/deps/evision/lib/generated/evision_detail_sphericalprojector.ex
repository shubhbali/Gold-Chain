defmodule Evision.Detail.SphericalProjector do
  ...
  @spec mapBackward(Keyword.t()) :: any() | {:error, String.t()}
  def mapBackward([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:u,:v,:y,:x])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_SphericalProjector_mapBackward()
    |> to_struct()
  end
  ...
  @spec mapForward(Keyword.t()) :: any() | {:error, String.t()}
  def mapForward([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:u,:y,:v,:x])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_SphericalProjector_mapForward()
    |> to_struct()
  end
