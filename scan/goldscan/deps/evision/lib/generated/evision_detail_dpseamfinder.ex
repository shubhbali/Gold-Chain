defmodule Evision.Detail.DpSeamFinder do
  ...
  def dpSeamFinder([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:costFunc])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_DpSeamFinder_DpSeamFinder()
    |> to_struct()
  end
  ...
  def setCostFunction([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:val])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_DpSeamFinder_setCostFunction()
    |> to_struct()
  end
