defmodule Evision.ThinPlateSplineShapeTransformer do
  ...
  ```
  """
  @spec getRegularizationParameter(Keyword.t()) :: any() | {:error, String.t()}
  def getRegularizationParameter([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.thinPlateSplineShapeTransformer_getRegularizationParameter()
    |> to_struct()
  end
  ...
  @spec setRegularizationParameter(Keyword.t()) :: any() | {:error, String.t()}
  def setRegularizationParameter([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:beta])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.thinPlateSplineShapeTransformer_setRegularizationParameter()
    |> to_struct()
  end
