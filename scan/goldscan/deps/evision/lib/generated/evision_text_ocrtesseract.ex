defmodule Evision.Text.OCRTesseract do
  ...
  **Note**: The char_whitelist default is changed after OpenCV 4.7.0/3.19.0 from "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" to "".
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:psmode,:char_whitelist,:datapath,:language,:oem])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_OCRTesseract_create_static()
    |> to_struct()
  end
  ...
  **Note**: The char_whitelist default is changed after OpenCV 4.7.0/3.19.0 from "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" to "".
  ...
  def run([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:component_level,:min_confidence,:mask,:image])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_OCRTesseract_run()
    |> to_struct()
  end
  ...
  @spec setWhiteList(Keyword.t()) :: any() | {:error, String.t()}
  def setWhiteList([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:char_whitelist])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.text_text_OCRTesseract_setWhiteList()
    |> to_struct()
  end
