defmodule Evision.BOWTrainer do
  ...
  ```
  """
  @spec cluster(Keyword.t()) :: any() | {:error, String.t()}
  def cluster([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:descriptors])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.bowTrainer_cluster()
    |> to_struct()
  end
  ...
  ```
  """
  @spec descriptorsCount(Keyword.t()) :: any() | {:error, String.t()}
  def descriptorsCount([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.bowTrainer_descriptorsCount()
    |> to_struct()
  end
  ...
  ```
  """
  @spec getDescriptors(Keyword.t()) :: any() | {:error, String.t()}
  def getDescriptors([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.bowTrainer_getDescriptors()
    |> to_struct()
  end
