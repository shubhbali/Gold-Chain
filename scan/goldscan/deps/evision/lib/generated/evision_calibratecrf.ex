defmodule Evision.CalibrateCRF do
  ...
  def clear([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.calibrateCRF_clear()
    |> to_struct()
  end
  ...
  def empty([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.calibrateCRF_empty()
    |> to_struct()
  end
  ...
  def getDefaultName([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.calibrateCRF_getDefaultName()
    |> to_struct()
  end
  ...
  def process([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.calibrateCRF_process()
    |> to_struct()
  end
  ...
  def write([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.calibrateCRF_write()
    |> to_struct()
  end
