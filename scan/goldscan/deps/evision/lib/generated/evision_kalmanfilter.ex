defmodule Evision.KalmanFilter do
  ...
  @spec kalmanFilter(Keyword.t()) :: any() | {:error, String.t()}
  def kalmanFilter([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:type,:dynamParams,:controlParams,:measureParams])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.kalmanFilter_KalmanFilter()
    |> to_struct()
  end
  ...
  @spec correct(Keyword.t()) :: any() | {:error, String.t()}
  def correct([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:measurement])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.kalmanFilter_correct()
    |> to_struct()
  end
  ...
  def predict([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:control])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.kalmanFilter_predict()
    |> to_struct()
  end
