defmodule Evision.MCC.CCheckerDraw do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:thickness,:pChecker,:color])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.mcc_mcc_CCheckerDraw_create_static()
    |> to_struct()
  end
  ...
  def draw([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:img])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.mcc_mcc_CCheckerDraw_draw()
    |> to_struct()
  end
