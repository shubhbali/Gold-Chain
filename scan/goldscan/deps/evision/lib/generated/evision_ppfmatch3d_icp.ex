defmodule Evision.PPFMatch3D.ICP do
  ...
  def iCP([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:sampleType,:tolerence,:iterations,:rejectionScale,:numLevels,:numMaxCorr])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ppf_match_3d_ppf_match_3d_ICP_ICP()
    |> to_struct()
  end
  ...
  @spec registerModelToScene(Keyword.t()) :: any() | {:error, String.t()}
  def registerModelToScene([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:poses,:residual,:pose,:dstPC,:srcPC])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.ppf_match_3d_ppf_match_3d_ICP_registerModelToScene()
    |> to_struct()
  end
