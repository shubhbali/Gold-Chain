defmodule Evision.Detail.Blender do
  ...
  def blend([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    |> :evision_nif.detail_detail_Blender_blend()
    |> to_struct()
  end
  ...
  def createDefault([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:type,:try_gpu])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Blender_createDefault_static()
    |> to_struct()
  end
  ...
  def feed([{arg, _} | _] = named_args) when is_atom(arg) do
    ...
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Blender_feed()
    |> to_struct()
  end
  ...
  def prepare([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:sizes,:dst_roi,:corners])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.detail_detail_Blender_prepare()
    |> to_struct()
  end
