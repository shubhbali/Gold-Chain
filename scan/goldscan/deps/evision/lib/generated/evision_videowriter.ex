defmodule Evision.VideoWriter do
  ...
  @spec videoWriter(Keyword.t()) :: any() | {:error, String.t()}
  def videoWriter([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:fourcc,:params,:fps,:filename,:isColor,:frameSize,:apiPreference])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.videoWriter_VideoWriter()
    |> to_struct()
  end
  ...
  @spec fourcc(Keyword.t()) :: any() | {:error, String.t()}
  def fourcc([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:c1,:c4,:c3,:c2])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.videoWriter_fourcc_static()
    |> to_struct()
  end
  ...
  **Note**: Stream should be opened.
  ...
  def getBackendName([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.videoWriter_getBackendName()
    |> to_struct()
  end
  ...
  def isOpened([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.videoWriter_isOpened()
    |> to_struct()
  end
  ...
  def release([{arg, _} | _] = named_args) when is_atom(arg) do
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.videoWriter_release()
    |> to_struct()
  end
