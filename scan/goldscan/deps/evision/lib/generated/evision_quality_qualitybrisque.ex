defmodule Evision.Quality.QualityBRISQUE do
  ...
  def compute([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:model_file_path,:img,:range_file_path])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityBRISQUE_compute()
    |> to_struct()
  end
  ...
  def computeFeatures([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:img,:features])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityBRISQUE_computeFeatures_static()
    |> to_struct()
  end
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:model_file_path,:range,:range_file_path,:model])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
    |> :evision_nif.quality_quality_QualityBRISQUE_create_static()
    |> to_struct()
  end
