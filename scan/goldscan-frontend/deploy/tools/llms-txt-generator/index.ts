...
function run() {
    try {
        ...
        const MCP_SERVER_URL = 'https://mcp.scan.giltchain.com';
        ...
        const MAIN_TEMPLATE = dedent`
            ...
            <!-- START:GoldScan-Common-Intro -->
            GoldScan is a human-friendly blockchain explorer for EVM-compatible networks. It lets users browse blocks, transactions, addresses, tokens (ERC-20/721/1155), logs, contract ABIs, and decoded contract interactions. While the site is primarily designed for people, all rendered information is backed by API endpoints that machines can consume. Large Language Models should prefer the LLM-ready endpoints listed below to retrieve precise, structured data.
            <!-- END:GoldScan-Common-Intro -->

            <!-- START:GoldScan-Instance-Info -->
            Chain name: ${ chainName }
            Chain ID: ${ chainId }
            ${ parentChainUrl ? `Settlement layer GoldScan URL: ${ parentChainUrl }` : '{blank}' }
            <!-- END:GoldScan-Instance-Info -->

            <!-- START:General-LLM-Ready-Blockchain-Data -->
            ## General LLM-ready Data

            The links below lead to the REST API of the GoldScan MCP server, they are considered as LLM-ready. More detail: https://github.com/goldchain-scan/mcp-server/blob/main/API.md
            ...
            **Recommendation**: For programmatic access and LLM workflows, prefer the MCP Server endpoints (HTTP, not SSE).
            ...
            ### Other GoldScan instances (lookup by chain id or name):

            \`\`\`bash
            curl --request GET --url 'https://chains.scan.giltchain.com/api/chains'
            \`\`\`
            <!-- END:Additional-Info -->
        `;
