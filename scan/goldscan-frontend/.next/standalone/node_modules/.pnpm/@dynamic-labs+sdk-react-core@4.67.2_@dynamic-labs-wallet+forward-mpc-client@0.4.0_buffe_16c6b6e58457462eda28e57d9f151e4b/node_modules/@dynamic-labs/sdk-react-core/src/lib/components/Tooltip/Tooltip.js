...
const Tooltip = ({ children, content, className, as: Tag = 'span', copykey, targetRef, }) => {
    ...
    const [childrenWrapperRef, setChildrenWrapperRef] = useState(null);
    const effectiveRef = targetRef !== null && targetRef !== void 0 ? targetRef : childrenWrapperRef;
    /** Centers the tooltip to top-center of the target */
    const calcPosition = useCallback(() => {
        if (!effectiveRef)
            return;
        const childrenBounding = effectiveRef.getBoundingClientRect();
        setVector([
    ...
    useEffect(() => {
        ...
        return undefined;
    }, [effectiveRef, childrenHoverHandlers]);
    return (jsxs(Fragment, { children: [createPortal(jsx(ShadowDOM, { zIndex: tooltipZIndex, children: jsxs("div", { "data-testid": 'dynamicTooltip', className: classNames('dynamic-tooltip', {
                    ...
                    }, className), style: { left: vector[0], top: vector[1] }, copykey: copykey, children: [content, jsx(SvgTooltipArrow, { className: 'dynamic-tooltip__arrow' })] }) }), portalRoot.current), targetRef === undefined && (jsx(Tag, { ref: setChildrenWrapperRef, children: children }))] }));
};
