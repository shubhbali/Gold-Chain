...
const useSocialAccounts = ({ onError } = {}) => {
    ...
    // Provider-specific processing states
    const [processingStates, setProcessingStates] = useState(new Map());
    // Helper functions for provider-specific processing state
    const setProcessingForProvider = useCallback((provider, isProcessing) => {
        setProcessingStates((prev) => new Map(prev).set(provider, isProcessing));
    }, []);
    const isProcessingForProvider = useCallback((provider) => { var _a; return (_a = processingStates.get(provider)) !== null && _a !== void 0 ? _a : false; }, [processingStates]);
    const handleFarcasterUrl = useCallback((url) => pushView('farcaster-connect-view', { url }, {
        onBackClick: () => {
    ...
    const linkSocialAccount = useCallback((provider_1, ...args_1) => __awaiter(void 0, [provider_1, ...args_1], void 0, function* (provider, { forcePopup, redirectUrl, showWidgetAfterConnection, triggerFundFromExchangeOnSuccess, payingWithDynamic, } = {
        ...
        setProcessingForProvider(provider, true);
        try {
            ...
            setProcessingForProvider(provider, false);
        }
    }), [checkValidOAuthLinking, connectSocialAccount, setProcessingForProvider]);
    const findVerifiedCredentialById = useCallback((provider, verifiedCredentialId) => {
        var _a;
    ...
    const unlinkSocialAccount = useCallback((provider, verifiedCredentialId) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        handleError,
        setError,
        setProcessingForProvider,
        findVerifiedCredentialById,
    ]);
    ...
    const signInWithSocialAccount = useCallback((provider_2, ...args_2) => __awaiter(void 0, [provider_2, ...args_2], void 0, function* (provider, { redirectUrl, payingWithDynamic, showWidgetAfterConnection, triggerFundFromExchangeOnSuccess, } = {
        redirectUrl: undefined,
    }) {
        setProcessingForProvider(provider, true);
        try {
    ...
    }), [checkValidOAuthSignIn, connectSocialAccount, setProcessingForProvider]);
    // Maintain backwards compatibility for global isProcessing
    const isProcessingGlobal = useMemo(() => Array.from(processingStates.values()).some(Boolean), [processingStates]);
    return useMemo(() => ({
        error,
        ...
        getLinkedAccounts,
        isLinked,
        isProcessing: isProcessingGlobal, // Backwards compatible
        isProcessingForProvider, // New provider-specific function
        linkSocialAccount,
