...
class IframeManager {
   ...
   */ async initializeMessageTransport() {
        ...
        if (this.secureStorage) {
            this.iframeRequestChannel = this.setupSecureStorageHandlers(this.messageTransport);
        }
        await this.initAuthToken();
    }
    /**
   * Sets up message handlers for secureStorage operations from iframe
   */ setupSecureStorageHandlers(transport) {
   ...
   */ async initializeIframeDisplayForContainer({ container }) {
        try {
            ...
            // Set up secure storage handlers on this transport so the display iframe
            // can access key shares (e.g. for exportPrivateKey)
            if (this.secureStorage) {
                this.setupSecureStorageHandlers(transport);
            }
            var _this_authMode;
...
class DynamicWalletClient extends IframeManager {
    ...
    async signMessage({ message, accountAddress, password = undefined, signedSessionId, authToken, mfaToken, context, traceContext, bitcoinConfig }) {
    ...
    async signRawMessage({ message, accountAddress, password = undefined, signedSessionId, authToken, mfaToken, context, traceContext, bitcoinConfig }) {
   ...
   */ async signTransaction({ senderAddress, transaction, password = undefined, signedSessionId, authToken, mfaToken, chainId, traceContext, bitcoinConfig }) {
    ...
    async setPassword({ accountAddress, newPassword, signedSessionId, authToken, traceContext }) {
   ...
   */ async offlineExportPrivateKey({ keyShares, derivationPath, traceContext }) {
    ...
    // Note: secureStorage is already set by IframeManager constructor
    }
}
