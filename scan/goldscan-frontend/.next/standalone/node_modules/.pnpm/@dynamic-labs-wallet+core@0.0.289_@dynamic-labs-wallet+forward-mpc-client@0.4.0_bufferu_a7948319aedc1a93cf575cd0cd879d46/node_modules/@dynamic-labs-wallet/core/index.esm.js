...
const logger = new Logger('DynamicApiClient', LogLevel.DEBUG);
...
class BaseClient {
    ...
    sdkVersion, forwardMPCClient, logger: logger$1 }){
        this.logger = logger$1 != null ? logger$1 : logger;
        const headers = {};
...
class DynamicApiClient extends BaseClient {
    ...
    async exportKey({ mfaToken, elevatedAccessToken, dynamicRequestId, walletId, exportId, bitcoinConfig, onError, traceContext }) {
        const { addressType } = bitcoinConfig || {};
        const requestBody = _extends({
            exportId
        }, addressType ? {
    ...
    async recoverEncryptedBackupByWallet({ walletId, keyShareIds, signedSessionId, mfaToken, requiresSignedSessionId = false, traceContext }) {
        ...
        const dynamicRequestId = v4().replace('-', '');
        logger.info('[walletMap] recoverEncryptedBackupByWallet request', {
            context: {
                dynamicRequestId,
                walletId,
                keyShareIds,
