...
const GoldchainGovernance = () => {
  const { data, isError, isPlaceholderData, pagination } = useQueryWithPages({
    resourceName: 'general:goldchain_governance_events',
    options: {
      placeholderData: generateListStub<'general:goldchain_governance_events'>(
        {
          event_id: '0x',
          transaction_hash: '0x',
          log_index: 0,
          ...
          governance_actor_address_hash: null,
          route_asset: null,
          amount: null,
          token_id: null,
      ...
      { data.items.map((item, index) => (
        <Box key={ `${ item.event_id }-${ index }` } borderWidth="1px" borderRadius="md" p={ 3 }>
          ...
          <Flex alignItems="center" columnGap={ 3 } rowGap={ 2 } flexWrap="wrap" mt={ 2 }>
            ...
            { item.governance_actor_address_hash && (
              <AddressStringOrParam address={ item.governance_actor_address_hash } isLoading={ isPlaceholderData } noCopy truncation="constant_long"/>
            ) }
  ...
  return (
    <Box>
      ...
      <DataListDisplay
        ...
        emptyText="There are no governance events."
        actionBar={ actionBar }
      >
        { content }
      </DataListDisplay>
...
export default GoldchainGovernance;
