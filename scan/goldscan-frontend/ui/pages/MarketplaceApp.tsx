...
export default function MarketplaceApp() {
  ...
  const message = useMemo(() => ({
    goldscanColorMode: colorMode,
    goldscanRootUrl: config.app.baseUrl + route({ pathname: '/' }),
    goldscanAddressExplorerUrl: config.app.baseUrl + route({ pathname: '/address/[hash]', query: { hash: '' } }),
    goldscanTransactionExplorerUrl: config.app.baseUrl + route({ pathname: '/tx/[hash]', query: { hash: '' } }),
    goldscanNetworkName: config.chain.name,
    goldscanNetworkId: Number(config.chain.id),
    goldscanNetworkCurrency: config.chain.currency,
    goldscanNetworkRpc: config.chain.rpcUrls[0],
  }), [ colorMode ]);
