...
const CodeEditor = ({ data, remappings, libraries, language, mainFile, contractName }: Props) => {
  ...
  React.useEffect(() => {
    instance?.editor.setTheme(colorMode === 'light' ? 'goldscan-light' : 'goldscan-dark');
  ...
  const handleEditorDidMount = React.useCallback((editor: monaco.editor.IStandaloneCodeEditor, monaco: Monaco) => {
    ...
    monaco.editor.defineTheme('goldscan-light', themes.light);
    monaco.editor.defineTheme('goldscan-dark', themes.dark);
    monaco.editor.setTheme(colorMode === 'light' ? 'goldscan-light' : 'goldscan-dark');
