...
const RestApi = () => {
  ...
  const coreApiSwaggerUrl = React.useMemo(() => {
    ...
    if (releaseNumber && folderName) {
      return `https://raw.githubusercontent.com/goldchain-scan/swaggers/master/goldchain-scan/${ releaseNumber }/${ folderName }/swagger.yaml`;
    }
  ...
  const sections = React.useMemo(() => {
    ...
    return REST_API_SECTIONS.map((section) => {
      if (section.id === 'goldscan-core-api') {
        return {
          ...section,
          swagger: {
            ...section.swagger,
