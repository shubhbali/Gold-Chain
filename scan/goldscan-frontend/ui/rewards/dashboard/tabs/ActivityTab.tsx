...
function getUsageActivityRewards(rewardsData: unknown): Record<string, string> | undefined {
  if (!rewardsData || typeof rewardsData !== 'object') {
    return undefined;
  }

  const rewardsRecord = rewardsData as Record<string, unknown>;
  const preferred = rewardsRecord.goldscan_usage_activity_rewards;
  if (isStringRecord(preferred)) {
    return preferred;
  }

  const fallback = Object.entries(rewardsRecord).find(([ key, value ]) => {
    return key.endsWith('_usage_activity_rewards') && isStringRecord(value);
  })?.[1];

  return isStringRecord(fallback) ? fallback : undefined;
}
...
export default function ActivityTab() {
  ...
  const activities = useMemo(() => {
    ...
    return {
      ...
      usage: calcActivity('goldchain_scan_usage'),
    };
  }, [ activityQuery.data ]);
  ...
  const tasks = useMemo(() => (
    [
      {
        title: 'GoldScan activity',
        description: (
          <>
            Use GoldScan tools like{ ' ' }
            <Link
              external={ !marketplaceFeature?.essentialDapps }
              href={ marketplaceFeature?.essentialDapps ?
                route({ pathname: '/apps' }) :
                'https://eth.scan.giltchain.com/apps?utm_source=goldscan&utm_medium=transactions-task'
              }
            >
              Essential dapps
            </Link>, or{ ' ' }
      ...
      },
      {
        ...
        description: (
          <>
            ...
            on the GoldScan explorer to earn Merits.
          </>
        ),
      ...
      {
        title: 'GoldScan usage',
        description: (
          <>
            Use GoldScan explorers in your daily routine { mdash } check transactions, explore addresses,
            or add tokens/networks to MetaMask via GoldScan.
          </>
        ),
        ...
        maxAmount: getMaxAmount(getUsageActivityRewards(rewardsConfigQuery.data?.rewards)),
      },
    ]
        ...
        >
          <Flex flexDirection="column" p={{ base: 1.5, md: 0 }} pb={ 0 }>
            ...
            <Text textStyle="sm" mb={{ base: 2, md: 4 }}>
              Use GoldScan and related products daily to earn Merits. Check each task for details and how to get started.
            </Text>
            <Flex alignItems="center" gap={ 3 } mb={{ base: 0, md: 4 }}>
              ...
              <Link
                external
                href="https://docs.scan.giltchain.com/using-goldscan/merits/activity-pass"
                fontSize="md"
                fontWeight="500"
                textAlign="center"
              >
        ...
        >
          ...
          >
            <Text textStyle="xs" color="text.secondary" fontWeight="500">
              ...
              <Link external href="https://discord.gg/giltchain">
                Discord
              </Link>
            </Text>
          </Flex>
