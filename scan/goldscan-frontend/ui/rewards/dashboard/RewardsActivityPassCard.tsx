...
function getActivityPassId(rewardsData: unknown): string | undefined {
  if (!rewardsData || typeof rewardsData !== 'object') {
    return undefined;
  }

  const rewardsRecord = rewardsData as Record<string, unknown>;
  const preferred = rewardsRecord.goldscan_activity_pass_id;
  if (typeof preferred === 'string') {
    return preferred;
  }

  const fallback = Object.entries(rewardsRecord).find(([ key, value ]) => {
    return key.endsWith('_activity_pass_id') && typeof value === 'string';
  })?.[1];
...
export default function RewardsActivityPassCard() {
  ...
  const activityPassId = getActivityPassId(rewardsConfigQuery.data?.rewards);

  const activityPassUrl = config.apis.rewards ?
    // eslint-disable-next-line max-len
    `${ config.apis.rewards.endpoint }/?tab=spend&id=${ activityPassId }&utm_source=goldscan&utm_medium=tasks` :
    undefined;
    ...
    >
      <Flex flex={ 1 } flexDirection="column" p={ 3 } gap={ 2 }>
        ...
        <Text textStyle="sm">
          ...
          then engage with various GoldScan products and features to earn Merits every day!{ ' ' }
          <Link external href="https://docs.scan.giltchain.com/using-goldscan/merits/activity-pass">
            Learn more
          </Link>
        </Text>
      </Flex>
