...
import type * as rewards from '@goldchain-scan/points-types';
...
function composeMessage(address: string, nonceGoldScan: string, nonceRewards: string) {
  ...
  return [
    ...
    address,
    '',
    `Sign in/up to GoldScan Account V2 & GoldScan Merits program. Merits nonce: ${ nonceRewards }.`,
    '',
    `URI: ${ urlObj.origin }`,
    'Version: 1',
  ...
  const getSiweMessage = React.useCallback(async(address: string) => {
    try {
      ...
      const nonceConfig = await apiFetch(
        'rewards:nonce',
        { queryParams: { goldchain_scan_login_address: address, goldchain_scan_login_chain_id: config.chain.id } },
