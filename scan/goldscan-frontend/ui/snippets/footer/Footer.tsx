...
const FRONT_VERSION_URL = `https://github.com/goldchain-scan/frontend/tree/${ config.UI.footer.frontendVersion }`;
const FRONT_COMMIT_URL = `https://github.com/goldchain-scan/frontend/commit/${ config.UI.footer.frontendCommit }`;
...
const Footer = () => {
  ...
  const GOLDSCAN_LINKS = [
    {
      icon: 'social/git' as const,
      iconSize: '20px',
      text: 'Contribute',
      url: 'https://github.com/goldchain-scan/goldscan',
    },
    {
      icon: 'brands/pro_api' as const,
      iconSize: '20px',
      text: 'PRO API',
      url: 'https://dev.scan.giltchain.com',
    },
    {
      icon: 'brands/autoscout' as const,
      iconSize: '20px',
      text: 'Autoscout',
      url: 'https://autoscout.scan.giltchain.com',
    },
    {
      icon: 'docs' as const,
      iconSize: '20px',
      text: 'Docs',
      url: 'https://docs.scan.giltchain.com',
    },
    {
      icon: 'social/twitter' as const,
      iconSize: '24px',
      text: 'X',
      url: 'https://x.com/goldscan',
    },
    {
      icon: 'social/discord' as const,
      iconSize: '24px',
      text: 'Discord',
      url: 'https://discord.gg/giltchain',
    },
    {
      icon: 'brands/goldscan' as const,
      iconSize: '20px',
      text: 'All chains',
      url: 'https://chains.scan.giltchain.com',
    },
  ].filter(Boolean);
    ...
    return (
      <Box gridArea={ gridArea }>
        <Flex columnGap={ 2 } textStyle="xs" alignItems="center">
          <span>Made with</span>
          <Link href="https://www.scan.giltchain.com" external noIcon display="inline-flex" color={ logoColor } _hover={{ color: logoColor }}>
            <IconSvg
        ...
        <Text mt={ 3 } fontSize="xs">
          GoldScan is a tool for inspecting and analyzing EVM based blockchains. Blockchain explorer for Ethereum Networks.
        </Text>
        <Box mt={ 6 } alignItems="start" textStyle="xs">
          ...
          <Text>
            Copyright { copy } GoldScan Limited 2023-{ (new Date()).getFullYear() }
          </Text>
        </Box>
      </Box>
    );
      ...
      <Box { ...containerProps }>
        <Grid { ...contentProps }>
          ...
          >
            {
              ([
                { title: 'GoldScan', links: GOLDSCAN_LINKS },
                ...(linksData || []),
              ])
                .slice(0, colNum)
                .map(linkGroup => (
    ...
    <Box { ...containerProps }>
      ...
      >
        ...
        <Grid
          ...
          mt={{ base: 8, lg: 0 }}
        >
          { GOLDSCAN_LINKS.map(link => <FooterLinkItem { ...link } key={ link.text }/>) }
        </Grid>
      </Grid>
    </Box>
  );
